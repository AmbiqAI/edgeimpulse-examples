...
{
	metal_assert(mutex);
	mutex->m = xSemaphoreCreateRecursiveMutex();
...
{
	metal_assert(mutex && mutex->m);
	return xSemaphoreTakeRecursive(mutex->m, (TickType_t)0);
}

static inline void __metal_mutex_acquire(metal_mutex_t *mutex)
{
	metal_assert(mutex && mutex->m);
	xSemaphoreTakeRecursive(mutex->m, portMAX_DELAY);
}

static inline void __metal_mutex_release(metal_mutex_t *mutex)
{
	metal_assert(mutex && mutex->m);
	xSemaphoreGiveRecursive(mutex->m);
}
