...
/*    Name:             ADXL362.h (library)                                   */
/*    Version:          3.0                                                   */
/*    Description:      This library describes functions user can use to      */
...
#include "stdint.h"
extern uint8_t g_ADXLDataReady;
/******************************************************************************/
/*                    PROTOTYPE FUNCTIONS                                     */
/******************************************************************************/
              ...
              void getXYZTdata12B_ADXL362(int16_t *_X, int16_t *_Y, int16_t *_Z, int16_t *_T);
              void setSOFT_RESET_ADXL362(void);
              void setActivityAndInactivity_ADXL362(uint8_t control_byte, uint16_t thresholdAct, uint8_t timeAct, uint16_t thresholdInact, uint16_t timeInact);
              void getActivityAndInactivity_ADXL362(uint8_t *control_byte, uint16_t *thresholdAct, uint8_t *timeAct, uint16_t *thresholdInact, uint16_t *timeInact);
              void setFIFOConf_ADXL362(uint8_t temperature, uint8_t FIFOmode, uint16_t FIFOsamples);
              void getFIFOConf_ADXL362(uint8_t *temperature, uint8_t *FIFOmode, uint16_t *FIFOsamples);
              ...
              void getFIFO(uint16_t _lengthR, uint8_t *_arrayR);
              uint8_t getFIFOXYZ(uint16_t _lengthR, int16_t *_X, int16_t *_Y, int16_t *_Z);
              void getFIFOXYZT(uint16_t _lengthR, int16_t *_X, int16_t *_Y, int16_t *_Z, int16_t *_T);

              void setRegisterADXL362(uint8_t _reg, uint8_t _value);
              void getRegisterADXL362( uint8_t _reg, uint16_t _lengthR, uint8_t *_arrayR);
...
#define PRM_HALF_BW_ENABLE        ((0x1)<<4)                                    //When '1', the bandwidth of the anti-aliasing filters is set to 1/4 the output data rate (ODR) for more conservative filtering.
#define PRM_HALF_BW_DISABLE       ((0x0)<<4)                                    //When '0', the bandwidth of the filters is set to 1/2 the ODR for a wider bandwidth.
...
extern int16_t ADXLDrv_OpenDriver(uint8_t inrate, uint8_t watermark);
extern int16_t ADXLDrv_CloseDriver(void);
extern int16_t ADXLDrv_ReadData(uint8_t pollMode, int16_t *rxData, uint32_t *time);
extern uint16_t ADXLDrv_SelfTest(void);
#endif // ADXL362.h
