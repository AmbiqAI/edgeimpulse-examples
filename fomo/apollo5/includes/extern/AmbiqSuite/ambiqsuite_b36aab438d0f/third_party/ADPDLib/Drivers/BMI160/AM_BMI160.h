...
/*    Name:             BMI160.h (library)                                   */
/*    Version:          3.0                                                   */
/*    Description:      This library describes functions user can use to      */
/*                      control the accelerometer called BMI160. User must   */
...
void getXYZTdata12B_BMI160(int16_t *_X, int16_t *_Y, int16_t *_Z, int16_t *_T);
void setSOFT_RESET_BMI160(void);
void setActivityAndInactivity_BMI160(uint8_t control_byte, uint16_t thresholdAct, uint8_t timeAct, uint16_t thresholdInact, uint16_t timeInact);
void getActivityAndInactivity_BMI160(uint8_t *control_byte, uint16_t *thresholdAct, uint8_t *timeAct, uint16_t *thresholdInact, uint16_t *timeInact);
void setFIFOConf_BMI160(uint8_t temperature, uint8_t FIFOmode, uint16_t FIFOsamples);
void getFIFOConf_BMI160(uint8_t *temperature, uint8_t *FIFOmode, uint16_t *FIFOsamples);
...
void getFIFO(uint32_t _lengthR, uint16_t *_arrayR);
...
void getRegisterBMI160( uint8_t _reg, uint16_t _lengthR, uint8_t *_arrayR);
...
#define REG_STATUS                0x0B                                          //This register includes the following bits that describe various conditions of the BMI160.
...
#define REG_ACT_INACT_CTL         0x27                                          //This register controls the BMI160's ACTIVITY/INACTIVITY.
#define REG_FIFO_CONTROL          0x28                                          //This register controls the BMI160's FIFO MODE.
...
#define REG_FILTER_CTL            0x2C                                          //This register controls the BMI160's filter.
#define REG_POWER_CTL             0x2D                                          //This registre controls BMI160's status.
#define REG_SELF_TEST             0x2E                                          //This registre enables the BMI160's self test
/******************************************************************************/
...
/******************************************************************************/
#define PRM_ERR_USER_REGS_MASK    ((0x1)<<7)                                    //SEU Error Detect. A '1' indicates one of two things: either an SEU event, such as an alpha particle of power glitch, has disturbed a user register setting; or the BMI160 is unconfigured.
...
#define PRM_INACT_REF_REF_MODE    ((0x1)<<3)                                    //Referenced / Absolute Inactivity Select. When '1', the Inactivity detection function operates in Referenced Mode. When '1', the Inactivity detection function operates in Absolute Mode.
...
extern int16_t BMIDrv_OpenDriver(uint8_t inrate, uint8_t watermark);
extern int16_t BMIDrv_CloseDriver(void);
extern int16_t BMIDrv_ReadData(uint8_t pollMode, int16_t *rxData, uint32_t *time);
extern uint16_t BMIDrv_SelfTest(void);
