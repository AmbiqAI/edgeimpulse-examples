 ...
 *            image file.
 * @nflag: pointer to next flag of segment. The flag has PF_X, PF_W and PF_R
 *
...
int elf_load(struct remoteproc *rproc, const void *img_data,
	     ...
	     size_t *noffset, size_t *nlen,
	     unsigned char *padding, size_t *nmemsize, unsigned int *nflag);
...
/**
 * elf_load_section - load section data
 *
 * It will parse the ELF image and return the target device address,
 * offset to the start of the ELF image of the section data to load and the
 * length of the section data to load.
 *
 * @img_info: pointer to ELF image information
 * @name: pointer to section name
 ...
 * @name: pointer to section flag. The flag has SHF_WRITE, SHF_ALLOC and SHF_EXECINSTR
 *
 * return 0 if successfully load section data, negative value for
 * failure.
 */
int elf_load_section(void *elf_info, const char *name, metal_phys_addr_t *da,
                size_t *offset, size_t *size, unsigned int *flags);
...
int elf_parse(void *img_data, void **img_info);
int elf_load_ph_header(void *img_data, void **img_info, int *ph_number);
int elf_parse_ph_header(void **img_info, int index, unsigned int *ph_type, size_t *ph_offset,
                    metal_phys_addr_t *ph_vaddr, metal_phys_addr_t *ph_paddr, size_t *ph_fsize,
                    size_t *ph_msize, unsigned int *ph_flag, unsigned int *ph_align);
int elf_parse_section_from_name(void *img_data, void **img_info, const char *s_name,
                unsigned int *s_type, unsigned int *s_flags, metal_phys_addr_t *s_addr,
                size_t *s_offset, size_t *s_size, unsigned int *s_align);
#if defined __cplusplus
}
#endif
