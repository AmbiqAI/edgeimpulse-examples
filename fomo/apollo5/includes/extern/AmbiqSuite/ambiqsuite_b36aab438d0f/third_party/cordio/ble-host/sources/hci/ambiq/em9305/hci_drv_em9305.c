...
HciDrvRadioBoot(bool bColdBoot)
{
    ...
    am_hal_gpio_interrupt_register(AM_HAL_GPIO_INT_CHANNEL_0, IntNum, (am_hal_gpio_handler_t)HciDrvIntService, NULL);
    am_devices_em9305_enable_interrupt();
#ifdef AM_IRQ_PRIORITY_DEFAULT
    NVIC_SetPriority(GPIO0_809F_IRQn, AM_IRQ_PRIORITY_DEFAULT);
#endif // AM_IRQ_PRIORITY_DEFAULT
    NVIC_EnableIRQ(GPIO0_809F_IRQn);
#else
    am_hal_gpio_interrupt_register(AM_BSP_GPIO_EM9305_INT, HciDrvIntService);
    am_devices_em9305_enable_interrupt();
    NVIC_EnableIRQ(GPIO0_809F_IRQn);
#endif
    return ui32Status;
...
HciDrvHandler(wsfEventMask_t event, wsfMsgHdr_t *pMsg)
{
    ...
    while (ui32NumHciTransactions++ < HCI_DRV_MAX_HCI_TRANSACTIONS)
    {
        ...
        //
        if ( am_hal_gpio_input_read(AM_BSP_GPIO_EM9305_INT) )
        {
            ...
            if ( ui32ErrorStatus == AM_DEVICES_EM9305_STATUS_SUCCESS)
            {
                ...
                {
                    if (am_hal_gpio_input_read(AM_BSP_GPIO_EM9305_INT) == 0 || g_ui32InterruptsSeen != ui32OldInterruptsSeen)
                    {
                        break;
                    }
...
{
    HciVendorSpecificCmd(0xFC43, 6, g_BLEMacAddress);
}
/*************************************************************************************************/
/*!
