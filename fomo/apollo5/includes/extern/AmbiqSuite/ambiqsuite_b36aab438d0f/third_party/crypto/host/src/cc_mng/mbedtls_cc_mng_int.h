...
extern uint32_t am_hal_otp_read_word(uint32_t offset, uint32_t *pVal);
#define CC_READ_OTP_WORD(otpAddr, otpData)							\
	do { 																				\
        uint32_t _otp_read_data;                        \
        am_hal_otp_read_word(otpAddr, &_otp_read_data); \
        otpData = _otp_read_data;                       \
	}while(0)
#elif defined(DX_PLAT_APOLLO5)
extern uint32_t am_hal_infoc_read_word(uint32_t offset, uint32_t *pVal);
#define CC_READ_OTP_WORD(otpAddr, otpData)              \
    do {                                                \
        uint32_t _otp_read_data;                        \
        am_hal_infoc_read_word(otpAddr, &_otp_read_data); \
        otpData = _otp_read_data;                       \
    }while(0)
#endif
