...
#define HFRC_BASE (APOLLO5_FPGA*1000000)
#define XT_BASE   32768
#define LFRC_BASE 1024
#define PLL_250MHZ_BASE   250000000 / 12
#define RF_XTAL_48MHZ     48000000 / 12
#elif defined(AM_PART_APOLLO4)
#define HFRC_BASE 48000000
#define XT_BASE   32768
#define LFRC_BASE 1024
#elif defined(AM_PART_APOLLO4B) || defined(AM_PART_APOLLO4P) || defined(AM_PART_APOLLO4L) || defined(AM_PART_BRONCO) || defined(AM_PART_APOLLO5A)
...
#define XT_BASE   32768
#define LFRC_BASE 1024
#define HFRC2_125MHZ_BASE 125000000
#define XTAL_HS_BASE 32000000
#elif defined(AM_PART_APOLLO510L)
#define HFRC_BASE 96000000
#define XT_BASE   32768
#define LFRC_BASE 1024
#define PLL_250MHZ_BASE   250000000
#define RF_XTAL_48MHZ     48000000
#endif

#if defined(AM_PART_APOLLO5B)
#define TEST_CLKSRC_ITEM_COUNT  AM_HAL_TIMER_CLOCK_XTAL_HS_DIV4 + 1
#elif defined(AM_PART_APOLLO510L)
#define TEST_CLKSRC_ITEM_COUNT  AM_HAL_TIMER_CLOCK_RF_XTAL_DIV4 + 1
#else
#define TEST_CLKSRC_ITEM_COUNT  AM_HAL_TIMER_CLOCK_RTC_100HZ + 1
#endif
...
struct
{
    uint32_t    lower;
    uint32_t    upper;
} am_widget_timer_1sec_count[TEST_CLKSRC_ITEM_COUNT] =
{
    // Lower Bound Pass         Upper Bound Pass
//  { HFRC_BASE / 4    * 95 / 100 - 5, HFRC_BASE / 4    * 105 / 100},
//  Change order of operations of above line to avoid int overflow
	{ HFRC_BASE / 4    / 100 * 95 - 5, HFRC_BASE / 4    / 100 * 105},
    ...
    ,
    { 481,                             537 },
    { HFRC2_125MHZ_BASE / 8      * 95 / 100 - 5, HFRC2_125MHZ_BASE / 8      * 105 / 100},
    { HFRC2_125MHZ_BASE / 16     * 95 / 100 - 5, HFRC2_125MHZ_BASE / 16     * 105 / 100},
    { HFRC2_125MHZ_BASE / 32     * 95 / 100 - 5, HFRC2_125MHZ_BASE / 32     * 105 / 100},
    { HFRC2_125MHZ_BASE / 64     * 95 / 100 - 5, HFRC2_125MHZ_BASE / 64     * 105 / 100},
    { HFRC2_125MHZ_BASE / 128    * 95 / 100 - 5, HFRC2_125MHZ_BASE / 128    * 105 / 100},
    { HFRC2_125MHZ_BASE / 256    * 95 / 100 - 5, HFRC2_125MHZ_BASE / 256    * 105 / 100},
//  Change order of operations of above line to avoid int overflow
    { XTAL_HS_BASE               / 100 * 95 - 5, XTAL_HS_BASE               / 100 * 105},
    { XTAL_HS_BASE / 2           * 95 / 100 - 5, XTAL_HS_BASE / 2           * 105 / 100},
    { XTAL_HS_BASE / 4           * 95 / 100 - 5, XTAL_HS_BASE / 4           * 105 / 100}
#endif
#else
    { 0,                               LFRC_BASE / 1024 * 105 / 100},
    { XT_BASE          * 95 / 100 - 5, XT_BASE          * 105 / 100},
    ...
    { XT_BASE / 16     * 95 / 100 - 5, XT_BASE / 16     * 105 / 100},
    { XT_BASE / 32     * 95 / 100 - 5, XT_BASE / 32     * 105 / 100},
    { XT_BASE / 1024    * 95 / 100 - 5, XT_BASE / 1024    * 105 / 100},
    { 93,                              102 },
    { XT_BASE / 64     * 95 / 100 - 5, XT_BASE / 64     * 105 / 100},
    { XT_BASE / 128     * 95 / 100 - 5, XT_BASE / 128     * 105 / 100},

// PLL clock source
    { PLL_250MHZ_BASE             * 95 / 100 - 5, PLL_250MHZ_BASE          * 105 / 100},
    #warning:FIXME:need to check the divide setting of PLL clock
    { PLL_250MHZ_BASE             * 95 / 100 - 5, PLL_250MHZ_BASE          * 105 / 100},
    { PLL_250MHZ_BASE / 4         * 95 / 100 - 5, PLL_250MHZ_BASE / 4      * 105 / 100},
    { PLL_250MHZ_BASE / 8         * 95 / 100 - 5, PLL_250MHZ_BASE / 8      * 105 / 100},
    { PLL_250MHZ_BASE / 16        * 95 / 100 - 5, PLL_250MHZ_BASE / 16     * 105 / 100},

// RF XTAL clock source
    { RF_XTAL_48MHZ               / 100 * 95 - 5, RF_XTAL_48MHZ               / 100 * 105},
    { RF_XTAL_48MHZ / 2           * 95 / 100 - 5, RF_XTAL_48MHZ / 2           * 105 / 100},
    { RF_XTAL_48MHZ / 4           * 95 / 100 - 5, RF_XTAL_48MHZ / 4           * 105 / 100}
#endif
};
                            ...
                            am_hal_timer_clock_e eTimerClock)
{
    ...
    // 
    // Configure the RTC clock source for XT: 
    // CLKGEN registers are only reset on POI, so there is a 
    // possibility this will be configured as LFRC if not 
    // explicitly configured for XT.
    //
    am_hal_rtc_config_t sRTCConfig =
    {
        .eOscillator = AM_HAL_RTC_OSC_XT,
    };
    am_hal_rtc_config(&sRTCConfig);
                         ...
                         am_hal_timer_clock_e eTimerClock)
{
    ...
    for (uint8_t j = 0; j < ui8FreqIndex; j++)
    {
        ...
        for (uint8_t i = 0; i < sizeof(g_ui8DutyCycle); i++)
        {
            ...
            am_util_stdio_printf("\ncompare0:%d, compare1:%d, Duty cycle %d\n", TimerConfig.ui32Compare0, TimerConfig.ui32Compare1, g_ui8DutyCycle[i]);
                                        ...
                                        am_hal_timer_clock_e eTimerClock)
{
    ...
    {
        uint32_t    inputClock;
    } am_widget_timer_clock[TEST_CLKSRC_ITEM_COUNT] =
    {
        ...
        { 512                       },
        { HFRC2_125MHZ_BASE / 8     },
        { HFRC2_125MHZ_BASE / 16    },
        { HFRC2_125MHZ_BASE / 32    },
        { HFRC2_125MHZ_BASE / 64    },
        { HFRC2_125MHZ_BASE / 128   },
        { HFRC2_125MHZ_BASE / 256   },
        { XTAL_HS_BASE              },
        { XTAL_HS_BASE / 2          },
        { XTAL_HS_BASE / 4          }
#endif
