...
} am_widget_timer_1sec_count[AM_HAL_TIMER_CLOCK_RTC_100HZ+1] =
{
    ...
    { LFRC_BASE        * 95 / 100 - 5, LFRC_BASE        * 105 / 100},
    { LFRC_BASE / 2    * 95 / 100 - 5, LFRC_BASE / 2    * 105 / 100},
    { LFRC_BASE / 32   * 95 / 100 - 5, LFRC_BASE / 32   * 105 / 100},
                           ...
                           am_hal_timer_clock_e eTimerClock)
{
    ...
    // Set the Compare1 register to the clock value if the calculated value is zero. 
    
    if (!TimerConfig.ui32Compare1)
    {
      TimerConfig.ui32Compare1 = am_widget_timer_1sec_count[eTimerClock].upper;
    }
    ...
    if ( (ui32TimerCount >= am_widget_timer_1sec_count[eTimerClock].lower) &&
             (ui32TimerCount <= am_widget_timer_1sec_count[eTimerClock].upper) + 1)
    {
      am_util_stdio_printf(".");
      return true;
    }
                                        ...
                                        am_hal_timer_clock_e eTimerClock)
{
    ...
    // 
    // The variance is kept at 2% implying 98%-102%. 
    // 
    // Note:
    //
    ...
    if ((int32_t)((((am_widget_timer_clock[eTimerClock].inputClock / (SEC_TO_MICROSEC/DELAY_MICROSEC)) / sSourceTimerConfig.ui32Compare0) * 98/100) - 4) < 1)
    {
        ui32Timer1SecLowerBound = 0;
    }
    else
    {      
        ui32Timer1SecLowerBound = (uint32_t)((((am_widget_timer_clock[eTimerClock].inputClock / (SEC_TO_MICROSEC/DELAY_MICROSEC)) / sSourceTimerConfig.ui32Compare0) * 98/100) - 4);
    }
        
    ui32Timer1SecUpperBound = ((((am_widget_timer_clock[eTimerClock].inputClock / (SEC_TO_MICROSEC/DELAY_MICROSEC)) / sSourceTimerConfig.ui32Compare0) * 102/100));
