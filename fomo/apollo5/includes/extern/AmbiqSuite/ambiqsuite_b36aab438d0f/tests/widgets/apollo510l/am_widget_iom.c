...
#if 1
static uint8_t g_pucInBuffer[(MAX_TRANS_SIZE+4)*2];     // RX buffer at TCM
static uint8_t g_pucOutBuffer[(MAX_TRANS_SIZE+4)*2];    // TX buffer at TCM
#else
AM_SHARED_RW uint8_t g_pucInBuffer[(MAX_TRANS_SIZE+4)*2] __attribute__((aligned(32)));  // RX buffer at SSRAM
AM_SHARED_RW uint8_t g_pucOutBuffer[(MAX_TRANS_SIZE+4)*2] __attribute__((aligned(32))); // TX buffer at SSRAM
#endif
...
{
    uint8_t  devName[20];
    uint32_t (*iom_device_init)(uint32_t ui32Module, am_widget_iom_devices_config_t *pDevConfig, void **ppHandle, void **ppIomHandle);
...
//
//*****************************************************************************
bool g_bIOMNonBlockCompleteWr;
bool g_bIOMNonBlockCompleteRd;
...
uint32_t
am_devices_loop_init(uint32_t ui32Module, am_widget_iom_devices_config_t *pDevConfig, void **ppHandle, void **ppIomHandle)
{
    void *pIomHandle;
...
//
//*****************************************************************************
bool am_widget_iom_test_setup(am_widget_iom_config_t *pTestCfg,
                                  void **ppWidget, char *pErrStr)
{
    ...
    *ppWidget = &g_IomSpiWidget;
    ...
    if( ui32Status != IOM_WIDGET_DEVICE_SUCCESS )
    {
      return false;
    }
...
uint32_t am_widget_iom_test_blocking_write_read(void *pWidget, void *pCfg, char *pErrStr)
{
...
#else
    for (uint32_t i = 0; i < pTestCfg->ui32NumBytes; i++)
    {
        g_pucOutBuffer[pTestCfg->ui32ByteOffset + i] = (i & 0xFF);
    }
#endif
    //
    // Write the buffer to the FRAM device.
    //
    ui32Status = iom_device_func.iom_device_blocking_write(g_IomDevHdl, &g_pucOutBuffer[pTestCfg->ui32ByteOffset], 0, pTestCfg->ui32NumBytes);

    if (ui32Status != IOM_WIDGET_DEVICE_SUCCESS)
    {
        am_util_stdio_printf("Failed to blocking write . Status = %d\n",ui32Status);
        return AM_WIDGET_ERROR;
    }
    // ui32Status = am_widget_iom_power_saverestore(pWidget, IOM_TEST_POWERSAVE_MODE);
    // if (ui32Status != IOM_WIDGET_DEVICE_SUCCESS)
    // {
    //     return AM_WIDGET_ERROR;
    // }
    ...
    //
    ui32Status = iom_device_func.iom_device_blocking_read(g_IomDevHdl, &g_pucInBuffer[pTestCfg->ui32ByteOffset], 0, pTestCfg->ui32NumBytes);
    if (ui32Status != IOM_WIDGET_DEVICE_SUCCESS)
    {
        am_util_stdio_printf("Failed to blocking read . Status = %d\n",ui32Status);
        return AM_WIDGET_ERROR;
    }
...
am_widget_iom_test_nonblocking_write_read(void *pWidget, void *pCfg, char *pErrStr)
{
    ...
    for ( uint8_t i = 0; i < (sizeof(g_IOM_SpeedMax) / sizeof(g_IOM_SpeedMax[0])); i++ )
    {
        if ( pWidgetCfg->IOMConfig.ui32ClockFreq == g_IOM_SpeedMax[i].MHz )
        {
            ...
            if ( pTestCfg->ui32NumBytes > ui32Limit )
            {
                pTestCfg->ui32NumBytes = ui32Limit - (AM_HAL_IOM_MAX_TXNSIZE_SPI - pTestCfg->ui32NumBytes);
            }
        }
    }
#endif
    ...
    // Write the buffer to the FRAM device.
    //
    g_bIOMNonBlockCompleteWr = false;
    ui32Status = iom_device_func.iom_device_nonblocking_write(g_IomDevHdl,
                                                               &g_pucOutBuffer[pTestCfg->ui32ByteOffset],
                                                              0,
                                                              pTestCfg->ui32NumBytes,
                                                              pfnWidget_IOM_Callback_Write,
                                                              0);
    if (ui32Status != IOM_WIDGET_DEVICE_SUCCESS)
    {
        am_util_stdio_printf("Failed to non blocking write . Status = %d\n",ui32Status);
        return AM_WIDGET_ERROR;
    }
    ...
    //
    while(g_bIOMNonBlockCompleteWr == false)
    {
        am_hal_sysctrl_sleep(pTestCfg->sleepMode);
    }
    ...
    // Read the buffer from the FRAM device.
    //
    g_bIOMNonBlockCompleteRd = false;
    ui32Status = iom_device_func.iom_device_nonblocking_read(g_IomDevHdl,
                                                            &g_pucInBuffer[pTestCfg->ui32ByteOffset],
                                                            0,
                                                            pTestCfg->ui32NumBytes,
                                                            pfnWidget_IOM_Callback_Read,
                                                            0);

    if (ui32Status != IOM_WIDGET_DEVICE_SUCCESS)
    {
        am_util_stdio_printf("Failed to non blocking read . Status = %d\n",ui32Status);
        return AM_WIDGET_ERROR;
    }
    ...
    //
    while(g_bIOMNonBlockCompleteRd == false)
    {
        am_hal_sysctrl_sleep(pTestCfg->sleepMode);
    }
...
am_widget_iom_test_queue_write_read(void *pWidget, void *pCfg, char *pErrStr)
{
    ...
    uint32_t blockSize               = pTestCfg->ui32NumBytes / (pTestCfg->ui32QueueSize / 8) + 1;
    uint32_t ui32Status = 0;

    //
    // Create the transmit buffer to write.
    //
    load_buffers(ui32Offset, pTestCfg->ui32NumBytes, 0);

    am_hal_sysctrl_bus_write_flush();

    uint32_t ui32Bytes = 0;
    while ((0 == ui32Status) && (ui32Bytes < pTestCfg->ui32NumBytes))
    {
        uint32_t size = (blockSize < (pTestCfg->ui32NumBytes - ui32Bytes)) ? blockSize : (pTestCfg->ui32NumBytes - ui32Bytes);
    ...
    if (0 == ui32Status)
    {
        ...
        ui32Bytes = 0;
        while ((ui32Status == 0) && (ui32Bytes < pTestCfg->ui32NumBytes))
        {
            uint32_t size = (blockSize < (pTestCfg->ui32NumBytes - ui32Bytes)) ? blockSize : (pTestCfg->ui32NumBytes - ui32Bytes);
            ui32Status = iom_device_func.iom_device_nonblocking_read(g_IomDevHdl, &g_pucInBuffer[0] + ui32Offset + ui32Bytes, ui32Bytes, size,
                                       (size == pTestCfg->ui32NumBytes - ui32Bytes) ? pfnWidget_IOM_Callback_Read : 0, 0);
            ui32Bytes += size;
        }
    }
    ...
    // Compare the receive buffer to the transmit buffer.
    //
    if (ui32Status || !compare_buffers(ui32Offset, pTestCfg->ui32NumBytes, 0) )
    {
        return AM_WIDGET_ERROR;
    }
    else
...
uint32_t am_widget_fullduplex_spi_iom_setup(am_widget_iom_config_t *pTestCfg,
                                  void **ppWidget, char *pErrStr)
{
    uint32_t ui32Status = 0;
    ...
    *ppWidget = &g_IomSpiWidget;
    ...
    // Full duplex only support non-DMA mode
    am_hal_iom_interrupt_disable(g_pIOMHandle, 0xFF);
    NVIC_DisableIRQ(IOMSTR0_IRQn + g_IomSpiWidget.ui32Module);

    // am_hal_pwrctrl_periph_disable((am_hal_pwrctrl_periph_e)(AM_HAL_PWRCTRL_PERIPH_IOM0 + IOM_DUPLEX_TEST_MODULE));

    // Set up the host IO interrupt
    uint32_t IntNum = HANDSHAKE_IOM_PIN;
    am_hal_gpio_pinconfig(HANDSHAKE_IOM_PIN, g_AM_BSP_GPIO_HANDSHAKE);
...
uint32_t
am_widget_fullduplex_spi_iom_test(void *pWidget, void *pCfg, char *pErrStr)
{
    am_widget_iom_test_t          *pTestCfg = (am_widget_iom_test_t *)pCfg;
    uint32_t                      ui32Status;
    uint32_t ui32Offset           = pTestCfg->ui32ByteOffset & 0x3;
    ...
    // Initialize the buffer section which will be used - to initialize out and in different
    for (ix = 0; ix < pTestCfg->ui32NumBytes; ix++)
    {
    ...
    AM_WIDFET_DEBUG_PRINTF("\nStep 1 :Enable the INTEN until receive the handshake signal from SLAVE\n");


    uint32_t char_count = 0;
    while( !bHandshake ){
        // HOST Enable the IOINTEN  for the slave device to initiate a handshake
        data = 0xFF;
    ...
    am_util_stdio_printf("\n");
    // if the handshake is set, Read the IOINT status to check for the handshake signal from the slave
    iom_slave_read(IOSOFFSET_READ_INTSTAT, &data, 1);
    if (! (data & HANDSHAKE_IOS_TO_IOM) )
    {
        am_util_stdio_printf("The INSTAT status read by the host does not match that slave set");

        return AM_WIDGET_ERROR;
    }

    //the delay to wait for the slave to be ready to receive data.
    am_util_delay_ms(10);
    ...
    Transaction.ui32NumBytes    = pTestCfg->ui32NumBytes;
    ...
    //
    AM_WIDFET_DEBUG_PRINTF("Step 2: Start read and write data to SLAVE\n");

    ui32Status = am_hal_iom_spi_blocking_fullduplex(g_pIOMHandle, &Transaction);
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
        am_util_stdio_printf("Full Duplex transaction fail\n");
    }

    AM_WIDFET_DEBUG_PRINTF("Step 3: Compare the receive buffer to the transmit buffer\n");
    //
    // Compare the receive buffer to the transmit buffer.
    //
    if ( !compare_buffers(ui32Offset, pTestCfg->ui32NumBytes, 0) )
    {
        ret = AM_WIDGET_ERROR;
    }
    else
    {
        ret = AM_WIDGET_SUCCESS;
    }

    // Clear IOINT status after transaction completes
    AM_WIDFET_DEBUG_PRINTF("Step 4: Clear IOINT status and IOINTEN after transaction complete\n");
...
uint32_t am_widget_fullduplex_spi_iom_cleanup(void *pWidget, char *pErrStr)
{
    uint32_t ui32Status = 0;
    //am_widget_iom_config_t        *pTestCfg = (am_widget_iom_config_t *)pWidget;
