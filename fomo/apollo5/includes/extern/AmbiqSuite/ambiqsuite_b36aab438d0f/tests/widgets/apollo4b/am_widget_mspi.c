...
#ifdef AM_WIDGET_MSPI_HEX_DDR_PSRAM_DEVICE
//#define TEST_HALF_SLEEP
#endif
...
mspi_device_func_t mspi_device_func =
{
    ...
    .mspi_init_timing_check = am_devices_mspi_psram_aps12808l_ddr_init_timing_check,
    .mspi_init_timing_apply = am_devices_mspi_psram_aps12808l_apply_ddr_timing,
...
uint32_t
am_widget_mspi_power_saverestore(void *pWidget, am_hal_sysctrl_power_state_e sleepMode)
{
    am_widget_mspi_config_t        *pWidgetConfig = (am_widget_mspi_config_t *)pWidget;
#if defined(TEST_HALF_SLEEP)
    if(sleepMode == AM_HAL_SYSCTRL_NORMALSLEEP || sleepMode == AM_HAL_SYSCTRL_DEEPSLEEP)
    {
        if (AM_HAL_STATUS_SUCCESS != am_devices_mspi_psram_aps25616n_enter_halfsleep(pWidgetConfig->pDevHandle))
        {
            return AM_WIDGET_MSPI_ERROR;
        }
    }
#endif
    if (AM_HAL_STATUS_SUCCESS != am_hal_mspi_power_control(pWidgetConfig->pHandle, sleepMode, true))
    ...
    {
        if (AM_HAL_STATUS_SUCCESS != am_devices_mspi_psram_aps25616n_exit_halfsleep(pWidgetConfig->pDevHandle))
        {
            return AM_WIDGET_MSPI_ERROR;
        }
    }
#endif    
    return AM_WIDGET_MSPI_SUCCESS;
}
...
bool am_widget_mspi_test_write_read(void *pWidget, void *pTestCfg, char *pErrStr)
{
  ...
  if (AM_WIDGET_MSPI_SUCCESS != ui32Status)
  {
    ...
    if (AM_WIDGET_SUCCESS != am_widget_mspi_power_save_and_restore(pWidgetConfig, MSPI_TEST_POWERSAVE_MODE))
    {
         return false;
    }
...
bool am_widget_mspi_test_b2b(void *pWidget, void *pTestCfg, char *pErrStr)
{
  ...
  while (numBytesToRead)
  {
      ...
      if (AM_WIDGET_MSPI_SUCCESS != ui32Status)
      {
        am_util_stdio_printf("Failed to read from target sector. numBytesToRead = %lu,  Status = %d\n", numBytesToRead, ui32Status);
        return false;
      }
      numBytesToRead -= num;
  }
