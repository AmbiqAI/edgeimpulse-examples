...
#define STIMER_CAP_GPIOIO_NUM   (5)
#define TEST_TIMES_LIMIT
...
am_widget_stimer_int_test(void)
{
  ...
  NVIC_ClearPendingIRQ(STIMER_CMPR2_IRQn);
  NVIC_ClearPendingIRQ(STIMER_CMPR3_IRQn);
  NVIC_ClearPendingIRQ(STIMER_CMPR4_IRQn);
  NVIC_ClearPendingIRQ(STIMER_CMPR5_IRQn);
  NVIC_ClearPendingIRQ(STIMER_CMPR6_IRQn);
  NVIC_ClearPendingIRQ(STIMER_CMPR7_IRQn);
...
am_widget_stimer_capture_test(uint32_t ui32Speed, uint32_t ui32CaptureNum)
{
    ...
    uint32_t ui32GPIONumber = STIMER_CAP_GPIOIO_NUM;
...
am_widget_stimer_counter_test(uint32_t ui32Speed)
{
    ...
    bool bPass = true;
#if defined(TEST_TIMES_LIMIT) || defined(APOLLO5_FPGA)
...
#ifdef APOLLO5_FPGA
        if (ui32TestCnt++ > 10000)
        {
            am_util_stdio_printf("\nSTimer counter test passed with clock source %d in 10 thouthand times of tests.\n", ui32Speed);
            break;
        }
#endif
...
am_widget_stimer_sleep_wakeup_test(void)
{
    ...
    uint32_t ui32CapIONum = STIMER_CAP_GPIOIO_NUM;
