...
uint32_t am_widget_fullduplex_slave_cleanup(void)
{
    ...
    am_hal_pwrctrl_periph_disable((am_hal_pwrctrl_periph_e)(AM_HAL_PWRCTRL_PERIPH_IOSFD0 + AM_HAL_IOSFD_WR));
    am_hal_pwrctrl_periph_disable((am_hal_pwrctrl_periph_e)(AM_HAL_PWRCTRL_PERIPH_IOSFD0 + AM_HAL_IOSFD_RD));
...
void log_waiting_process(char log_char, uint32_t chars_per_line) {

    static uint32_t char_count = 0;

    am_util_stdio_printf("%c", log_char);
...
am_widget_fullduplex_slave_test(void *pTestCfg)
{
    ...
    AM_WIDFET_DEBUG_PRINTF("\nStep 1 :Wait and read the IOINTEN until the host is init, IOINTEN can only be set by the HOST\n");
    wait_count = 0;
    // Wait until the host is ready
    while(1)
    {
        // read the IOINTEN to check the if the host is init
        am_hal_ios_control(g_pIOSFD0Handle, AM_HAL_IOS_REQ_HOST_INTEN_GET, &ui32Arg);
        if(ui32Arg == 0xFF)
        {
            break;
        }
        // Delay for 1s to avoid flooding logs
        am_util_delay_ms(1);
        if (wait_count++ >= 1000)
        {
            log_waiting_process('#', 32);
            wait_count = 0;
        }
    }
    am_util_stdio_printf("\n");

    // receive the host ready signal
    ...
    AM_WIDFET_DEBUG_PRINTF("Step 2: Start read and write data from HOST\n");
    ...
    AM_WIDFET_DEBUG_PRINTF("Step 4: Wait until the host Clear IOINT status and IOINTEN after transaction complete\n");
    ...
    while(1)
    {
        ...
        if(! (ui32Status & HANDSHAKE_IOS_TO_IOM) )
        {
            break;
        }
        am_util_delay_ms(1);
