                                  ...
                                  void **ppWidget, char *pErrStr)
{
    ...
    *ppWidget = &g_IomSpiWidget;
    ...
    if (AM_HAL_IOM_SPI_MODE == pTestCfg->IOMConfig.eInterfaceMode)
    {
        if ( g_IomSpiWidget.ui32DeviceType == AM_WIDGET_DEVICE_EM9304 )
        {
            ui32Status = am_devices_em9304_init(g_IomSpiWidget.ui32Module,
                                                &g_IomSpiWidget.IOMConfig, &g_pIOMHandle);
            ui32Status = (ui32Status == AM_DEVICES_EM9304_STATUS_SUCCESS) ? AM_WIDGET_SUCCESS : AM_WIDGET_ERROR;

        }
        else
        {
#if MB85RS1MT
            ui32Status = am_devices_mb85rs1mt_init(pTestCfg->ui32Module, &pTestCfg->IOMConfig, &g_pIOMHandle);
            ui32Status = (ui32Status == AM_DEVICES_MB85RS1MT_STATUS_SUCCESS) ? AM_WIDGET_SUCCESS : AM_WIDGET_ERROR;
#elif MB85RQ4ML
            ui32Status = am_devices_mb85rq4ml_init(pTestCfg->ui32Module, &pTestCfg->IOMConfig, &g_pIOMHandle);
            ui32Status = (ui32Status == AM_DEVICES_MB85RQ4ML_STATUS_SUCCESS) ? AM_WIDGET_SUCCESS : AM_WIDGET_ERROR;
#elif MB85RS64V
            ui32Status = am_devices_mb85rs64v_init(pTestCfg->ui32Module, &pTestCfg->IOMConfig, &g_pIOMHandle);
            ui32Status = (ui32Status == AM_DEVICES_MB85RS64V_STATUS_SUCCESS) ? AM_WIDGET_SUCCESS : AM_WIDGET_ERROR;
#elif APS6404L
            ui32Status = am_devices_spipsram_init(pTestCfg->ui32Module, &pTestCfg->IOMConfig, &g_pIOMHandle);
            ui32Status = (ui32Status == AM_DEVICES_SPIPSRAM_STATUS_SUCCESS) ? AM_WIDGET_SUCCESS : AM_WIDGET_ERROR;
#endif
        }
    }
    else
    {
        ui32Status = am_devices_mb85rc256v_init(pTestCfg->ui32Module, &pTestCfg->IOMConfig, &g_pIOMHandle);
        if (AM_DEVICES_MB85RC256V_STATUS_SUCCESS == ui32Status)
        {
            ui32Status = AM_WIDGET_SUCCESS;
        }
        else
        {
            ui32Status = AM_WIDGET_ERROR;
        }
    }
...
uint32_t am_widget_iom_test_cleanup(void *pWidget, char *pErrStr)
{
    ...
    if ( pTestCfg->IOMConfig.eInterfaceMode == AM_HAL_IOM_SPI_MODE )
    {
#if MB85RS1MT
        am_devices_mb85rs1mt_term(pTestCfg->ui32Module);
#elif MB85RQ4ML
        am_devices_mb85rq4ml_term(pTestCfg->ui32Module);
#elif MB85RS64V
        am_devices_mb85rs64v_term(pTestCfg->ui32Module);
#elif APS6404L
        am_devices_spipsram_term(pTestCfg->ui32Module);
#endif
    }
    else
    {
        am_devices_mb85rc256v_term(pTestCfg->ui32Module);
    }
...
am_widget_iom_test_read_id(void *pWidget, char *pErrStr)
{
    ...
    {
#if MB85RS1MT
        return ((am_devices_mb85rs1mt_read_id(&ui32DeviceId) || ( ui32DeviceId != AM_DEVICES_MB85RS1MT_ID )) ? AM_WIDGET_ERROR: AM_WIDGET_SUCCESS);
#elif MB85RQ4ML
        return ((am_devices_mb85rq4ml_read_id(&ui32DeviceId) || ( ui32DeviceId != AM_DEVICES_MB85RQ4ML_ID )) ? AM_WIDGET_ERROR: AM_WIDGET_SUCCESS);
#elif MB85RS64V
        return ((am_devices_mb85rs64v_read_id(&ui32DeviceId) || ( ui32DeviceId != AM_DEVICES_MB85RS64V_ID )) ? AM_WIDGET_ERROR: AM_WIDGET_SUCCESS);
#elif APS6404L
        return ((am_devices_spipsram_read_id(&ui32DeviceId) || ( ui32DeviceId != AM_DEVICES_SPIPSRAM_KGD_PASS )) ? AM_WIDGET_ERROR: AM_WIDGET_SUCCESS);
#endif
    }
    else
    {
        if (am_devices_mb85rc256v_read_id(&ui32DeviceId) ||
            ((ui32DeviceId != AM_DEVICES_MB85RC256V_ID ) && (ui32DeviceId != AM_DEVICES_MB85RC64TA_ID)))
        {
            am_util_stdio_printf("\nDevice ID Failed | ID = %8X\n", ui32DeviceId);
            return AM_WIDGET_ERROR;
        }
        else
        {
...
am_widget_iom_test_blocking_write_read(void *pWidget, void *pCfg, char *pErrStr)
{
    ...
    //
    if ( pWidgetCfg->IOMConfig.eInterfaceMode == AM_HAL_IOM_SPI_MODE )
    {
        ui32Status =
#if MB85RS1MT
            am_devices_mb85rs1mt_blocking_write(&g_pucOutBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
#elif MB85RQ4ML
            am_devices_mb85rq4ml_blocking_write(&g_pucOutBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
#elif MB85RS64V
            am_devices_mb85rs64v_blocking_write(&g_pucOutBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
#elif APS6404L
            am_devices_spipsram_blocking_write(&g_pucOutBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
#endif
    }
    else
    {
        ui32Status = am_devices_mb85rc256v_blocking_write(&g_pucOutBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
    }
    ...
    if (ui32Status == 0)
    {
        ...
        if ( pWidgetCfg->IOMConfig.eInterfaceMode == AM_HAL_IOM_SPI_MODE )
        {
            ui32Status =
#if MB85RS1MT
                am_devices_mb85rs1mt_blocking_read(&g_pucInBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
#elif MB85RQ4ML
                am_devices_mb85rq4ml_blocking_read(&g_pucInBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
#elif MB85RS64V
                am_devices_mb85rs64v_blocking_read(&g_pucInBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
#elif APS6404L
                am_devices_spipsram_blocking_read(&g_pucInBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
#endif
        }
        else
        {
            ui32Status = am_devices_mb85rc256v_blocking_read(&g_pucInBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
        }
    }
...
am_widget_iom_test_nonblocking_write_read(void *pWidget, void *pCfg, char *pErrStr)
{
    ...
    if ( pWidgetCfg->IOMConfig.eInterfaceMode == AM_HAL_IOM_SPI_MODE )
    {
        ui32Status =
#if MB85RS1MT
            am_devices_mb85rs1mt_nonblocking_write(&g_pucOutBuffer[0] + ui32Offset,
#elif MB85RQ4ML
            am_devices_mb85rq4ml_nonblocking_write(&g_pucOutBuffer[0] + ui32Offset,
#elif MB85RS64V
            am_devices_mb85rs64v_nonblocking_write(&g_pucOutBuffer[0] + ui32Offset,
#elif APS6404L
            am_devices_spipsram_nonblocking_write(&g_pucOutBuffer[0] + ui32Offset,
#endif
                                               0,
                                               pTestCfg->ui32BufferSize,
                                               pfnWidget_IOM_Callback_Write, 0);
    }
    else
    {
        ui32Status = am_devices_mb85rc256v_nonblocking_write(&g_pucOutBuffer[0] + ui32Offset,
                                                0,
                                                pTestCfg->ui32BufferSize,
                                                pfnWidget_IOM_Callback_Write, 0);
    }
    ...
    if (ui32Status == 0)
    {
        ...
        if ( pWidgetCfg->IOMConfig.eInterfaceMode == AM_HAL_IOM_SPI_MODE )
        {
            ui32Status =
#if MB85RS1MT
                am_devices_mb85rs1mt_nonblocking_read(&g_pucInBuffer[0] + ui32Offset,
#elif MB85RQ4ML
                am_devices_mb85rq4ml_nonblocking_read(&g_pucInBuffer[0] + ui32Offset,
#elif MB85RS64V
                am_devices_mb85rs64v_nonblocking_read(&g_pucInBuffer[0] + ui32Offset,
#elif APS6404L
                am_devices_spipsram_nonblocking_read(&g_pucInBuffer[0] + ui32Offset,
#endif
                                                   0,
                                                   pTestCfg->ui32BufferSize,
                                                   pfnWidget_IOM_Callback_Read, 0);
        }
        else
        {
            ui32Status = am_devices_mb85rc256v_nonblocking_read(&g_pucInBuffer[0] + ui32Offset,
                                                   0,
                                                   pTestCfg->ui32BufferSize,
                                                   pfnWidget_IOM_Callback_Read, 0);
        }
...
am_widget_iom_test_queue_write_read(void *pWidget, void *pCfg, char *pErrStr)
{
    ...
    if ( pWidgetCfg->IOMConfig.eInterfaceMode == AM_HAL_IOM_SPI_MODE )
    {
        ...
        while ((0 == ui32Status) && (ui32Bytes < pTestCfg->ui32BufferSize))
        {
            ...
            ui32Status =
#if MB85RS1MT
                am_devices_mb85rs1mt_nonblocking_write(&g_pucOutBuffer[0] + ui32Offset + ui32Bytes,
#elif MB85RQ4ML
                am_devices_mb85rq4ml_nonblocking_write(&g_pucOutBuffer[0] + ui32Offset + ui32Bytes,
#elif MB85RS64V
                am_devices_mb85rs64v_nonblocking_write(&g_pucOutBuffer[0] + ui32Offset + ui32Bytes,
#elif APS6404L
                am_devices_spipsram_nonblocking_write(&g_pucOutBuffer[0] + ui32Offset + ui32Bytes,
#endif
                                               ui32Bytes,
                                               size,
                                               0, 0);
        ...
        if (0 == ui32Status)
        {
            ...
            while ((ui32Status == 0) && (ui32Bytes < pTestCfg->ui32BufferSize))
            {
                ...
                ui32Status =
#if MB85RS1MT
                    am_devices_mb85rs1mt_nonblocking_read(&g_pucInBuffer[0] + ui32Offset + ui32Bytes,
#elif MB85RQ4ML
                    am_devices_mb85rq4ml_nonblocking_read(&g_pucInBuffer[0] + ui32Offset + ui32Bytes,
#elif MB85RS64V
                    am_devices_mb85rs64v_nonblocking_read(&g_pucInBuffer[0] + ui32Offset + ui32Bytes,
#elif APS6404L
                    am_devices_spipsram_nonblocking_read(&g_pucInBuffer[0] + ui32Offset + ui32Bytes,
#endif
                                                   ui32Bytes,
                                                   size,
    ...
    else
    {
        ...
        while ((0 == ui32Status) && (ui32Bytes < pTestCfg->ui32BufferSize))
        {
            uint32_t size =
                   (blockSize < (pTestCfg->ui32BufferSize - ui32Bytes)) ? blockSize : (pTestCfg->ui32BufferSize - ui32Bytes);
            ui32Status = am_devices_mb85rc256v_nonblocking_write(&g_pucOutBuffer[0] + ui32Offset + ui32Bytes,
                                                ui32Bytes,
                                                size,
                                                0, 0);
            ui32Bytes += size;
        ...
        if (0 == ui32Status)
        {
            ...
            while ((ui32Status == 0) && (ui32Bytes < pTestCfg->ui32BufferSize))
            {
                uint32_t size =
                       (blockSize < (pTestCfg->ui32BufferSize - ui32Bytes)) ? blockSize : (pTestCfg->ui32BufferSize - ui32Bytes);
                ui32Status = am_devices_mb85rc256v_nonblocking_read(&g_pucInBuffer[0] + ui32Offset + ui32Bytes,
                                                   ui32Bytes,
                                                   size,
                                                   (size == pTestCfg->ui32BufferSize - ui32Bytes) ? pfnWidget_IOM_Callback_Read : 0,
                                                   0);
...
am_widget_iom_test_duplex(void *pWidget, void *pCfg, char *pErrStr)
{
    ...
    ui32Status = am_devices_em9304_fullduplex(0,     // ui32WriteAddress,
                                 g_pucOutBuffer,
                                 g_pucInBuffer,
...
am_widget_iom_test_blocking_write_read_error(void *pWidget, void *pCfg, char *pErrStr, uint32_t errorMask)
{
    ...
    {
        ui32Status = am_devices_mb85rc256v_blocking_write(&g_pucOutBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize/2);
        if ( ui32Status )
        {
            return AM_WIDGET_ERROR;
        }
...
#endif
        ui32Status = am_devices_mb85rc256v_blocking_write(&g_pucOutBuffer[0] + ui32Offset + pTestCfg->ui32BufferSize/2, pTestCfg->ui32BufferSize/2, (pTestCfg->ui32BufferSize - pTestCfg->ui32BufferSize/2));
        if ( ui32Status )
        {
            return AM_WIDGET_ERROR;
        }
    ...
    {
        ui32Status = am_devices_mb85rc256v_blocking_read(&g_pucInBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize/2);
        if ( ui32Status )
        {
            return AM_WIDGET_ERROR;
        }
...
#endif
        ui32Status = am_devices_mb85rc256v_blocking_read(&g_pucInBuffer[0] + ui32Offset + pTestCfg->ui32BufferSize/2, pTestCfg->ui32BufferSize/2, (pTestCfg->ui32BufferSize - pTestCfg->ui32BufferSize/2));
        if ( ui32Status )
        {
            return AM_WIDGET_ERROR;
        }
    ...
    if ( pWidgetCfg->IOMConfig.eInterfaceMode == AM_HAL_IOM_SPI_MODE )
    {
        ...
        while ((0 == ui32Status) && (ui32Bytes < pTestCfg->ui32BufferSize))
        {
            ...
            if (errorMask & AM_WIDGET_IOM_ERRMASK_DMA_START) // Send SPI Write - with DMA from illegal address
            {
                ...
                ui32Status =
#if MB85RS1MT
                    am_devices_mb85rs1mt_nonblocking_write((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE),
#elif MB85RQ4ML
                    am_devices_mb85rq4ml_nonblocking_write((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE),
#elif MB85RS64V
                    am_devices_mb85rs64v_nonblocking_write((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE),
#elif APS6404L
                    am_devices_spipsram_nonblocking_write((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE),
#endif
                                                   ui32Bytes,
                                                   size,
                                                  errCallback,
            ...
            if ((errorMask & AM_WIDGET_IOM_ERRMASK_DMA_MID) && (size >= 2)) // Send SPI Write - with DMA from end of Flash (should cause DMAERR in the middle)
            {
                ...
                ui32Status =
#if MB85RS1MT
                    am_devices_mb85rs1mt_nonblocking_write((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE - size/2),
#elif MB85RQ4ML
                    am_devices_mb85rq4ml_nonblocking_write((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE - size/2),
#elif MB85RS64V
                    am_devices_mb85rs64v_nonblocking_write((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE - size/2),
#elif APS6404L
                    am_devices_spipsram_nonblocking_write((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE - size/2),
#endif
                                                   ui32Bytes,
                                                   size,
                                                  errCallback,
        ...
        if (0 == ui32Status)
        {
            ...
            while ((ui32Status == 0) && (ui32Bytes < pTestCfg->ui32BufferSize))
            {
                ...
                if (errorMask & AM_WIDGET_IOM_ERRMASK_DMA_START) // Send SPI Write - with DMA from illegal address
                {
                    ...
                    ui32Status =
#if MB85RS1MT
                        am_devices_mb85rs1mt_nonblocking_read((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE),
#elif MB85RQ4ML
                        am_devices_mb85rq4ml_nonblocking_read((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE),
#elif MB85RS64V
                        am_devices_mb85rs64v_nonblocking_read((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE),
#elif APS6404L
                        am_devices_spipsram_nonblocking_read((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE),
#endif
                                                       ui32Bytes,
                                                       size,
                                                      errCallback,
                ...
                if ((errorMask & AM_WIDGET_IOM_ERRMASK_DMA_MID) && (size >= 2)) // Send SPI Write - with DMA from end of Flash (should cause DMAERR in the middle)
                {
                    ...
                    ui32Status =
#if MB85RS1MT
                        am_devices_mb85rs1mt_nonblocking_read((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE - size/2),
#elif MB85RQ4ML
                        am_devices_mb85rq4ml_nonblocking_read((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE - size/2),
#elif MB85RS64V
                        am_devices_mb85rs64v_nonblocking_read((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE - size/2),
#elif APS6404L
                        am_devices_spipsram_nonblocking_read((uint8_t *)(SRAM_BASEADDR + SRAM_SIZE - size/2),
#endif
                                                       ui32Bytes,
                                                       size,
                                                      errCallback,
