...
#pragma message("FB_FORMAT = NEMA_RGB24")
#else
#define FB_FORMAT NEMA_RGBA8888
#pragma message("FB_FORMAT = NEMA_RGBA8888")
#endif  //defined( AM_PART_APOLLO4P ) || defined( AM_PART_APOLLO4L )

#if (DISP_CTRL_IP == DISP_CTRL_IP_DC)

#if (FB_FORMAT == NEMA_RGB24)
#define DC_LAYER_FORMAT NEMADC_RGB24
#pragma message("DC_LAYER_FORMAT = NEMADC_RGB24")
#elif (FB_FORMAT == NEMA_RGBA8888)
#define DC_LAYER_FORMAT NEMADC_RGBA8888
#pragma message("DC_LAYER_FORMAT = NEMADC_RGBA8888")
#else
#error unsupported DC layer format
#endif
...
#define PSRAM_W3_21x182_RGBA       (PSRAM_XIP_BASE_ADDRESS + PSRAM_TEXTURE_AREA_OFFSET)
...
{
    {{0},  FB_RESX, FB_RESY / SMALLFB_STRIPES, -1, 1, FB_FORMAT, NEMA_FILTER_BL},
    {{0},  FB_RESX, FB_RESY / SMALLFB_STRIPES, -1, 1, FB_FORMAT, NEMA_FILTER_BL},
};
...
extern mspi_psram_device_func_t mspi_psram_device_func;
//*****************************************************************************
//
// Task function.
//
...
#if defined( AM_PART_APOLLO4P ) ||  defined( AM_PART_APOLLO4L )
    ui32Ret = mspi_psram_device_func.mspi_nonblocking_read(g_pPsramHandle,
...
#endif
    TEST_ASSERT_TRUE(ui32Ret == AM_HAL_STATUS_SUCCESS);
...
watchface_free(void)
{
    g_sSportsDialBuffer[0].h = FB_RESY / SMALLFB_STRIPES;
    g_sSportsDialBuffer[1].h = FB_RESY / SMALLFB_STRIPES;
    nema_buffer_destroy(&g_sSportsDialBuffer[0].bo);
    nema_buffer_destroy(&g_sClockDialBuffer[0].bo);
    nema_buffer_destroy(&g_sFrameBuffer[0].bo);
    nema_buffer_destroy(&g_sSportsDialBuffer[1].bo);
    nema_buffer_destroy(&g_sClockDialBuffer[1].bo);
    nema_buffer_destroy(&g_sFrameBuffer[1].bo);
    vSemaphoreDelete(g_semDMAFinish);
    vSemaphoreDelete(g_semDCStart);
    vSemaphoreDelete(g_semDCEnd);
    vSemaphoreDelete(g_semGPUStart);
    vSemaphoreDelete(g_semGPUEnd);
}
...
GuiTask(void *pvParameters)
{
    ...
    TEST_ASSERT_TRUE(g_semDMAFinish != NULL);
    ...
    while (1)
    {
...
#endif
        {
            ...
            if ( crcPart >= 0 )
            {
                ...
                TEST_ASSERT_TRUE(crc_result == g_GPUChecksumTable[crcPart]);
        ...
        //print fps
        nema_calculate_fps();
        am_util_stdio_printf("GuiTask: FrameCnt is %d\n", ui32FrameCnt);
        ui32FrameCnt++;
        if (ui32FrameCnt > FRAMES_TO_RUN)
        {
            vTaskDelete(RenderTaskHandle);
            am_util_stdio_printf("GPU render task end!\n");

            vTaskDelete(DisplayTaskHandle);
            am_devices_display_generic_deinit();
            am_util_stdio_printf("Display task end!\n");           
    ...
    //nema_cl_destroy(&cl);
    watchface_free();
    am_util_stdio_printf("GUI task end!\n");
    vTaskDelete(NULL);
    while (1);
}
