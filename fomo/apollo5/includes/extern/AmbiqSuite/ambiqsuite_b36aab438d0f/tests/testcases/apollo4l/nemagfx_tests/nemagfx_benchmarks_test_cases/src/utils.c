...
int
parse_params(int* testno, execution_mode* mode, int argc, char* argv[])
{
    return 0;
}
...
//#if defined(FORMAT_24BIT)
//    uint32_t index = 3;
//    for(uint32_t i=4;i< Ambiq_logo.w * Ambiq_logo.h * 4;i++)
//    {
//        *(uint8_t *)(Ambiq_logo.bo.base_phys + index++) = *(uint8_t *)(Ambiq_logo.bo.base_phys + i++);
//        *(uint8_t *)(Ambiq_logo.bo.base_phys + index++) = *(uint8_t *)(Ambiq_logo.bo.base_phys + i++);
//        *(uint8_t *)(Ambiq_logo.bo.base_phys + index++) = *(uint8_t *)(Ambiq_logo.bo.base_phys + i++);
//    }
//#else
//    uint8_t temp[2];
//    for(uint32_t i=0;i< Ambiq_logo.w * Ambiq_logo.h * 2;i += 2)
//    {
//        temp[0] = *(uint8_t *)(Ambiq_logo.bo.base_phys + i);
//        temp[1] = *(uint8_t *)(Ambiq_logo.bo.base_phys + i+1);
//        *(uint8_t *)(Ambiq_logo.bo.base_phys + i)= temp[1];
//        *(uint8_t *)(Ambiq_logo.bo.base_phys + i+1) = temp[0];
//    }
//#endif

    if (sCurrentCfg.eFbLocation == FB_PSRAM)
    {
        fb.bo.base_phys = (uintptr_t)(MSPI_XIP_BASE_ADDRESS + Ambiq_logo.w*Ambiq_logo.h*4 + DejaVuSerif12pt8b.bitmap_size);
    ...
    //nema_reg_write(0xe4, 0x80040000);
...
float perfs[64];
void
bench_report(float perf, char *mes_unit, float utilization)
{
    int perf_i = (int)perf;
    perf = perf-perf_i;
