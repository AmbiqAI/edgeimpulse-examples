...
//!
//! @brief Exercise the TCM, SSRAM, MRAM, Extended Memory, and PSRAM across all
...
//! configurations. This testcase includes the following 11 tests.
//!
//! System Test Board Configurations:
//! 
//! STMBv2
//! ----------------------------------------------------------------------------
//! | STMBv2 Switches:
//! ----------------------------------------------------------------------------
//! | [SW31:ENn]  [SW25:X16] [SW17:NO]  [SW16:NO] [SW11:NO]   [SW10:DIS] [SW26:NO]
//! | [SW27:I2S0] [SW28:NO]  [SW21:EN]  [SW24:EN] [SW23:eMMC] [SW12:AP]  [SW18:X8]
//! | [SW29:NO]   [SW30:EN]  [SW15:ETM] [SW20:EN] [SW14:X16]  [SW19:EN]  [SW13:ETM]
//! ----------------------------------------------------------------------------
//!
//! STMBv1
//! ----------------------------------------------------------------------------
...
//! | *I2S0 DATA (J26) <-> CEn (J13) for I2S loopback                       
//! ----------------------------------------------------------------------------
//! | STMBv1 Switches:
//! ----------------------------------------------------------------------------
//! |            [SW17:OB ] [SW16:OB] [SW11:OB  ] [SW10:Hi-Z]
//! | [SW25:X16] [SW21:ON ] [SW24:ON] [SW23:eMMC] [SW12:AP  ] [SW18:ON ]
...
//! data in source location. Run to end without any hardfaults and wasn't stuck.
//!
//! 3. dc_read_cross_internal_memory_test
//! Objective: DC accesses all internal memories and XIPMM.
//! Test Outline:
//! DC reads data from source memory (MRAM, TCM, SSRAM0, DSP0_IRAM, DSP0_DRAM,
//! SSRAM1 and PSRAM (XIPMM0)), and sends data out through display interface (SPI).
//! This test covers 0 - 16 bytes offsets both for source and destination addresses.
//! This test crosses block boundaries within a memory and boundaries between
//! 2 memories.
//! Source and destination data buffer size for once copy is fixed to 32 bytes.
//! Get the reference CRC from DC CRC register after DC finishes reading data
//! from an address-aligned and non-boundary crossed region in SSRAM0. Compare
//! CRC with reference value when DC finishes reading data from other memory regions.
//! Pass/Fail Criteria: All CRCs must be matched with reference value. Run to end
//! without any hardfaults and wasn't stuck.
//!
//! 4. apbdma_mspi_cross_memory_test
...
//! data in source location. Run to end without any hardfaults and wasn't stuck.
//!
//! 5. apbdma_sdio_cross_memory_test
...
//! data in source location. Run to end without any hardfaults and wasn't stuck.
//!
//! 6. apbdma_iom_cross_memory_test
...
//! data in source location. Run to end without any hardfaults and wasn't stuck.
//!
//! 7. apbdma_i2s_cross_memory_test
...
//! data in source location. Run to end without any hardfaults and wasn't stuck.
//!
//! 8. apbdma_crc_cross_memory_test
...
//!
//! 9. apbdma_pdm_cross_memory_test
...
//!
//! 10. apbdma_adc_cross_memory_test
...
am_memory_config_t sMemCfg[] = {
    ...
    { // sDsp0IramCfg
        .ui32StartAddr = 0x10160000,
        .ui32Size = 0x20000,
        .ui32BlockSize = 0x10000,//0x4000,
        .bFollowedByMem = true
    },
    { // sDsp0DramCfg
        .ui32StartAddr = 0x10180000,
        .ui32Size = 0x40000,
        .ui32BlockSize = 0x20000,//0x4000,
        .bFollowedByMem = true
    },
    { // sSsram1Cfg
        .ui32StartAddr = 0x101C0000,
        .ui32Size = 0x100000,
        .ui32BlockSize = 0x80000,//0x8000,
        .bFollowedByMem = false
    },
    { // sXIPMM0Cfg
...
am_memory_config_t sMemCfg[] = {
    ...
    { // sMcuTcmCfg
        .ui32StartAddr = 0x10008000, // Skip 0x10008000 boundary tests
        .ui32Size = 0x60000,
...
globalSetUp(void)
{
    ...
    // Print the banner.
    //
    am_util_stdio_printf("\n\nApollo4p cross memory boundaries test cases\n\n");
    //
    // just for test
    //
...
//*****************************************************************************
uint32_t
dc_read_memory (uint32_t ui32SrcStartAddr, uint32_t ui32BuffSizeBytes)
{
    uint32_t ui32ResX;
    uint32_t ui32ResY;
    ui32ResX = 4;
    ui32ResY = ui32BuffSizeBytes / 2 / 4;
    nemadc_MIPI_CFG_out(MIPICFG_SPI4 | MIPICFG_SPI_CSX_V | MIPICFG_DBI_EN | MIPICFG_RESX | MIPICFG_1RGB565_OPT0 | MIPICFG_DIS_TE);
    nemadc_timing(ui32ResX, 4, 10, 10,
                  ui32ResY, 10, 50, 10);
    nemadc_layer_t layer0 = {0};
    layer0.resx          = ui32ResX;
    layer0.resy          = ui32ResY;
    layer0.buscfg        = 0;
    ...
    layer0.flipx_en      = 0;
    layer0.flipy_en      = 0;
    layer0.baseaddr_virt = (void *)ui32SrcStartAddr;
         ...
         ui32SrcBoundAddr += sMemCfg[eSrcMem].ui32BlockSize)
    {
        for (ui32SrcOffset = 0; ui32SrcOffset <= 16; ui32SrcOffset++)
        {
            ...
            if (eSrcMem != MRAM)
            {
                ...
                if (eMaster == DC_READ)
                {
                    ui32RefBufferAddr = SSRAM0_ALIGNED_ADDR_WITHOUT_BOUNDARY;
                }
                else
                {
                ...
                {
                    //
                    // dc read memory and calculate CRC
                    //
                    ui32RefCheckSum = dc_read_memory(SSRAM0_ALIGNED_ADDR_WITHOUT_BOUNDARY, ui32BuffSizeBytes);
                    ui32SrcCheckSum = dc_read_memory(ui32SrcStartAddr, ui32BuffSizeBytes);
                    //
                    // Compare CRC
                    //
                    if(ui32RefCheckSum == 0)
                    {
                        am_util_stdio_printf("-----ERROR!----- The reference CRC is 0x0, DC may did not work.\n");
                        bTestPass = false;
                    }
                    else
                    {
                        ...
                        {
                            am_util_stdio_printf("-----ERROR!----- The CRC dc calculated(at address 0x%08X) and reference CRC did not match! REF CRC 0x%08X, Currect CRC 0x%08X.\n", ui32SrcStartAddr, ui32RefCheckSum, ui32SrcCheckSum);
                            bTestPass = false;
                        }
                        else
                        {
                            //am_util_stdio_printf("The CRC dc calculated(at address 0x%08X) and reference CRC matched! REF CRC 0x%08X, Currect CRC 0x%08X.\n", ui32SrcStartAddr, ui32RefCheckSum, ui32SrcCheckSum);
                        }
                    }
                    continue;
                }
                ...
                else
                {
                    for (ui32DestOffset = 0; ui32DestOffset <= 16; ui32DestOffset++)
                    {
                        ...
                        if ((eMaster != APBDMA_SDIO) && (eMaster != CPU_MEMCPY) && (eMaster != GPU_BLIT))
                        {
                            ...
                            if ((ui32DestBoundAddr == 0x10160000) || (ui32SrcBoundAddr == 0x10160000))
                            {
                                am_util_stdio_printf("Skipped boundary between SSRAM0 and ESRAM.\n");
                                break;
                            }
                            if ((ui32DestBoundAddr == 0x101C0000) || (ui32SrcBoundAddr == 0x101C0000))
                            {
                                am_util_stdio_printf("Skipped boundary between ESRAM and SSRAM1.\n");
                                break;
                            }
                        }
#endif
...
security_crc_calculation(am_access_master_e eMaster, am_memory_type_e eSrcMem, uint32_t ui32BuffSizeBytes)
{
         ...
         ui32SrcBoundAddr += sMemCfg[eSrcMem].ui32BlockSize)
    {
        ...
        if (ui32SrcBoundAddr == 0x10160000)
        {
            am_util_stdio_printf("Skipped boundary between SSRAM0 and ESRAM.\n");
            break;
        }
        if (ui32SrcBoundAddr == 0x101C0000)
        {
            am_util_stdio_printf("Skipped boundary between ESRAM and SSRAM1.\n");
            break;
        }
...
dma_store_data_into_memory(am_access_master_e eMaster, am_memory_type_e eDestMem, uint32_t ui32BuffSizeBytes)
{
         ...
         ui32DestBoundAddr += sMemCfg[eDestMem].ui32BlockSize)
    {
        ...
        for (ui32DestOffset = 0; ui32DestOffset <= 16; ui32DestOffset++)
        {
            ui32DestStartAddr = ui32DestBoundAddr - ui32DestOffset;
#ifdef SKIP_BONDARY_BTW_DIFF_MEM
            if ((eMaster == APBDMA_PDM) || (eMaster == APBDMA_ADC))
            {
//                if (ui32DestBoundAddr == 0x10060000)
//                {
//                    am_util_stdio_printf("Skipped boundary between MCU_TCM and SSRAM0.\n");
//                    break;
//                }
                if (ui32DestBoundAddr == 0x10160000)
                {
                    am_util_stdio_printf("Skipped boundary between SSRAM0 and ESRAM.\n");
                    break;
                }
//                if (ui32DestBoundAddr == 0x101C0000)
//                {
//                    am_util_stdio_printf("Skipped boundary between ESRAM and SSRAM1.\n");
//                    break;
//                }
...
//
//  Test Case
//  dc_read_cross_memory_test
//
//*****************************************************************************
void
dc_read_cross_internal_memory_test(void)
{
    uint32_t ui32BuffSizeBytes = 32;
    bool bTestPass = true;
    am_memory_type_e eSrcMem;
    am_memory_type_e eDestMem;
    am_access_master_e eMaster = DC_READ;
#ifdef WITH_DISPLAY
    nemadc_initial_config_t sDCConfig;
    am_devices_dc_xspi_raydium_config_t sDisplayPanelConfig;
    sDisplayPanelConfig.ui16ResX = 4;
    sDisplayPanelConfig.ui16ResY = 4;
    ...
    sDisplayPanelConfig.ui16MinX = 190 + g_sDispCfg.ui16Offset;
    sDisplayPanelConfig.ui16MinY = 190;
...
#endif
    am_util_stdio_printf("dc_read_cross_memory_test...\n");
    eDestMem = MCU_TCM; //!< eDestMem is useless in dc tests.
    for (eSrcMem = MRAM; eSrcMem < NUM_MEM; eSrcMem++)
    {
        bTestPass = memory_copy(eMaster, eSrcMem, eDestMem, ui32BuffSizeBytes);
    }
    TEST_ASSERT_TRUE(bTestPass);

    return;
} // dc_read_cross_internal_memory_test
...
apbdma_adc_cross_memory_test(void)
{
    ...
    am_util_stdio_printf("Only tested if ADC DMA hang at boundary, did not check data in RX buffer.\n");
#if defined(CACHE_TEST) || defined(DAXI_TEST)
    uint32_t i, j, ui32Status;
#endif
#ifdef CACHE_TEST
