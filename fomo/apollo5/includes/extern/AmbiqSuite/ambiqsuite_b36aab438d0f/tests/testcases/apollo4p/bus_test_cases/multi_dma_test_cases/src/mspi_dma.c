...
//! @file mspi_dma.c
//!
//! @brief mspi dma operations,it include MSPI0 operate PSRAM,MSPI2 operate Flash.
//!
//! TIPs:
...
volatile static bool g_bMspiFlashDMAFinish = true;
volatile static bool g_bMspiPsramDMAFinish = true;
...
psram_deinit(void)
{
    ...
    //
    // deinit the MSPI and PSRAM Device.
    //
...
psram_start(void)
{
    ...
    if(g_bMspiPsramDMAFinish == false)
    {
        //
        // don't start DMA transaction until current DMA completed.
        //
        return true;
    }
    ...
    {
#ifdef USE_DEBUG_PIN
    am_hal_gpio_output_set(DEBUG_PIN_9);
#endif
        return true;
    }
}
...
flash_deinit()
{
    ...
    NVIC_DisableIRQ(MspiInterrupts[MSPI_FLASH_MODULE]);
    return true;
}
//*****************************************************************************
//
...
flash_init(void)
{
    ...
    //
    // Init the flash device to work at 24MHZ. no support higher frequencies.(please refer jira FSV-962)
    //
    g_sMspiFlashConfig.eClockFreq = AM_HAL_MSPI_CLK_24MHZ;
    g_sMspiFlashConfig.eDeviceConfig = AM_HAL_MSPI_FLASH_OCTAL_CE0;

    ui32Status = am_devices_mspi_atxp032_sdr_init_timing_check(MSPI_FLASH_MODULE, (void*)&g_sMspiFlashConfig , &g_MSPITimingConfig);
    if (AM_DEVICES_MSPI_ATXP032_STATUS_SUCCESS != ui32Status)
    {
        am_util_stdio_printf("Failed to configure the MSPI and Flash Device correctly!\n");
        return false;
    ...
    ui32Status = am_devices_mspi_atxp032_apply_sdr_timing(g_pFlashHandle,&g_MSPITimingConfig);
    if (AM_DEVICES_MSPI_ATXP032_STATUS_SUCCESS != ui32Status)
    {
        am_util_stdio_printf("Failed to configure the MSPI and Flash Device correctly!\n");
        return false;
...
{
    uint32_t ui32Status;
    if(g_bMspiFlashDMAFinish == false)
    {
        //
        // don't start DMA transaction until current DMA completed.
        //
    ...
    {
#ifdef USE_DEBUG_PIN
        am_hal_gpio_output_set(DEBUG_PIN_10);
#endif
        return true;
    }
}
