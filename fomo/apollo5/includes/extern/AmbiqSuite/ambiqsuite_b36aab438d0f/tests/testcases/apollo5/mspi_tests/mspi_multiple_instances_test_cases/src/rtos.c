...
TaskHandle_t mspi_flash_task_handle;
TaskHandle_t mspi_psram_task_handle;
...
{
    if( (eTaskGetState(mspi_flash_task_handle) == eDeleted) && (eTaskGetState(mspi_psram_task_handle) == eDeleted) && !g_bReportedResults)
    {
...
setup_task(void *pvParameters)
{
    ...
    //
    am_util_stdio_printf("Running mspi multiple test cases tasks...\r\n");
    g_ui32TestCnt = 0;
    ...
    while(1)
    {
        ...
        //
        xTaskCreate(MspiPsramTask, "MspiPsramTask", 512, 0, TASK_PRIORITY_MIDDLE, &mspi_psram_task_handle);

        xTaskCreate(MspiFlashTask, "MspiFlashTask", 512, 0, TASK_PRIORITY_MIDDLE, &mspi_flash_task_handle);
