...
uint32_t check_inputstate(uint32_t ui32Output, uint32_t ui32Input);
...
gpio_test_connectivity(void)
{
    ...
    for ( i = 0; i < NUM_LOOPBACK_TESTS; i++ )
    {
        ...
        check_inputstate(sPinPair.pin1, sPinPair.pin2);
        check_inputstate(sPinPair.pin2, sPinPair.pin1);
...
//*****************************************************************************
//
// Function - check_inputstate() GAR-198
// Description - Configure GPIO pin. Toggle output pin, and make sure
//                                   connected input pin toggles with it.
// Input Parameters - ui32Output : GPIO Output Pin #
//                  - ui32Input :  GPIO Intput Pin #
// Return - ui32NumErrors : Return number of errors.
...
//*****************************************************************************
uint32_t
check_inputstate(uint32_t ui32Output, uint32_t ui32Input)
{
    uint32_t ui32NumErrors = 0;

    am_util_stdio_printf("\nCheck Input Test, drivepin %d, testpin %d\n", ui32Output, ui32Input);

    uint32_t ui32TestRet;
    am_hal_gpio_pincfg_t test_pincfg_output = sPushPull;
    am_hal_gpio_pincfg_t test_pincfg_input = am_hal_gpio_pincfg_default;

    if(ui32Output > AM_HAL_PIN_TOTAL_GPIOS)
    {
        TEST_ASSERT_TRUE(false);
        am_util_stdio_printf("Output pin exceed range :\n");
    }
    if(ui32Input > AM_HAL_PIN_TOTAL_GPIOS)
    {
        TEST_ASSERT_TRUE(false);
        am_util_stdio_printf("input pin exceed range :\n");
    }
    ...
    //https://ambiqmicro.atlassian.net/browse/CAYNSWS-1488?focusedCommentId=105033
    test_pincfg_input.GP.cfg_b.eForceInputEn = 1;
    for (uint32_t ui32Func = 0; ui32Func < AM_HAL_PIN_NUMFUNCS; ui32Func++)
    {
        //only test SDIF function as we don't know exactly which function would set the pad as output.
        if ( (am_hal_pin_fn_list[ui32Input][ui32Func] >= AM_HAL_PIN_FN_SDIF0_CMD &&
              am_hal_pin_fn_list[ui32Input][ui32Func] <= AM_HAL_PIN_FN_SDIF1_DAT7) ||
             (am_hal_pin_fn_list[ui32Input][ui32Func] >= AM_HAL_PIN_FN_MSPI0_0 &&
              am_hal_pin_fn_list[ui32Input][ui32Func] <= AM_HAL_PIN_FN_MSPI2_5) )

        {
            test_pincfg_input.GP.cfg_b.uFuncSel = ui32Func;
            // Configure input
            ui32TestRet = am_hal_gpio_pinconfig(ui32Input,test_pincfg_input);
            TEST_ASSERT_TRUE(ui32TestRet == AM_HAL_STATUS_SUCCESS);
            am_util_stdio_printf("\nSet Function: %d\n", ui32Func);

            // Set output high, and make sure input reads high and output is set high.
            am_hal_gpio_output_set(ui32Output);
            TEST_ASSERT_TRUE(TC_GPIO_OK == check_pin_state(1, ui32Output, ui32Input));

            // Set output low, and make sure input reads low and output is set high.
            am_hal_gpio_output_clear(ui32Output);
            TEST_ASSERT_TRUE(TC_GPIO_OK == check_pin_state(0, ui32Output, ui32Input));
        }
        else
        {
            am_util_stdio_printf("  Skipping other pin function\n");
        }
    }
    //
    ...
    //
    am_hal_gpio_output_clear(ui32Output);
    ui32TestRet = am_hal_gpio_pinconfig(ui32Input,am_hal_gpio_pincfg_default);
    TEST_ASSERT_TRUE(ui32TestRet == AM_HAL_STATUS_SUCCESS);
    ui32TestRet = am_hal_gpio_pinconfig(ui32Output,am_hal_gpio_pincfg_default);
    TEST_ASSERT_TRUE(ui32TestRet == AM_HAL_STATUS_SUCCESS);
...
#define GPIO_NUM                     80
#define GPIO_SPEED_TEST_LOOP_NUM     16
//*****************************************************************************
//
// Test the speed of GPIO
//
//*****************************************************************************
void
gpio_test_speed(void)
{
    am_util_stdio_printf("\nGPIO:%d Speed Test :\n", GPIO_NUM);

    uint32_t ui32TestRet = AM_HAL_STATUS_SUCCESS;
    uint32_t ui32Output = GPIO_NUM;
    ...
    if ( ui32Output < 32 )
    {
        ...
        for (uint32_t ui32LoopCnt = 0; ui32LoopCnt <= GPIO_SPEED_TEST_LOOP_NUM; ui32LoopCnt++)
        {
            GPIO->WTS0 = 1 << ui32OffSet;
            GPIO->WTC0 = 1 << ui32OffSet;
            GPIO->WTS0 = 1 << ui32OffSet;
            GPIO->WTC0 = 1 << ui32OffSet;
            GPIO->WTS0 = 1 << ui32OffSet;
            GPIO->WTC0 = 1 << ui32OffSet;
    ...
    else if ( ui32Output < 64 )
    {
        ...
        for (uint32_t ui32LoopCnt = 0; ui32LoopCnt <= GPIO_SPEED_TEST_LOOP_NUM; ui32LoopCnt++)
        {
            GPIO->WTS1 = 1 << ui32OffSet;
            GPIO->WTC1 = 1 << ui32OffSet;
            GPIO->WTS1 = 1 << ui32OffSet;
            GPIO->WTC1 = 1 << ui32OffSet;
            GPIO->WTS1 = 1 << ui32OffSet;
            GPIO->WTC1 = 1 << ui32OffSet;
    ...
    else if ( ui32Output < 96 )
    {
        ...
        for (uint32_t ui32LoopCnt = 0; ui32LoopCnt <= GPIO_SPEED_TEST_LOOP_NUM; ui32LoopCnt++)
        {
            GPIO->WTS2 = 1 << ui32OffSet;
            GPIO->WTC2 = 1 << ui32OffSet;
            GPIO->WTS2 = 1 << ui32OffSet;
            GPIO->WTC2 = 1 << ui32OffSet;
            GPIO->WTS2 = 1 << ui32OffSet;
            GPIO->WTC2 = 1 << ui32OffSet;
    ...
    else
    {
        ...
        for (uint32_t ui32LoopCnt = 0; ui32LoopCnt <= GPIO_SPEED_TEST_LOOP_NUM; ui32LoopCnt++)
        {
            GPIO->WTS3 = 1 << ui32OffSet;
            GPIO->WTC3 = 1 << ui32OffSet;
            GPIO->WTS3 = 1 << ui32OffSet;
            GPIO->WTC3 = 1 << ui32OffSet;
            GPIO->WTS3 = 1 << ui32OffSet;
            GPIO->WTC3 = 1 << ui32OffSet;
