...
IomPsramTask(void *pvParameters)
{
    ...
    for ( i = 0; i < IOM_MODULE_TOTAL; i++ )
    {
        ...
        //Bypass IOM2 and IOM3 as pin confliction with MSPI interface.
        if ( (i == 3) || ( i == 2 )  )
        {
            continue;
        }
        ...
        TEST_ASSERT_TRUE(g_semDMAFinish[i] != NULL );
    ...
    while(1)
    {
        ...
        // Verify Read PSRAM data
            ...
            {
                TEST_ASSERT_TRUE(gPatternBuf.bytes[j] == gRxBuf[i].bytes[j]);
                if (gPatternBuf.bytes[j] != gRxBuf[i].bytes[j])
                {
                    am_util_stdio_printf("IOM%d: Receive Data Compare failed at offset %d - Expected = 0x%x, Received = 0x%x\n",
                                         i, j, gPatternBuf.bytes[j], gRxBuf[i].bytes[j]);
                }
            }
#endif
        }
        ...
        {
            am_util_stdio_printf("IOM psram task is running...\n");
        }
    }
    ...
    for ( i = 0; i < IOM_MODULE_TOTAL; i++ )
    {
        ...
        vSemaphoreDelete(g_semDMAFinish[i]);
    }
