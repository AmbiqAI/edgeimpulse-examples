...
//!   Apollo5b FPGA Turbo
//!   ------------------
//!   * Uncomment #define APOLLO5_FPGA in mcu/apollo5b/am_mcu_apollo.h and
//!      change the value to the FPGA image clock in MHz, then rebuild HAL
//!   * Set BOARD?=apollo5b_fpga_turbo in example-defs.mk
...
//!   * Set BOARD?=apollo5b_eb_revb in example-defs.mk
...
AM_SHARED_RW  uint32_t        ui32DMATCBBuffer[4096];
...
globalSetUp(void)
{
    ...
    am_bsp_touch_als_enable(); // Touch not supported for RevB FPGA.
#endif //APOLLO5_FPGA
