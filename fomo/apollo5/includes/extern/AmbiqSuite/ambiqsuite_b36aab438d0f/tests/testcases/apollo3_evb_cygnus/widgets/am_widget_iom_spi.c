                                  ...
                                  void **ppWidget, char *pErrStr)
{
    ...
    *ppWidget = &g_IomSpiWidget;
    ...
    ui32Status = am_devices_w25q64fw_init(pTestCfg->ui32Module, &pTestCfg->IOMConfig, &g_IomDevHdl, &g_pIOMHandle);
    ui32Status = (ui32Status == IOM_WIDGET_DEVICE_SUCCESS) ? AM_WIDGET_SUCCESS : AM_WIDGET_ERROR;

    if (AM_WIDGET_SUCCESS == ui32Status)
    {
        am_devices_w25q64fw_reset(g_IomDevHdl);
    }
...
uint32_t am_widget_iom_test_cleanup(void *pWidget, char *pErrStr)
{
    ...
    am_devices_w25q64fw_term(g_IomDevHdl);
...
am_widget_iom_test_read_id(void *pWidget, char *pErrStr)
{
    ...
    return ((am_devices_w25q64fw_read_id(g_IomDevHdl, &ui32DeviceId) || ( ui32DeviceId != AM_DEVICES_W25Q64FW_ID )) ? AM_WIDGET_ERROR: AM_WIDGET_SUCCESS);
}
...
am_widget_iom_test_blocking_write_read(void *pWidget, void *pCfg, char *pErrStr)
{
    ...
    //
    // Erase the sector in the SPI Flash device.
    //
    ui32Status = am_devcies_w25q64fw_sector_erase(g_IomDevHdl, 0);

    //
    // Write the buffer to the SPI Flash device.
    //
    ui32Status = am_devices_w25q64fw_blocking_write(g_IomDevHdl, &g_pucOutBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
    ...
    if (ui32Status == 0)
    {
        ...
        //
        ui32Status = am_devices_w25q64fw_blocking_read(g_IomDevHdl, &g_pucInBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize);
    }
...
am_widget_iom_test_nonblocking_write_read(void *pWidget, void *pCfg, char *pErrStr)
{
    ...
    ui32Status = am_devices_w25q64fw_nonblocking_write(g_IomDevHdl, 
    ...
    if (ui32Status == 0)
    {
        ...
        ui32Status = am_devices_w25q64fw_nonblocking_read(g_IomDevHdl, &g_pucInBuffer[0] + ui32Offset, 0, pTestCfg->ui32BufferSize, pfnWidget_IOM_Callback_Read, 0);
...
am_widget_iom_test_queue_write_read(void *pWidget, void *pCfg, char *pErrStr)
{
    ...
    while ((0 == ui32Status) && (ui32Bytes < pTestCfg->ui32BufferSize))
    {
        ...
        ui32Status = am_devices_w25q64fw_nonblocking_write(g_IomDevHdl, &g_pucOutBuffer[0] + ui32Offset + ui32Bytes, ui32Bytes, size, 0, 0);
        ui32Bytes += size;
    }
    ...
    if (0 == ui32Status)
    {
        ...
        {
            uint32_t size = (blockSize < (pTestCfg->ui32BufferSize - ui32Bytes)) ? blockSize : (pTestCfg->ui32BufferSize - ui32Bytes);
            ui32Status = am_devices_w25q64fw_nonblocking_read(g_IomDevHdl, &g_pucInBuffer[0] + ui32Offset + ui32Bytes, ui32Bytes, size,
...
am_widget_iom_test_queue_write_read_error(void *pWidget, void *pCfg, char *pErrStr, uint32_t errorMask)
{
    ...
    // Must limit the block sizes to the SPI Flash page size.
    if (blockSize > AM_DEVICES_W25Q64FW_PAGE_SIZE)
    {
        blockSize = AM_DEVICES_W25Q64FW_PAGE_SIZE;
    }
    ...
    while ((0 == ui32Status) && (ui32Bytes < pTestCfg->ui32BufferSize))
    {
        uint32_t size =
            ...
            if (errorMask & AM_WIDGET_IOM_ERRMASK_DMA_START) // Send SPI Write - with DMA from illegal address
            {
                ...
                ui32Status = am_devices_w25q64fw_nonblocking_write(g_IomDevHdl, (uint8_t *)(SRAM_BASEADDR + SRAM_SIZE), ui32Bytes, size, errCallback, (void *)expectedErr);
                if ( ui32Status )
                {
                    return AM_WIDGET_ERROR;
                }
            ...
            {
                uint32_t expectedErr = AM_HAL_STATUS_FAIL;
                ui32Status = am_devices_w25q64fw_nonblocking_write(g_IomDevHdl, (uint8_t *)(SRAM_BASEADDR + SRAM_SIZE - size/2), ui32Bytes, size, errCallback, (void *)expectedErr);
                if ( ui32Status )
                {
                    return AM_WIDGET_ERROR;
                }
    ...
    //
    // Write the buffer to the SPI Flash without error.
    //
    ui32Bytes = 0;
    while ((0 == ui32Status) && (ui32Bytes < pTestCfg->ui32BufferSize))
    {
    ...
    if (0 == ui32Status)
    {
        ...
        while ((ui32Status == 0) && (ui32Bytes < pTestCfg->ui32BufferSize))
        {
            uint32_t size =
                ...
                {
                    uint32_t expectedErr = AM_HAL_STATUS_FAIL;
                    ui32Status = am_devices_w25q64fw_nonblocking_read(g_IomDevHdl, (uint8_t *)(SRAM_BASEADDR + SRAM_SIZE), ui32Bytes, size, errCallback, (void *)expectedErr);
                    if ( ui32Status )
                    {
                        return AM_WIDGET_ERROR;
                    }
                ...
                {
                    uint32_t expectedErr = AM_HAL_STATUS_FAIL;
                    ui32Status = am_devices_w25q64fw_nonblocking_read(g_IomDevHdl, (uint8_t *)(SRAM_BASEADDR + SRAM_SIZE - size/2), ui32Bytes, size, errCallback, (void *)expectedErr);
                    if ( ui32Status )
                    {
                        return AM_WIDGET_ERROR;
                    }
