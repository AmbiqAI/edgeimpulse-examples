...
/* configUSE_STATIC_ALLOCATION is set to 1, so the application must provide an
 * implementation of vApplicationGetTaskMemory() to provide the memory that is
 * used by the Idle task. */
void vApplicationGetTaskMemory( StaticTask_t ** ppxTaskTCBBuffer,
...
#ifdef REALLOC_RTOS_STACK
    static StackType_t uxTaskStack[ WORKLOAD_BOOTUP_TASK_STACK_SIZE ]  __attribute__ (( section(".rtos_stack") ));
#else
    static StackType_t uxTaskStack[ WORKLOAD_BOOTUP_TASK_STACK_SIZE ];
#endif

    /* Pass out a pointer to the StaticTask_t structure in which the Idle
     * task's state will be stored. */
    *ppxTaskTCBBuffer = &xTaskTCB;

    /* Pass out the array that will be used as the Idle task's stack. */
    *ppxTaskStackBuffer = uxTaskStack;

    /* Pass out the size of the array pointed to by *ppxIdleTaskStackBuffer.
     * Note that, as the array is necessarily of type StackType_t,
     * configMINIMAL_STACK_SIZE is specified in words, not bytes. */
    *pulTaskStackSize = WORKLOAD_BOOTUP_TASK_STACK_SIZE;
}
...
void vStartCoremark( void *pvParameters )
{
...
#ifdef CSV_PRINT
    am_util_stdio_printf("Test Num,CPU->CACHECFG,CPU->DAXICFG,CacheEnable,CacheLRU,CacheIEnable,CacheDEnable,CacheDescr,PASSTHROUGH,AGINGSENABLE,BUFFER,AGINGCOUNTER,FLUSHLEVEL(Free Buffer),WorkLoad,Time (secs)\n");
#endif
#ifdef CACHE_TEST
    for (j = 0; j < sizeof(g_sCacheCfg) / sizeof(cache_enable_config_t); j++)
    {
        ...
        for (i = 0; i < sizeof(g_sDaxiCfg) / sizeof(am_hal_daxi_config_t); i++)
        {
            ...
            argv[3] = "-v1";
            cjpeg_rose7_preset_main(argc, argv);
#ifndef ONLY_RUN_JPEG_ONE_ITERATION
            argv[0] = "core";
            argv[1] = "-c1";
            argv[2] = "-w1";
            argv[3] = "-v1";
...
run_tasks(void)
{
	...
	{
		StaticTask_t *pxTaskTCBBuffer = NULL;
		StackType_t *pxTaskStackBuffer = NULL;
		uint32_t ulTaskStackSize;

		/* The task is created using user provided RAM - obtain the
		address of the RAM then create the idle task. */
		vApplicationGetTaskMemory( &pxTaskTCBBuffer, &pxTaskStackBuffer, &ulTaskStackSize );
		/*xIdleTaskHandle = */xTaskCreateStatic( vStartCoremark,
												"coremark_pro",
												ulTaskStackSize,
												( void * ) NULL, /*lint !e961.  The cast is not redundant for all compilers. */
												WORKLOAD_BOOTUP_TASK_PRIORITY, 
												pxTaskStackBuffer,
												pxTaskTCBBuffer ); /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
