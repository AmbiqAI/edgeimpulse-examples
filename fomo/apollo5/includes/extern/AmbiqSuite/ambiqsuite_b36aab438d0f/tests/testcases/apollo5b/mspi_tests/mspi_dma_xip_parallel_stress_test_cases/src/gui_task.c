...
//DSP ram region am_util_stdio_printf("GUI task start!\n");
...
//*****************************************************************************
//
// Global Variables am_util_stdio_printf("GUI task start!\n");
//*****************************************************************************
//Textures
...
#ifdef ENABLE_ALLOC_BUFFER_IN_PSRAM
int
PSRAM_buffer_alloc(img_obj_t* img, uint32_t ui32AddrOffset)
{
    uint32_t size;

    img->bo.base_phys = PSRAM_FREAM_BUFFER_ADDR + ui32AddrOffset;
    img->bo.base_virt = (void*)img->bo.base_phys;
    img->bo.size = nema_texture_size(img->format, 0, img->w, img->h);
...
texture_framebuffer_init(void)
{
    ...
    if (memcmp((void*)PSRAM_W3_21x182_RGBA,      w3_21x182_rgba,      w3_21x182_rgba_length) !=0)
    {
        am_util_stdio_printf("---w3_21x182_rgba memcpy error\n");
        vErrorHandler();
    }
    ...
    if (memcmp((void*)PSRAM_W2_28x174_RGBA,      w2_28x174_rgba,      w2_28x174_rgba_length) !=0)
    {
        am_util_stdio_printf("---w2_28x174_rgba memcpy error\n");
        vErrorHandler();
    }
    ...
    if (memcmp((void*)PSRAM_W1_28x125_RGBA,      w1_28x125_rgba,      w1_28x125_rgba_length) !=0)
    {
        am_util_stdio_printf("---w1_28x125_rgba memcpy error\n");
        vErrorHandler();
    }
    ...
    if (memcmp((void*)PSRAM_W0_360x360_RGBA565,      w0_360x360_rgba565,      w0_360x360_rgba565_length) !=0)
    {
        am_util_stdio_printf("---w1_28x125_rgba memcpy error\n");
        vErrorHandler();
    }
    ...
    if (memcmp((void*)PSRAM_W0_360x196_RGBA4444,      w0_360x196_rgba4444,      w0_360x196_rgba4444_length) !=0)
    {
        am_util_stdio_printf("---w0_360x196_rgba4444 memcpy error\n");
        vErrorHandler();
    }
...
#ifdef ENABLE_ALLOC_BUFFER_IN_PSRAM
    ret = PSRAM_buffer_alloc(&g_sFrameBuffer[0], 0);
#else
...
#else
#ifdef ENABLE_ALLOC_BUFFER_IN_PSRAM
    ret = PSRAM_buffer_alloc(&g_sFrameBuffer[1], 0x60000);
#else
...
#if defined( LOAD_TEXTURE_BY_MSPI_DMA )
