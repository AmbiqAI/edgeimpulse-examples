...
am_ctimer_isr(void)
{
    ...
    if(ui32Status & (1 << CTIMER_INTSET_CTMRA0C0INT_Pos))
    {
        g_bTimeOut = 1;
    }
...
{
    //
    // Toggle the gpio for active.
    //
    am_hal_gpio_state_write(29, AM_HAL_GPIO_OUTPUT_TOGGLE);
...
setup_task(void *pvParameters)
{
    ...
    //
    xTaskCreate(RadioTask, "RadioTask", 512, 0, 2, &radio_task_handle);
...
run_tasks(void)
{
    ...
    BurstmodeSetup();
    xTaskCreate(BurstmodeTask, "BurstModeTask", 512, 0, 1, &Burst_mode_task_handle);
/*
    if(0 == SpiFramSetup())
    {
        xTaskCreate(SpiFramTask, "SpiFramTask", 512, 0, 2, &Spi_fram_task_handle);
    }
*/
    if(0 == I2cFramSetup())
    {
        xTaskCreate(I2cFramTask, "I2cFramTask", 512, 0, 2, &I2c_fram_task_handle);
    }

    xTaskCreate(IosLoopTask, "IosTask", 512, 0, 3, &Ios_task_handle);

    //
    // Set up the test output pin #29
    //
    am_hal_gpio_pinconfig(29, g_AM_HAL_GPIO_OUTPUT);
