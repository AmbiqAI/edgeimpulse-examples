...
//! 
//! HARDWARE CONFIG:
//! Required Hardware Fly-wire connections when PROCESS_SENSORS_TASK_ENABLE is 1:
//!
...
void sensor_hub_test(void)
{
    ...
    #warning Workaround for CAYNSWS-1225
    //
    // Use ARM sleep mode since the test is using Systick-based
    // tick with default tickless idle.  Eventually we need to use
    // Ambiq sleep with Ambiq tickless idle mode, but currently we cannot
    // due to STimer hang issues
    //
