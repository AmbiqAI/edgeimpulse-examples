...
//
mspi_psram_device_func_t mspi_psram_xip_device_func = 
{
...
uint32_t mspi_psram_xip_code_init(void)
{
    ...
    am_util_stdio_printf("PSRAM device name is %s, MSPI#%d interface mode is %s @ %s MHz !\n\n",\
                        mspi_psram_xip_device_func.devName,         \
    ...
    if ( AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS == mspi_psram_xip_device_func.mspi_init_timing_check(MSPI_PSRAM_XIPCODE_MODULE, &g_sMspiPsramConfig, &MSPIDdrTimingConfig) )
    {
#if defined(PSRAM_APS25616N)
    ...
    //
    ui32Status = mspi_psram_xip_device_func.mspi_init(MSPI_PSRAM_XIPCODE_MODULE, &g_sMspiPsramConfig, &g_pPsramXipCodeHandle, &g_pMSPIPsramXipCodeHandle);
    ...
    // MSPIDdrTimingConfig.sTimingCfg.bTxNeg = 1;
    // MSPIDdrTimingConfig.sTimingCfg.bRxNeg = 1;
    // MSPIDdrTimingConfig.sTimingCfg.bRxCap = 0;
    // MSPIDdrTimingConfig.sTimingCfg.ui8TxDQSDelay = 1;
    // MSPIDdrTimingConfig.sTimingCfg.ui8RxDQSDelay = 2;
    // MSPIDdrTimingConfig.sTimingCfg.ui8Turnaround = 10;

    ui32Status = mspi_psram_xip_device_func.mspi_init_timing_apply(g_pPsramXipCodeHandle, &MSPIDdrTimingConfig);
...
#ifdef MSPI_XIP_TASK_SCRAMBLING_ENABLE
    //
    // Enable MSPI scrambling.
    //
    ui32Status = mspi_psram_xip_device_func.mspi_scrambling_enable(g_pPsramXipCodeHandle);
    if (AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS != ui32Status)
    {
        am_util_stdio_printf("Failed to enable Scrambling in the MSPI:%d!\n", MSPI_PSRAM_XIPCODE_MODULE);
        return MSPI_PSRAM_STATUS_ERROR;
    }
#endif
