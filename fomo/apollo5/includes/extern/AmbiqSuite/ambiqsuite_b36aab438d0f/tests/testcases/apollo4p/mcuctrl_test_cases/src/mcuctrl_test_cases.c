...
mcuctrl_test_device_info(void)
{
    ...
    if ( _FLD2VAL(MCUCTRL_CHIPPN_PN, sMcuCtrlDevice.ui32ChipPN) !=
          MCUCTRL_CHIPPN_PN_APOLLO4 )
    {
...
#elif defined(AM_PART_APOLLO4P)
    if ( _FLD2VAL(MCUCTRL_CHIPREV_REVMAJ, sMcuCtrlDevice.ui32ChipRev) != MCUCTRL_CHIPREV_REVMAJ_C )
#endif
    {
...
#if APOLLO4_FPGA
    //
    // The FPGA images are not consistent on 2 SKU fields, SKUMIPIDSI and
    // SKUTURBOSPOT. So ignore these for the FPGA.
    //
    if ( (sMcuCtrlDevice.ui32SKU & ~(MCUCTRL_SKU_SKUMIPIDSI_Msk | MCUCTRL_SKU_SKUTURBOSPOT_Msk)) !=
         (APOLLO4_SKU_EXPECTED   & ~(MCUCTRL_SKU_SKUMIPIDSI_Msk | MCUCTRL_SKU_SKUTURBOSPOT_Msk)) )
    {
