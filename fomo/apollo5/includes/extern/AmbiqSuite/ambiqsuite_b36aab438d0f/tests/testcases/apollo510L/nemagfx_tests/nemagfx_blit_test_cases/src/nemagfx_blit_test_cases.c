...
#include "expected_data_clamp.h"
#include "expected_data_repeat.h"
#include "expected_data_border.h"
#include "expected_data_mirror.h"
//*****************************************************************************
//
// Macro definitions
//
...
uint32_t* g_gpu_check_table[4] = { (uint32_t*)expected_data_clamp, (uint32_t*)expected_data_repeat, (uint32_t*)expected_data_border, (uint32_t*)expected_data_mirror};

bool checkdata(uint32_t* dest_data)
{
    bool bPass = true;
    uint32_t * buff;
    uint32_t i;
...
nemagfx_blit_test(void)
{
    ...
    for(ui32SampleType = 0; ui32SampleType < 4; ui32SampleType++) // Removed MIRROR test
    {
        ...
        nema_cl_wait(&cl);
        nema_cl_rewind(&cl);
        if (checkdata(g_gpu_check_table[ui32SampleType]) == false)
        {
            bTestPass = false;
        }
