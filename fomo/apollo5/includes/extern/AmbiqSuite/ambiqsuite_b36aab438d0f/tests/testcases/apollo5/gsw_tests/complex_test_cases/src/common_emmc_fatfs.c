...
FATFS g_sEMMCFatFs;  /* Filesystem object */
uint8_t pui8FormatWorkBuf[FF_MAX_SS]; /* working buffer for format process */
am_hal_card_t eMMCs;
...
bool scrub_fs(void)
{
    ...
    res = f_mount(&g_sEMMCFatFs, "", 0);
    if (res != FR_OK)
        return false;
    res = f_mkfs("", 0, pui8FormatWorkBuf, sizeof(pui8FormatWorkBuf));
    if (res != FR_OK)
        return false;
