//*****************************************************************************
//
//! @file sdio_rs9116_emmc_multiple_instances_test_cases.c
//!
//! @brief Run sdio rs9116 emmc multiple instances test cases.
//!
//! This test includes emmc test and sdio_rs9116 wifi interface tasks. We delete all of tasks except setup tast
...
am_widget_timing_scan_hashtable_t rs9116_timing_table[DUT_NUM][SCAN_NUM];
am_hal_card_t sdio_rs9116[DUT_NUM];

am_widget_sdio_devices_config_t Rs9116_setting[DUT_NUM] =
{
  {
    .eCardPwrCtrlPolicy = AM_HAL_CARD_PWR_CTRL_SDHC_OFF,
    .pCardPwrCtrlFunc = NULL,
    .bAsyncWriteIsDone = false,
    .bAsyncReadIsDone = false,
  },
...
setUp(void)
{
  for (uint32_t index = 0; index < DUT_NUM; index++)
  {
    if ( index == 0 )
    {
      ...
      DUTs_setting[index].eCardType = AM_HAL_CARD_TYPE_SDIO;
      DUTs_setting[index].device_config = &Rs9116_setting[index];

      g_DUTs[index].pDevHandle = &sdio_rs9116[index];
      am_widget_setupTimingScanTable(rs9116_timing_table[index]);
#else
...
sdio_get_card_info_test(void)
{
    ...
    uint32_t index = AM_HAL_SDHC_CARD_HOST;

#if EMMC_USE_SDIO_HOST1
    index = AM_HAL_SDHC_CARD_HOST1;
#endif
...
int
sdio_rs9116_emmc_multiple_instances_test_cases(void)
{ 
    //
    ...
    //
    am_util_stdio_printf("sdio rs9116 emmc multiple instances test cases\n");
