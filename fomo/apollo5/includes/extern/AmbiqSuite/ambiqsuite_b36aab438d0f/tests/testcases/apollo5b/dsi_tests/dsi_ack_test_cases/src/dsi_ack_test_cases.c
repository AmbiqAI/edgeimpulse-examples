...
// #### INTERNAL BEGIN ####
    ...
    //
    if (dphy_init_with_para(DPHY_X6) != 0) 
    {
        bPass = false;
        return bPass;
    }
...
// #### INTERNAL END ####
    ...
    if (eFormat == FMT_RGB888)
    {
        nema_memcpy((char*)layer0.baseaddr_virt, windmill_200x200_rgba8888, sizeof(windmill_200x200_rgba8888));
    }
    else if (eFormat == FMT_RGB565)
    {
        nema_memcpy((char*)layer0.baseaddr_virt, windmill_200x200_rgba565, sizeof(windmill_200x200_rgba565));
    }
    else
    {
        //
...
dsi_ack_test(void)
{
    ...
    uint8_t ui8Width[2] = {16, 8};
    uint32_t ui32Trim[6] = {0x06, 0x07, 0x08, 0x09, 0x0A, 0x05};
        ...
        for(ui8I = 0; ui8I < NUM_OF_WIDTH_OPTIONS; ui8I++)
        {
            for(ui8K = 0; ui8K < NUM_OF_COLOR_MODES; ui8K++)
            {
                ...
                for(ui8J = 0; ui8J < NUM_OF_TRIM_OPTIONS; ui8J++)
                {
                    ...
                    {
                        am_util_stdio_printf("Higher than maximum bit rate which RM69330 supports, Skipped %d-Lane, %d-bit DBI width, Frequence trim value - 0x%02X, RGB565 DSI test.\n", ui8Lanes, ui8Width[ui8I], ui32Trim[ui8J]);
                    }
                    else
                    {
