...
extern volatile bool g_bMveTaskComplete;
...
{
    am_hal_gpio_output_set(GPIO_SLEEP_ENTRY);
...
{
    am_hal_gpio_output_clear(GPIO_SLEEP_ENTRY);
...
vApplicationIdleHook(void)
{
    ...
    if (g_bHelloTaskComplete && g_bTimerTaskComplete && g_bMveTaskComplete)
    {
...
setup_task(void *pvParameters)
{
    ...
    xTaskCreate(MveTask, "MveTask", 512, 0, TASK_PRIORITY_LOW, &mve_task_handle);
