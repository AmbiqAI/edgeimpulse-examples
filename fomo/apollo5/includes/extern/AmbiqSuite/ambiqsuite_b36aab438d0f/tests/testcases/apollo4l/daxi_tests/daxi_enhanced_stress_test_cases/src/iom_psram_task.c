...
IomPsramTask(void *pvParameters)
{
    ...
    {    
        ...
        TEST_ASSERT_TRUE(g_semDMAFinish[instance] != NULL );
    ...
    while(1)
    {
        for (i = 0; i < IOM_TEST_INSTANCE_TOTAL; i++)
        {
            ...
            {
                TEST_ASSERT_TRUE(gPatternBuf.bytes[j] == gRxBuf[instance].bytes[j]);
    ...
    for ( i = 0; i < IOM_TEST_INSTANCE_TOTAL; i++ )
    {
        ...
        vSemaphoreDelete(g_semDMAFinish[iom_test_instances[i]]);
    }
