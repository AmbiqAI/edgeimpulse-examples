...
uint32_t iom_instances[] = {0};
#else
uint32_t iom_instances[] = {0,1,2,3,5,6,7};
#endif
...
iom_test_globalSetUp(void)
{
    ...
    // am_util_stdio_printf("Apollo5 IOM Test Cases %p\n\n", g_IomRegs[0].pRegFIFO);
...
#if (ENABLE_BURST_MODE == 1)
    {
      ...
      if ( eCurrentPowerMode == AM_HAL_PWRCTRL_MCU_MODE_LOW_POWER )
      {
