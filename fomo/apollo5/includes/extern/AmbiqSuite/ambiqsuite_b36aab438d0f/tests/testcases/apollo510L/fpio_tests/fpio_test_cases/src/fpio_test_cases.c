//*****************************************************************************
//
//! @file fpio_test_cases.c
//!
//! @brief The fpio tests conducted are
//!                    fpio_test_config - Test if GPIO pins can be configured.
//!                    fpio_test_connectivity - Test GPIO loopback connectivity
...
#include "fpio_test_cases.h"

#ifdef APOLLO5_FPGA

#define USE_GSW_IMGAE
// #define USE_DISP_MSPI_SDIO_IMAGE
#define FPIO_SPEED_TEST

#ifdef USE_GSW_IMGAE
#include "gsw_pinout.h"
#elif  defined(USE_DISP_MSPI_SDIO_IMAGE)
...
const gpio_loopback_t LoopbackPairs[] =
{
    ...
    // GP_04 isn't used in GSW image
    {GP_06, GP_04},
    // GP_29 isn't used in GSW image
    {GP_29, GP_27},
    // GP_38 GP_39 isn't used in GSW image
    {GP_38, GP_39},
    // GP_40 GP_41 isn't used in GSW image
    {GP_41, GP_40},
    // GP_72 isn't used in GSW image
    {GP_58, GP_72},
    // GP_73 isn't used in GSW image
    {GP_59, GP_73},
#else
    {GP_06, GP_27},
    {GP_58, GP_59},
...
//
// when testing on FPGA, it is best to connect GPIOs further apart or using wires instead of jumpers
//
#else // APOLLO5_FPGA
...
const gpio_loopback_t LoopbackPairs[] =
{
    ...
    // Jump wire
    // {61, 62}         // Skip GPIO 61 62 94, See CAYNSWS-2178
    // {GP165, } GP165 is used by bootloader
    { 138, 144},
    {57,  59},
};
...
//*****************************************************************************
//
// Write FPIO state values
//
//*****************************************************************************
uint32_t
am_hal_fpio_state_write(uint32_t ui32GpioNum, am_hal_gpio_write_type_e eWriteType)
{
    //
    ...
    switch (eWriteType)
    {
        case AM_HAL_GPIO_OUTPUT_CLEAR:
            am_hal_fpio_output_clear(ui32GpioNum);
            break;

        case AM_HAL_GPIO_OUTPUT_SET:
            am_hal_fpio_output_set(ui32GpioNum);
            break;

        case AM_HAL_GPIO_OUTPUT_TOGGLE:
            am_hal_fpio_output_toggle(ui32GpioNum);
            break;

        case AM_HAL_GPIO_OUTPUT_TRISTATE_OUTPUT_DIS:
            am_hal_fpio_output_tristate_output_dis(ui32GpioNum);
            break;

        case AM_HAL_GPIO_OUTPUT_TRISTATE_OUTPUT_EN:
            am_hal_fpio_output_tristate_output_en(ui32GpioNum);
            break;

        case AM_HAL_GPIO_OUTPUT_TRISTATE_OUTPUT_TOG:
            am_hal_fpio_output_tristate_output_tog(ui32GpioNum);
            break;
    }

    return AM_HAL_STATUS_SUCCESS;

} // am_hal_fpio_state_write()

//*****************************************************************************
//
// Read FPIO state values
//
//*****************************************************************************
uint32_t
am_hal_fpio_state_read(uint32_t ui32GpioNum, am_hal_gpio_read_type_e eReadType,
                       uint32_t *pui32ReadState)
{
    ...
    switch (eReadType)
    {
        case AM_HAL_GPIO_INPUT_READ:
            pui32Target = AM_HAL_FPIO_RDn(ui32GpioNum);
            break;

        case AM_HAL_GPIO_OUTPUT_READ:
            pui32Target = AM_HAL_FPIO_WTn(ui32GpioNum);
            break;

        case AM_HAL_GPIO_ENABLE_READ:
            pui32Target = AM_HAL_FPIO_ENn(ui32GpioNum);
            break;
...
#ifdef FPIO_SPEED_TEST

#define FPIO_NUM                     80
#define FPIO_SPEED_TEST_LOOP_NUM     16

//*****************************************************************************
//
// Test the speed of FPIO
//
//*****************************************************************************
void
fpio_test_speed(void)
{
    am_util_stdio_printf("\nFPIO:%d Speed Test :\n", FPIO_NUM);

    uint32_t ui32TestRet = AM_HAL_STATUS_SUCCESS;
    uint32_t ui32Output = FPIO_NUM;
    uint32_t ui32OffSet = 0;

    if ( ui32Output > AM_HAL_PIN_TOTAL_GPIOS )
    {
        am_util_stdio_printf("\nGPIO:%d exceed the GPIO number maximum:%d\n", ui32Output, AM_HAL_PIN_TOTAL_GPIOS);
        ui32TestRet = AM_HAL_STATUS_FAIL;
    }
    TEST_ASSERT_TRUE(ui32TestRet == AM_HAL_STATUS_SUCCESS);
    ...
    //
    // Configure output
    ui32TestRet = am_hal_gpio_pinconfig(ui32Output,sPushPull);
    TEST_ASSERT_TRUE(ui32TestRet == AM_HAL_STATUS_SUCCESS);

    if ( ui32Output < 32 )
    {
        ui32OffSet = ui32Output;
        for (uint32_t ui32LoopCnt = 0; ui32LoopCnt <= FPIO_SPEED_TEST_LOOP_NUM; ui32LoopCnt++)
        {
            FPIO->WTS0 = 1 << ui32OffSet;
            FPIO->WTC0 = 1 << ui32OffSet;
            FPIO->WTS0 = 1 << ui32OffSet;
            FPIO->WTC0 = 1 << ui32OffSet;
            FPIO->WTS0 = 1 << ui32OffSet;
            FPIO->WTC0 = 1 << ui32OffSet;
    ...
    else if ( ui32Output < 64 )
    {
        ui32OffSet = ui32Output - 32;
        for (uint32_t ui32LoopCnt = 0; ui32LoopCnt <= FPIO_SPEED_TEST_LOOP_NUM; ui32LoopCnt++)
        {
            FPIO->WTS1 = 1 << ui32OffSet;
            FPIO->WTC1 = 1 << ui32OffSet;
            FPIO->WTS1 = 1 << ui32OffSet;
            FPIO->WTC1 = 1 << ui32OffSet;
            FPIO->WTS1 = 1 << ui32OffSet;
            FPIO->WTC1 = 1 << ui32OffSet;
    ...
    else if ( ui32Output < 96 )
    {
        ui32OffSet = ui32Output - 64;
        for (uint32_t ui32LoopCnt = 0; ui32LoopCnt <= FPIO_SPEED_TEST_LOOP_NUM; ui32LoopCnt++)
        {
            FPIO->WTS2 = 1 << ui32OffSet;
            FPIO->WTC2 = 1 << ui32OffSet;
            FPIO->WTS2 = 1 << ui32OffSet;
            FPIO->WTC2 = 1 << ui32OffSet;
            FPIO->WTS2 = 1 << ui32OffSet;
            FPIO->WTC2 = 1 << ui32OffSet;
    ...
    {
        ui32OffSet = ui32Output - 96;
        for (uint32_t ui32LoopCnt = 0; ui32LoopCnt <= FPIO_SPEED_TEST_LOOP_NUM; ui32LoopCnt++)
        {
            FPIO->WTS3 = 1 << ui32OffSet;
            FPIO->WTC3 = 1 << ui32OffSet;
            FPIO->WTS3 = 1 << ui32OffSet;
            FPIO->WTC3 = 1 << ui32OffSet;
            FPIO->WTS3 = 1 << ui32OffSet;
            FPIO->WTC3 = 1 << ui32OffSet;
...
check_pin_state(uint32_t ui32DesiredState, uint32_t ui32Output, uint32_t ui32Input)
{
    ...
    {
        ui32CounterTimeout++;
        ui32PinStateIn = am_hal_fpio_input_read(ui32Input);
        ui32PinStateOut = am_hal_fpio_output_read(ui32Output);
...
check_push_pull(uint32_t ui32Output, uint32_t ui32Input)
{
    ...
    am_hal_fpio_state_write(ui32Output, AM_HAL_GPIO_OUTPUT_SET);
    ui32read_status = am_hal_fpio_state_read(ui32Input,AM_HAL_GPIO_INPUT_READ,&ui32read_val);
    ...
    ui32read_val = 1;
    // Using functions instead of Macros to test the gpio_state functions.
    am_hal_fpio_state_write(ui32Output, AM_HAL_GPIO_OUTPUT_CLEAR);
    ui32read_status = am_hal_fpio_state_read(ui32Input,AM_HAL_GPIO_INPUT_READ,&ui32read_val);
    //ui32read_val = am_hal_fpio_input_read(ui32Input);
...
check_open_drain(uint32_t ui32Output, uint32_t ui32Input)
{
    ...
    {
        //
        // For HS pads, only check 50 k omron pullup
        //
        ...
        ui32write_val = 1;
        ui32read_val = 0;
        am_hal_fpio_output_set(ui32Output);
        ...
        ui32write_val = 0;
        ui32read_val = 1;
        am_hal_fpio_output_toggle(ui32Output);
        ui32read_val = am_hal_fpio_input_read(ui32Input);
        am_util_stdio_printf("Pin = %d Set Low -- ",ui32Output);
        if (ui32read_val == ui32write_val)
        {
...
check_tristate(uint32_t ui32Output, uint32_t ui32Input)
{
    ...
    am_hal_fpio_output_tristate_output_dis(ui32Output);
    am_util_delay_ms(1);
    // Read input pin.
    ...
    am_hal_fpio_output_tristate_output_tog(ui32Output);
    am_util_delay_ms(1);
    // Drive output pin low.
    am_hal_fpio_output_clear(ui32Output);
    am_util_delay_ms(1);
    // Read input and output pins.
    ui32write_val = am_hal_fpio_output_read(ui32Output);
    ui32read_val = am_hal_fpio_input_read(ui32Input);
    ...
    // Enabling push-pull on output pin by setting ENn to 1.
    am_hal_fpio_output_tristate_output_en(ui32Output);
    am_util_delay_ms(1);
