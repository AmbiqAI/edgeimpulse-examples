...
//!
//! Pins    :   IOS                         IOM0
//! 0 to 5  :   IOS SPI SCK/I2C SCL     to  IOM0 SPI SCK/I2C SCL
//! 1 to 6  :   IOS SPI MOSI/ I2C SDA   to  IOM0 SPI MISO/I2C SDA
//! 2 to 7  :   IOS SPI MISO            to  IOM0 SPI MOSI
//! 3 to 11 :   IOS SPI nCE             to  IOM0 SPI nCE
//! 4 to 10 :   IOS Interrupt           to  Handshake Interrupt
//! @endverbatim
//
//*****************************************************************************
...
#define HANDSHAKE_IOM_PIN        10 // GPIO used to connect the IOSINT to host side
...
apollo3_iomce0[AM_REG_IOM_NUM_MODULES] =
{
    {AM_BSP_GPIO_IOM0_CS, g_AM_BSP_GPIO_IOM0_CS, AM_BSP_IOM0_CS_CHNL},
    {AM_BSP_GPIO_IOM1_CS, g_AM_BSP_GPIO_IOM1_CS, AM_BSP_IOM1_CS_CHNL},
    {AM_BSP_GPIO_IOM2_CS, g_AM_BSP_GPIO_IOM2_CS, AM_BSP_IOM2_CS_CHNL},
    {AM_BSP_GPIO_IOM3_CS, g_AM_BSP_GPIO_IOM3_CS, AM_BSP_IOM3_CS_CHNL},
    {AM_BSP_GPIO_IOM4_CS, g_AM_BSP_GPIO_IOM4_CS, AM_BSP_IOM4_CS_CHNL},
    {AM_BSP_GPIO_IOM5_CS, g_AM_BSP_GPIO_IOM5_CS, AM_BSP_IOM5_CS_CHNL}
};
#endif
...
iom_clean_up(void)
{
    ...
    if (g_sIosWidget.bSpi)
    {
      ...
      Transaction.ui32Instr       = 0xF8;
      Transaction.eDirection      = AM_HAL_IOM_TX;
#if (!defined(AM_PART_APOLLO4))
      Transaction.uPeerInfo.ui32SpiChipSelect = AM_BSP_IOM0_CS_CHNL;
#else
      Transaction.uPeerInfo.ui32SpiChipSelect = AM_HAL_GPIO_NCE_IOM0CE0;
#endif
    ...
    am_hal_gpio_interrupt_disable(AM_HAL_GPIO_MASKBIT(pGpioIntMask, HANDSHAKE_IOM_PIN));
...
#if (!defined(AM_PART_APOLLO4))
    AM_HAL_GPIO_MASKCREATE(GpioIntMask);
    am_hal_gpio_interrupt_enable(AM_HAL_GPIO_MASKBIT(pGpioIntMask, HANDSHAKE_IOM_PIN));
...
ios_set_up(am_widget_iosspi_stimulus_ios_t *pCfg)
{
    ...
    // Set up the IOSINT interrupt pin
    am_hal_gpio_pinconfig(HANDSHAKE_IOS_PIN, g_AM_BSP_IOS_GPIO_ENABLE);
...
am_gpio0_001f_isr(void)
{
    ...
    //
    ui32Status = am_hal_gpio_interrupt_status_get(false, pGpioIntMask);
    if ( ui32Status )   // Avoid compiler warning
    {
    }

    am_hal_gpio_interrupt_clear(pGpioIntMask);
    am_hal_gpio_interrupt_service(pGpioIntMask);
#elif AM_PART_APOLLO3
    uint64_t ui64Status;

    //
    // Read and clear the GPIO interrupt status.
    //
    ui32Status = am_hal_gpio_interrupt_status_get(false, &ui64Status);
    if ( ui32Status )   // Avoid compiler warning
    {
    }
...
#elif AM_PART_APOLLO4
    am_hal_gpio_mask_t IntStatus;

    ui32Status = am_hal_gpio_interrupt_status_get(AM_HAL_GPIO_INT_CHANNEL_0,
                                     false,
                                     &IntStatus);
...
verify_result(am_widget_iosspi_t *pIosSpiWidget)
{
...
#endif
        if (am_hal_ios_pui8LRAM[pIosSpiWidget->roBase + i] != ROBUFFER_INIT)
        {
            return true;
        }
    }
    ...
    {
        if (am_hal_ios_pui8LRAM[pIosSpiWidget->ahbRamBase + i] != AHBBUF_INIT)
        {
            return true;
        }
    }
    // Make sure the Hardware FIFOSIZ is 0
    fifoCtr = IOSLAVE->FIFOCTR_b.FIFOCTR;
...
iom_send(uint32_t address, uint32_t size)
{
    ...
    {
      Transaction.ui32InstrLen    = 1;
      Transaction.ui32Instr       = (0x80|address);
      Transaction.eDirection      = AM_HAL_IOM_TX;
      Transaction.uPeerInfo.ui32SpiChipSelect = AM_BSP_IOM0_CS_CHNL;
      Transaction.ui32NumBytes    = size;
...
static void ios_read(uint32_t address, uint32_t size)
{
    ...
    for (i = 0; i < size; i++)
    {
        readByte = am_hal_ios_pui8LRAM[address+i];
        // Read data and prepare to be sent back after processing
    ...
    void **ppWidget, char *pErrStr)
{
    ...
    *ppWidget = &g_sIosWidget;
    ...
    am_hal_gpio_interrupt_register(HANDSHAKE_IOM_PIN, hostint_handler);
