...
uint32_t sbr_lock_test(security_t *sec_mode, char *info_buf)
{
   // get the expected SBR lock status
    ...
    if (MCUCTRL->BOOTLOADER_b.SBRUNLOCK == lockStatus)
    {
...
uint32_t sbl_lock_test(security_t *sec_mode, char *info_buf)
{
    ...
    if (MCUCTRL->BOOTLOADER_b.SBLUNLOCK == lockStatus)
    {
...
uint32_t bootrom_copy_test(security_t *sec_mode, char *info_buf)
{
    ...
    lockStatus = MCUCTRL->BOOTLOADER_b.SBRUNLOCK;
...
uint32_t helper_copy_test(security_t *sec_mode, char *info_buf)
{
    ...
    if (read_result != 0x00000000 && MCUCTRL->BOOTLOADER_b.SBRUNLOCK == 0)
            rtn_status = AM_STATUS_FAILURE;
...
{
    #define WORD_WRITE_VAL 0xabcd4321

    uint32_t rtn_status = AM_STATUS_FAILURE;
    uint32_t ui32WriteWord = 0; // initialize variable to 0

    // br_util_write_word
    ((void (*)(uint32_t *, uint32_t))(0x0200ff00+0x2C+1))(&ui32WriteWord, WORD_WRITE_VAL);

    // br_util_read_word
    uint32_t ui32ReadWord = ((uint32_t (*)(uint32_t *))(0x0200ff00+0x28+1))(&ui32WriteWord);

    if ((ui32WriteWord == ui32ReadWord) && (ui32ReadWord == WORD_WRITE_VAL))
        rtn_status = AM_STATUS_SUCCESS;

    am_util_stdio_sprintf(info_buf, "WriteWord=0x%08X, ReadWord=0x%08X, Expected=0x%08X", ui32WriteWord, ui32ReadWord, WORD_WRITE_VAL);
    return rtn_status;
}
...
static uint32_t crypto_hostao_lock_test( security_t* sec_lcs, char* info_buf )
{
    ...
    // For Apollo5b BR, mask out the RESETUPONDEBUGDISABLE bit (See CAYNSWS-1413)
    uint32_t ui32Mask = ~CRYPTO_HOSTAOLOCKBITS_RESETUPONDEBUGDISABLE_Msk;
    
    for(uint32_t i = 0; i < 9; i++)
    {
        if ( (ui32hostaolockReg & ui32Mask) != (ui32refHostAoLockVal & ui32Mask))
        {
