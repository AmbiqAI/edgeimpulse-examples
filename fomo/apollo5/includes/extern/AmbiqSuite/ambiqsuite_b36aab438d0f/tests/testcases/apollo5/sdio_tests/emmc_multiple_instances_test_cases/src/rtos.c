...
TaskHandle_t emmc0_task_handle;
TaskHandle_t emmc1_task_handle;
...
{
    if( (eTaskGetState(emmc0_task_handle) == eDeleted) && (eTaskGetState(emmc1_task_handle) == eDeleted) && !g_bReportedResults)
    {
...
setup_task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        //
        g_semEMMC0Write = xSemaphoreCreateBinary();
        TEST_ASSERT_TRUE(g_semEMMC0Write != NULL);
        if ( g_semEMMC0Write == NULL )
        {
            am_util_stdio_printf("Create semphone failed!\n");
            vTaskDelete(NULL);
        }

        g_semEMMC0Read = xSemaphoreCreateBinary();
        TEST_ASSERT_TRUE(g_semEMMC0Read != NULL);
        if ( g_semEMMC0Read == NULL )
        {
            am_util_stdio_printf("Create semphone failed!\n");
            vTaskDelete(NULL);
        }
        g_semEMMC1Write = xSemaphoreCreateBinary();
        TEST_ASSERT_TRUE(g_semEMMC1Write != NULL);
        if ( g_semEMMC1Write == NULL )
        {
            am_util_stdio_printf("Create semphone failed!\n");
            vTaskDelete(NULL);
        }

        g_semEMMC1Read = xSemaphoreCreateBinary();
        TEST_ASSERT_TRUE(g_semEMMC1Read != NULL);
        if ( g_semEMMC1Read == NULL )
        {
            am_util_stdio_printf("Create semphone failed!\n");
            vTaskDelete(NULL);
        }
        ...
        //
        xTaskCreate(Emmc0Task, "Emmc0Task", 512, 0, 4, &emmc0_task_handle);

        xTaskCreate(Emmc1Task, "Emmc1Task", 512, 0, 4, &emmc1_task_handle);
