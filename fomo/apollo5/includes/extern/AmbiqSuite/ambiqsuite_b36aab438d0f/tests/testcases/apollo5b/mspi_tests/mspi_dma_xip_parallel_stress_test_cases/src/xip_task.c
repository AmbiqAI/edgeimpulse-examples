...
#define MAX_XIP_TASK_DELAY    (200 * 2)
...
#define MSPI_DMA_OPERATION_PIN      138
#define MSPI_XIPMM_OPERATION_PIN    195
...
#if defined(MSPI_DMA_XIP_PARALLEL_TEST)
#define MSPI_DMA_BUFFER_LEN               470*1024
uint8_t ui8XIPTXBuffer[AM_DEVICES_MSPI_PSRAM_PAGE_SIZE];
uint8_t ui8XIPRXBuffer[AM_DEVICES_MSPI_PSRAM_PAGE_SIZE];

AM_SHARED_RW uint8_t  ui8DMATXBuffer[MSPI_DMA_BUFFER_LEN]  __attribute__((aligned(32)));      // 500K SRAM TX buffer
AM_SHARED_RW uint8_t  ui8DMARXBuffer[MSPI_DMA_BUFFER_LEN]  __attribute__((aligned(32)));      // 500K SRAM RX buffer.
#endif
...
#endif
    am_hal_gpio_output_set(MSPI_XIPMM_OPERATION_PIN);
    testPassed = mspi_xipmm(pXIPTestCfg, errStr);
    am_hal_gpio_output_clear(MSPI_XIPMM_OPERATION_PIN);
...
dma_xip_parallel(am_widget_mspi_test_t *pDMATestCfg, am_widget_mspi_test_t *pXIPTestCfg)
{
  ...
  am_hal_gpio_output_set(MSPI_DMA_OPERATION_PIN);
  ...
  for (uint32_t i = 0; i < AM_WIDGET_MSPI_TIMEOUT; i++)
  {
    if (bDMAComplete)
    {
      am_hal_gpio_output_clear(MSPI_DMA_OPERATION_PIN);
      break;
    }
...
mspi_dma_xip_parallel_test(void)
{
    ...
    am_hal_gpio_output_clear(MSPI_DMA_OPERATION_PIN);
    am_hal_gpio_output_clear(MSPI_XIPMM_OPERATION_PIN);
    am_hal_gpio_pinconfig(MSPI_DMA_OPERATION_PIN, am_hal_gpio_pincfg_output);
    am_hal_gpio_pinconfig(MSPI_XIPMM_OPERATION_PIN, am_hal_gpio_pincfg_output);

    bool  testPassed = true;

    g_Test_controls.sDMATestConfig.NumBytes = MSPI_DMA_BUFFER_LEN;
    g_Test_controls.sDMATestConfig.ByteOffset = 0;
    g_Test_controls.sDMATestConfig.SectorAddress = PSRAM_DMA_ADDR_OFFSET;
