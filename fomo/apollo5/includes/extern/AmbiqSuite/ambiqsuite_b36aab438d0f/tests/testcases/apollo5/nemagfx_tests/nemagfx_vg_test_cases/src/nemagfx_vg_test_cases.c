...
//*****************************************************************************
//
// Test case run condition.
//
//*****************************************************************************
typedef struct
{
    am_hal_pwrctrl_mcu_mode_e cpu_mode;
    am_hal_pwrctrl_gpu_mode_e gpu_mode;
} RunCondition_t;

RunCondition_t g_sPowerMode[4] = 
{
    {AM_HAL_PWRCTRL_MCU_MODE_LOW_POWER, AM_HAL_PWRCTRL_GPU_MODE_LOW_POWER},
    {AM_HAL_PWRCTRL_MCU_MODE_HIGH_PERFORMANCE, AM_HAL_PWRCTRL_GPU_MODE_LOW_POWER},
    {AM_HAL_PWRCTRL_MCU_MODE_LOW_POWER, AM_HAL_PWRCTRL_GPU_MODE_HIGH_PERFORMANCE},
    {AM_HAL_PWRCTRL_MCU_MODE_HIGH_PERFORMANCE, AM_HAL_PWRCTRL_GPU_MODE_HIGH_PERFORMANCE},
};
...
#define TIGER_CRC 0xA1B6DAA6UL
#define TIGER_CRC_ROTATE 0x8CFA8655UL

const uint32_t g_ui32ShapePaintCrc[5][6] = 
{
    {0xB9CF1301, 0x4606CA7B, 0x6B6403C9, 0xFFB40D4F, 0x93A1E98B, 0xE5DC2DAF},
    {0xFCE4844F, 0x80BB5EF7, 0x0EDF0F22, 0x84390749, 0x8A46E2AF, 0xD80AC044},
    {0xBEC1E1B2, 0x8F21D758, 0x777C02A9, 0x894687FA, 0xD4042F8A, 0x07EFBA52},
    {0x09A71EB4, 0x1F8A2F76, 0x64B96CDA, 0x6631EC31, 0xC60EC88C, 0x6A4BF568},
    {0xA446A14A, 0x79225ABF, 0x8F0DFA7A, 0xD76FADD6, 0xCA57A3DF, 0xA05C5533},    
};

#define RAW_PATCH_CRC_H_EVEN_ODD 0x6D616FF2UL
#define RAW_PATCH_CRC_e_EVEN_ODD 0x87F3FC45UL
#define RAW_PATCH_CRC_H_NON_ZERO 0xA4BAA0C6UL
#define RAW_PATCH_CRC_e_NON_ZERO 0xE5CED46DUL

#define TTF_CRC_EVEN_ODD         0xEEC9796EUL
#define TTF_CRC_STROKE           0x163E455CUL
...
//
//*****************************************************************************
void set_cpu_gpu_power_mode(am_hal_pwrctrl_mcu_mode_e cpu_mode, am_hal_pwrctrl_gpu_mode_e gpu_mode)
{
    ...
    if(nema_error_code != NEMA_VG_ERR_NO_ERROR)
    {
        am_util_stdio_printf("nemagfx reinit failed: errorcode = %8X\n", nema_error_code);
    }

    // Check and print the CPU and GPU power mode
    RunCondition_t mode_status;
    am_hal_pwrctrl_mcu_mode_status(&mode_status.cpu_mode);
    am_hal_pwrctrl_gpu_mode_status(&mode_status.gpu_mode);
    am_util_stdio_printf("CPU frequency: %d\n", 
                         (mode_status.cpu_mode == AM_HAL_PWRCTRL_MCU_MODE_LOW_POWER) ? 96 : 250);
    am_util_stdio_printf("GPU frequency: %d\n", 
                         (mode_status.gpu_mode == AM_HAL_PWRCTRL_GPU_MODE_LOW_POWER) ? 96 : 250);
...
void draw_tiger_common(void* texture_location, float rotate_degree, float scale_x, float scale_y)
{
    ...
    for(uint32_t i = 0; i < sizeof(g_sPowerMode)/sizeof(g_sPowerMode[0]); i++)
    {
        am_util_stdio_printf("\n\n");

        set_cpu_gpu_power_mode(g_sPowerMode[i].cpu_mode, g_sPowerMode[i].gpu_mode);
...
{
    const float       stops[5]  = {0.0f, 0.25f, 0.50f, 0.75f, 1.0f};
    const color_var_t colors[5] =
    {
        {0, 0, 255, 255},     // blue
        {0, 255, 0, 255},     // green
