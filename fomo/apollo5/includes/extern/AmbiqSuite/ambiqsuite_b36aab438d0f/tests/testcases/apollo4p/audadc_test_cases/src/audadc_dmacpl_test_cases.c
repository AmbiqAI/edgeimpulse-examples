...
globalSetUp(void)
{
    ...
    am_util_stdio_printf("Apollo4p Audio ADC Test Cases\n\n");
}
...
#define MAX_DURATION (0.00267f * 1.5f)
...
audadc_config_local(void)
{
    ...
    //
#if XTALHS_EN
    am_util_stdio_printf("AM_HAL_AUDADC_CLKSEL_XTHS_24MHz(Crystal on sys test board is 32MHz actually)\n");
    AUDADCConfig.eClock             = AM_HAL_AUDADC_CLKSEL_XTHS_24MHz; // 32MHz crystal on sys test board
#else
    am_util_stdio_printf("AM_HAL_AUDADC_CLKSEL_HFRC2_48MHz\n");
...
audadc_dmacpl_test(void)
{
...
#if XTALHS_EN == 1
    am_util_stdio_printf("Enabling the 32MHz Crystal\n");
    //
    // Enable the 32 MHz XTALHS.
    //
    // am_hal_clkgen_control(AM_HAL_CLKGEN_CONTROL_XTAL24M_ENABLE, 0);
    ...
    //
    // Wait for 1 second for the 32 MHz XTALHS to startup and stabilize.
    //
    am_util_delay_ms(1000);
    ctrlArgs.b_arg_force_update = true ;  // second time through force it
