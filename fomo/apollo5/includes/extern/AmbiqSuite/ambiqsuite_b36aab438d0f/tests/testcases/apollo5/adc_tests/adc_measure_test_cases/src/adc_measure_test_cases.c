...
void globalSetUp(void)
{
    ...
    am_util_stdio_printf("Apollo5 ADC Measurement Test Cases with DAC MCP4725\n\n");
...
void am_adc_isr(void)
{
    ...
    if ((ui32IntStatus & AM_HAL_ADC_INT_FIFOOVR2) || (ui32IntStatus & AM_HAL_ADC_INT_DCMP))
    {
        ...
        //
        am_hal_cachectrl_dcache_invalidate(&(am_hal_cachectrl_range_t){(uint32_t)g_ui32ADCSampleBuffer, sizeof(uint32_t) * ADC_TOTAL_SAMPLE}, false);
...
void adc_init(void)
{
    ...
    {
        am_util_stdio_printf("Error - triggering ADC failed.\n");
    }
