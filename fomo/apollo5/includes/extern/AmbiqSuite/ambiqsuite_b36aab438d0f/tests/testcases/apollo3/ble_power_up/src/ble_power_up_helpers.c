...
//
//*****************************************************************************
static uint32_t ble_power_up(void);
...
ble_test_run(void)
{
    ...
    for (ui32Trial = 0; ui32Trial < NUM_TRIALS; ui32Trial++)
    {
        ...
        do
        {
            ...
            am_hal_ble_default_copy_patch_apply(BLE);
#endif
            am_hal_ble_default_trim_set_ramcode(BLE);
            am_hal_ble_default_patch_apply(BLE);
            if (ui32Status != AM_HAL_STATUS_SUCCESS)
            {
                am_util_stdio_printf("Patching Error %d.", ui32Status);
                ui32Status = BLE_PATCH_ERROR;
                break;
            }
            ...
            // Send patch complete.
            //
            ui32Status = am_hal_ble_patch_complete(BLE);
            if (ui32Status != AM_HAL_STATUS_SUCCESS)
            {
                am_util_stdio_printf("Hal Error %d.", ui32Status);
                ui32Status = BLE_HAL_ERROR;
                break;
            }
...
// Power up the BLE
//
// This function bypasses the normal HAL routine, because we want to be
// absolutely sure we catch failures gracefully.
//
//*****************************************************************************
static uint32_t
ble_power_up(void)
{
    //
    // Make sure the BLE isn't already enabled.
    //
    if ( PWRCTRL->DEVPWRSTATUS_b.BLEL != 0)
    {
        return BLE_POWER_ERROR_MODULE_NOT_DISABLED;
    }

    //
    // Enable the feature bit.
    //
    MCUCTRL->FEATUREENABLE = 1;

    am_util_delay_ms(1);

    WHILE_TIMEOUT(((MCUCTRL->FEATUREENABLE & 0x7) != 0x7),
                    GENERIC_TIMEOUT,
                    BLE_POWER_ERROR_FEATURE_NOT_ENABLED);
    ...
    WHILE_TIMEOUT(PWRCTRL->DEVPWRSTATUS_b.BLEL == 0,
                  GENERIC_TIMEOUT,
                  BLE_POWER_ERROR_POWER_STATUS);
    ...
    // #### INTERNAL END ####
    //
    BLEIF->CLKCFG = _VAL2FLD(BLEIF_CLKCFG_CLK32KEN, 1);
    BLEIF->BLEDBG_b.DBGDATA = 1 << 14;
    BLEIF->BLECFG_b.PWRSMEN = 1;

    if ((MCUCTRL->CHIPREV_b.REVMAJ == MCUCTRL_CHIPREV_REVMAJ_A) &&
    ...
    //
    WHILE_TIMEOUT(BLEIF->BSTATUS_b.PWRST != 3,
                  POWER_UP_TIMEOUT_MS,
                  BLE_POWER_ERROR_POWER_STATE);
