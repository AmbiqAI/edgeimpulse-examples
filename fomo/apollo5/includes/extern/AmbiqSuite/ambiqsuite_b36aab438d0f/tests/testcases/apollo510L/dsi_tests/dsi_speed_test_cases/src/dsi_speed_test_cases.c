...
//! Need to connect RM69330 to DSI interface, you will see a golden image on
...
// #### INTERNAL BEGIN ####
    ...
    //
    if (dphy_init_with_para(DPHY_X4) != 0) 
    {
        bPass = false;
        return bPass;
    }
...
// #### INTERNAL END ####
    ...
    nema_memcpy((char*)layer0.baseaddr_virt, golden_340_rgba565,framebuffersize);
...
#ifdef APOLLO5_FPGA
    //
    // measured transfer time is meaningless,because the hardware(Arason TC board) determined transfer opportunity and time on FPGA testing. 
    //
    bPass = true;
#else
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#ifdef APOLLO5_FPGA
    am_util_stdio_printf("Please measure transfer time on the Arason TC board and check the panel.\n");
#else
// #### INTERNAL END ####
