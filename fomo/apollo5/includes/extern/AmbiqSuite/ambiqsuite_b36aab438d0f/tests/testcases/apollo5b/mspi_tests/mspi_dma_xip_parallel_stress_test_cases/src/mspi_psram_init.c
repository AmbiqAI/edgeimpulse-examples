...
AM_SHARED_RW uint32_t ui32DMATCBBuffer2[IOM_MODULE_TOTAL][1024];
void            *g_pPsramHandle;
...
am_devices_mspi_psram_config_t g_sMspiPsramConfig =
{
#if defined(APOLLO5_FPGA) || defined(AM_BSP_MSPI_FLASH_DEVICE_IS25WX064)
...
uint32_t mspi_psram_init(void)
{
    ...
    ui32Status = mspi_psram_device_func.mspi_scrambling_enable(g_pPsramHandle);
    if (AM_DEVICES_MSPI_PSRAM_STATUS_SUCCESS != ui32Status)
    {
