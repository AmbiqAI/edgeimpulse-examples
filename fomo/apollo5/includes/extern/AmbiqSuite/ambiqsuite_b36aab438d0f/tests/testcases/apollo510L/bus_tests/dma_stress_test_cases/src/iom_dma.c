...
volatile bool g_bIomDmaFinish[IOM_MODULE_TOTAL] = {true, true, true, true, true, true};
...
iom_init(void)
{
    ...
    {
        //only IOM1 can be used independently on apollo510L_fpga_turbo
        if ( i != 1 )
        {
            continue;
        }
...
iom_wait(void)
{
    ...
    while((!g_bIomDmaFinish[1]) && (count < IOM_WAIT_TIMEOUT_MS))
    {
        am_util_delay_ms(1);
        count ++;
    }
