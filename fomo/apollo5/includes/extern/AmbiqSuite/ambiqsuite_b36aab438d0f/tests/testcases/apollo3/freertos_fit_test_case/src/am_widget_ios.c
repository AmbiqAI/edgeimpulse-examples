...
#define HANDSHAKE_IOM_PIN        38 // GPIO used to connect the IOSINT to host side
...
am_gpio_isr(void)
{
    ...
    ui32Status = am_hal_gpio_interrupt_status_get(false, pGpioIntStatusMask);
    ...
    ui32Status = am_hal_gpio_interrupt_status_get(false, &ui64IntStatus);
    if ( ui32Status )   // Avoid compiler warning
    {
    }
    am_hal_gpio_interrupt_clear(ui64IntStatus);
    am_hal_gpio_interrupt_service(ui64IntStatus);
#else
