//*****************************************************************************
//
//! @file daxi_coremark_pro_test_cases.c
//!
...
//!
//! We can also run this test with the following memory allocation by using the
//! attached IAR linker scripts in
//! tests\testcases\apollo4p\daxi_test_cases\daxi_coremark_pro_test_cases
//! and enabling macro "REALLOC_RTOS_STACK" and
//! "ONLY_RUN_JPEG_ONE_ITERATION" in config-template.ini.
//!
//! block/section          memory region
//! .text                  TCM
//! .rodata                SSRAM
...
daxi_coremark_pro_test(void)
{
    ...
    //
#ifdef WINBOND_PSRAM
    ui32Status = am_devices_mspi_psram_w958d6nw_ddr_init(MSPI_PSRAM_MODULE, &MSPI_PSRAM_CE1MSPIConfig, &g_pPSRAMHandle, &g_pMSPIHandle);
#else
