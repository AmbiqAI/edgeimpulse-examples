...
volatile bool g_bGuiTaskComplete = false;
...
//*****************************************************************************
//
// Free buffer in SSRAM 
//
//*****************************************************************************
void
SSRAM_buffer_free(img_obj_t* img)
{
    // Free frame buffer
    nema_buffer_destroy(&img->bo);
...
GuiTask(void *pvParameters)
{
    ...
    g_bGuiTaskComplete = true;	
		
    am_util_stdio_printf("\n GUI task, Display Task and Render Task end!\n");	
		
    display_panel_disable();
		
    // Display panel de-initialization.
    am_devices_display_generic_deinit();
		
    am_hal_pwrctrl_periph_disable(AM_HAL_PWRCTRL_PERIPH_GFX);
	
    // Free frame buffer
#ifndef USE_EXTENDED_RAM
    SSRAM_buffer_free(&g_sClockDialBuffer[0]);		
    SSRAM_buffer_free(&g_sClockDialBuffer[1]);
    SSRAM_buffer_free(&g_sSportsDialBuffer[0]);
    SSRAM_buffer_free(&g_sSportsDialBuffer[1]);
    SSRAM_buffer_free(&g_sFrameBuffer[1]);
#endif 
