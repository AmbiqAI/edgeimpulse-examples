...
void UnityPrintFloat(const UNITY_DOUBLE input_number)
{
    ...
    else if (number <= (UNITY_DOUBLE)0.0000005 && number > 0) UnityPrint("0.000000..."); /* Small number */
    else if (number < (UNITY_DOUBLE)4294967295.9999995) /* Rounded result fits in 32 bits, "%.6f" format */
    {
...
void UnityDefaultTestRun(UnityTestFunction Func, const char* FuncName, const int FuncLineNum)
{
    ...
    Unity.NumberOfTests++;
    AM_UPDATE_TESTNUM((UNITY_INT)Unity.NumberOfTests);
    UNITY_CLR_DETAILS();
    if (TEST_PROTECT())
    {
        setUp();
...
int UnityEnd(void)
{
    ...
    UNITY_PRINT_EOL();
    AM_UPDATE_TEST_RESULT((UNITY_INT)(Unity.NumberOfTests), (UNITY_INT)(Unity.TestFailures), (UNITY_INT)(Unity.TestIgnores));
