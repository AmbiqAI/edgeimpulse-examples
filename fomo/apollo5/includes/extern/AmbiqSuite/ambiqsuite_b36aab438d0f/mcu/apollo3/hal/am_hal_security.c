...
//*****************************************************************************
//
// CRC external addressing test simulation.
//
//*****************************************************************************
#if ENABLE_EXTMEM_CRC
//
// Set CRC_TEST_HOOK to 1 only when running the test, security_test_cases.
//  The test hook provides for simulation and testing of the external address
//  data path when only SRAM is available.
// In order to complete the simulation, two additional functions must be called
//  by the test program.
// am_hal_crc32_testhook_enable() - The test should call this function in
//  order to simulate use of an SRAM address as an external address. This
//  simulation allows the external addressing data path to be tested.
// am_hal_crc32_testhook_extaddrpath_get() - Returns the results of the
//  simulated external address testing.
//
#define CRC_TEST_HOOK   1

#if CRC_TEST_HOOK
bool g_bTesthookForceExtAddrPath = false;
bool g_bTesthookUseExtAddrPath = false;

//*****************************************************************************
//! @brief
//!
//! @details a test function
//!
//! @param bEnable
//! @return true
//! @return false
//*****************************************************************************
bool am_hal_crc32_testhook_enable(bool bEnable)
{
    bool bTmp = g_bTesthookForceExtAddrPath;    // Get current setting
    g_bTesthookForceExtAddrPath = bEnable;
    return bTmp;                                // Return previous value
}
...
//! @return false
//*****************************************************************************
bool am_hal_crc32_testhook_extaddrpath_get(void)
{
    return g_bTesthookUseExtAddrPath;
}
#endif // CRC_TEST_HOOK
#endif // ENABLE_EXTMEM_CRC
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#if CRC_TEST_HOOK
    //
    // Simulate that an external address was provided.
    //
    if ( g_bTesthookForceExtAddrPath )
    {
        g_bTesthookUseExtAddrPath = true;
        if ( bInternal )
        {
            bInternal = false;
        }
    ...
    {
        g_bTesthookUseExtAddrPath = false;
    }
#endif // CRC_TEST_HOOK
// #### INTERNAL END ####
    if ( bInternal )
