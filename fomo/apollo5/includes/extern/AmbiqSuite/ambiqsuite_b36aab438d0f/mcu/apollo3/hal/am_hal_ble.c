...
// #### INTERNAL BEGIN ####
// CAJ:
// Rev b0 parts have falling-edge interrupts for the flow control signals that
// they can use to skip some blocking portions of the HCI-over-SPI
// transactions. For first-silicon, we can disable this behavior for
// simplicity.
// #### INTERNAL END ####
...
am_hal_ble_power_control(void *pHandle, uint32_t ui32PowerState)
{
    ...
    if (ui32PowerState == AM_HAL_BLE_POWER_ACTIVE)
    {
        ...
        if ( PWRCTRL->DEVPWRSTATUS_b.BLEL == 0)
        {
            ...
            //
            // #### INTERNAL BEGIN ####
            // Should be able to use the power control HAL API here.
            // #### INTERNAL END ####
            ...
            // enable the clocks.
            // #### INTERNAL BEGIN ####
            // CAJ: Added a 5ms delay at Shan's request. Not sure if this
            // actually increases power-up stability.
            // #### INTERNAL END ####
            //
...
// #### INTERNAL BEGIN ####
#if 0 // static function not currently used.
//*****************************************************************************
//
...
am_hal_ble_blocking_transfer(void *pHandle, am_hal_ble_transfer_t *psTransfer)
{
    ...
    //
    // #### INTERNAL BEGIN ####
    // CAJ: This means that NOTHING will be able to happen during a blocking
    // SPI transfer. I'm not sure we actually want that, but it should help us
...
// #### INTERNAL END ####
    ...
    {
        // #### INTERNAL BEGIN ####
        // TODO: Consider checking command complete as a failsafe.
        // #### INTERNAL END ####
        while (pBle->ui32TransferIndex < pBle->sCurrentTransfer.ui16Length)
        {
    ...
    //
// #### INTERNAL BEGIN ####
   //fixme moved further down to cover am_hal_ble_bus_release();
// #### INTERNAL END ####
    AM_CRITICAL_END;
    ...
    //
    // #### INTERNAL BEGIN ####
    // TODO: Consider switching memset away from C libraries.
    // #### INTERNAL END ####
    memset(&pBle->sCurrentTransfer, 0, sizeof(am_hal_ble_transfer_t));
    pBle->ui32TransferIndex = 0;
...
// #### INTERNAL BEGIN ####
// TODO: Read the bug on CMDCMP vs DCMP
// #### INTERNAL END ####
//*****************************************************************************
//
...
} // am_hal_ble_lock_and_check_status()
#endif
// #### INTERNAL END ####
//*****************************************************************************
//
...
// #### INTERNAL BEGIN ####
#if 1
#warning The apollo3p version contains the following els e which returns AM_HAL_STATUS_FAIL. Does apollo3 also need this?
#else
    else
    {
      return AM_HAL_STATUS_FAIL;
...
// #### INTERNAL BEGIN ####
#if 1
#warning am_hal_ble_plf_reg_read() A2SD-1392 Need to disable IOCLKON.
#else
// A2SD-1392 1/20/2020
// Leaving IOCLKON enabled increases BLE power usage.  However this was found
// too late to be safely included in R2.4.0 and will be implemented in a
// subsequent release.
    //
    // Disable IOCLK
    //
    BLEIFn(ui32Module)->BLEDBG_b.IOCLKON = 0;
#endif
// #### INTERNAL END ####
    //
    // Re-enable BLE interrupts.
...
// #### INTERNAL BEGIN ####
#if 1
#warning am_hal_ble_plf_reg_write() A2SD-1392 Need to disable IOCLKON.
#else
// A2SD-1392 1/20/2020
// Leaving IOCLKON enabled increases BLE power usage.  However this was found
// too late to be safely included in R2.4.0 and will be implemented in a
