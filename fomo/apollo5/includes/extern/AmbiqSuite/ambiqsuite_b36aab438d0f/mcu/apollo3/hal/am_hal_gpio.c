...
// #### INTERNAL BEGIN ####
// By default no validation is done to determine whether the pin is actually
// configured as GPIO as it is very time consuming. However if desired for
// debugging purposes, that validation can be enabled below.
// #### INTERNAL END ####
//
//*****************************************************************************
uint32_t
...
// #### INTERNAL BEGIN ####
// This additional validation code must default to disabled, the customer would need to activate it if desired!
// AS-658 and A2SD-1848 This additional validation code was originally provided
//        to Nike on 06/04/20 as a one-off.
// A2SD-1848 Provide this as part of the SDK.
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
// AS-658 and A2SD-1848 REH 06/29/20
// These are a couple of extra functions that were provided on 6/29/20 as a
// one-off to Nike via Gary Allen so that they could determine whether a
// particular pin was configured as a GPIO and/or as an input.
// They seemed like handy functions that could prove useful, so they are kept
// here in case we'd like to formalize them.
// In fact, Nike mentioned that they would like to see these shipped in the SDK.
//
// Via A2SD-1848, these new functions are now part of Apollo3 and Apollo3p SDK.
// #### INTERNAL END ####
//*****************************************************************************
//
//  am_hal_gpio_isinput()
...
//*****************************************************************************
//
// This structure is used for pin configuration, encapsulating all 3 necessary
// registers.
//
//*****************************************************************************
typedef struct
{
    uint32_t GPCfgAddr;             // O: Addr of the GPIOCFG register for the pin
    uint32_t PadregAddr;            // O: Addr of the PADREG register for the pin
    uint32_t AltpadAddr;            // O: Addr of the ALTPAD  register for the pin
    uint32_t CfgClrMask;            // O: Clear mask for GPIOCFG for the pin
    uint32_t PadClrMask;            // O: Clear mask PADREG and ALTPAD for the pin
    uint32_t CfgShift;              // O: GPIOCFG shift
    uint32_t PadShift;              // O: PADREG and ALTPAD shift
    //
    uint32_t ui32Modify;            // I: b0=mod CFG, b1=mod PAD, b2=mod ALT
                                    //    0=return above values only
    uint32_t ui32CfgMsk;            // I: Mask of the bits to be modified in CFG
    uint32_t ui32CfgVal;            // I: Value to be shifted and set in CFG
    uint32_t ui32PadMsk;            // I: Mask of the bits to be modified in PADREG
    uint32_t ui32PadVal;            // I: Value to be shifted and set in PADREG
    uint32_t ui32AltMsk;            // I: Mask of the bits to be modified in ALTPAD
    uint32_t ui32AltVal;            // I: Value to be shifted and set in ALTPAD
} am_hal_gpio_cfgregs_t;

//*****************************************************************************
//
// This function uses the am_hal_gpio_cfgregs_t structure to compute all 3
// register locations, shifts, and masks for a given pins number.
// The function always returns the upper elements of am_hal_gpio_cfgregs_t.
// And if specified, will use the lower elements to actually set new values
// in the registers, thus implementing the configuration.
//
//*****************************************************************************
static void
gpio_pincfg_get_and_mod(uint32_t ui32Pin, am_hal_gpio_cfgregs_t *pCfgRegs)
{
    uint32_t ui32Reg, ui32Clr;

    pCfgRegs->GPCfgAddr  = AM_REGADDR(GPIO, CFGA)       + ((ui32Pin >> 1) & ~0x3);
    pCfgRegs->PadregAddr = AM_REGADDR(GPIO, PADREGA)    + (ui32Pin & ~0x3);
    pCfgRegs->AltpadAddr = AM_REGADDR(GPIO, ALTPADCFGA) + (ui32Pin & ~0x3);

    pCfgRegs->CfgShift   = ((ui32Pin & 0x7) << 2);
    pCfgRegs->PadShift   = ((ui32Pin & 0x3) << 3);
    pCfgRegs->CfgClrMask = ~((uint32_t)0xF  << pCfgRegs->CfgShift);
    pCfgRegs->PadClrMask = ~((uint32_t)0xFF << pCfgRegs->PadShift);

    if ( pCfgRegs->ui32Modify & 0x7 )
    {
        AM_CRITICAL_BEGIN
        GPIO->PADKEY = GPIO_PADKEY_PADKEY_Key;

        if ( pCfgRegs->ui32Modify & 0x1 )
        {
            //
            // Modify the CFG register
            //
            ui32Reg = (pCfgRegs->ui32CfgVal & pCfgRegs->ui32CfgMsk) << pCfgRegs->CfgShift;
            ui32Clr = ~(pCfgRegs->ui32CfgMsk << pCfgRegs->CfgShift);
            AM_REGVAL(pCfgRegs->GPCfgAddr) = (AM_REGVAL(pCfgRegs->GPCfgAddr) & ui32Clr) | ui32Reg;
        }

        if ( pCfgRegs->ui32Modify & 0x2 )
        {
            //
            // Modify the PADREG register
            //
            ui32Reg = (pCfgRegs->ui32PadVal & pCfgRegs->ui32PadMsk) << pCfgRegs->PadShift;
            ui32Clr = ~(pCfgRegs->ui32PadMsk << pCfgRegs->PadShift);
            AM_REGVAL(pCfgRegs->PadregAddr) = (AM_REGVAL(pCfgRegs->PadregAddr) & ui32Clr) | ui32Reg;
        }

        if ( pCfgRegs->ui32Modify & 0x4 )
        {
            //
            // Modify the ALTPADCFG register
            //
            ui32Reg = (pCfgRegs->ui32AltVal & pCfgRegs->ui32AltMsk) << pCfgRegs->PadShift;
            ui32Clr = ~(pCfgRegs->ui32AltMsk << pCfgRegs->ui32AltMsk);
            AM_REGVAL(pCfgRegs->AltpadAddr) = (AM_REGVAL(pCfgRegs->AltpadAddr) & ui32Clr) | ui32Reg;
        }

        GPIO->PADKEY = 0;
        AM_CRITICAL_END
    }
} // gpio_pincfg_get_and_mod()

//*****************************************************************************
//
//! @brief Reconfigure a parameter for a given pin.
//!
//! @param ui32Pin        - pin number to be reconfigured.
//! @param eModifyCfgType - The parameter to reconfigure.  Currently supports:
//!                         AM_HAL_GPIO_CFGMODIFY_DRVSTRGTH
//! @param ui32ParamVal   - The parameter value to be reconfigured to.
//!
//! This function modifies an existing pin configuration according to the
//! parameter type and value specified.
//! The new value is validated only for appropriateness for the given
//! configuration field.  It is not validated in relation to other fields.
//! Therefore it is possible to set an invalid pin configuration using this
//! function and is the caller's responsibility to make sure parameters are
//! appropriate for the given pin.
//!
//! @return Status.
//
//*****************************************************************************
uint32_t
am_hal_gpio_pinconfig_modify(uint32_t ui32Pin,
                             am_hal_gpio_cfgmod_type_e eModifyCfgType,
                             uint32_t ui32Param)
{
    am_hal_gpio_drivestrength_e eDrvStr;
    uint32_t ui32GPcfg, ui32Padreg, ui32AltPadCfg;
    am_hal_gpio_cfgregs_t sCfgRegs;

#ifndef AM_HAL_DISABLE_API_VALIDATION
    if ( (ui32Pin        >= AM_HAL_GPIO_MAX_PADS)       ||
         (eModifyCfgType >  AM_HAL_GPIO_CFGMODIFY_LAST) )
    {
        return AM_HAL_STATUS_INVALID_ARG;
    }
#endif // AM_HAL_DISABLE_API_VALIDATION
    ...
    ui32GPcfg = ui32Padreg = ui32AltPadCfg = 0;
    sCfgRegs.ui32Modify = 0;    // Init to no fields to be updated

    switch ( eModifyCfgType )
    {
        case AM_HAL_GPIO_CFGMODIFY_DRVSTRGTH:
            eDrvStr = (am_hal_gpio_drivestrength_e)ui32Param;
            if ( eDrvStr > AM_HAL_GPIO_PIN_DRIVESTRENGTH_12MA )
            {
                return AM_HAL_STATUS_INVALID_ARG;
            }

            //
            // ui32Modify: bit0=CFG, bit1=PADREG, bit2=ALTPAD
            //
            sCfgRegs.ui32Modify = (0 << 0) | (1 << 1) | (1 << 2);  // Modify PADREG and ALTPAD
            switch ( eDrvStr )
            {
                // DRIVESTRENGTH is a 2-bit field.
                //  bit0 maps to bit2 of a PADREG field.
            ...
            sCfgRegs.ui32CfgMsk = 0;                            // Nothing to update
            sCfgRegs.ui32CfgVal = ui32GPcfg;
            sCfgRegs.ui32PadMsk = (1 << PADREG_FLD_DRVSTR_S);   // Update a single bit in the pinfield
            sCfgRegs.ui32PadVal = ui32Padreg;
            sCfgRegs.ui32AltMsk = (1 << 0);                     // Update a single bit in the pinfield
            sCfgRegs.ui32AltVal = ui32AltPadCfg;

            //
            // Modify the specified fields in each register
            //
            gpio_pincfg_get_and_mod(ui32Pin, &sCfgRegs);
            break;
