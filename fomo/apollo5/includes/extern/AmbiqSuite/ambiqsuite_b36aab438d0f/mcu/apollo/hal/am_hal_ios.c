...
am_hal_ios_access_int_status_get(bool bEnabledOnly)
{
    if (bEnabledOnly)
    {
        uint32_t ui32RetVal = AM_REG(IOSLAVE, REGACCINTSTAT);
        return ui32RetVal & AM_REG(IOSLAVE, REGACCINTEN);
...
am_hal_ios_int_status_get(bool bEnabledOnly)
{
    if (bEnabledOnly)
    {
        uint32_t ui32RetVal = AM_REG(IOSLAVE, INTSTAT);
        return ui32RetVal & AM_REG(IOSLAVE, INTEN);
...
//##### INTERNAL BEGIN #####
// Details of issue being addressed here - SHELBY-1654
//##### INTERNAL END #####
