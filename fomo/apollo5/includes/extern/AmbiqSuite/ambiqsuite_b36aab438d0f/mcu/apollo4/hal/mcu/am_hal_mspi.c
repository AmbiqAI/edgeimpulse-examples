            ...
            case AM_HAL_MSPI_FLASH_OCTAL_DDR_CE0:
                switch ( eDeviceNum )
                {
                    case AM_HAL_MSPI_DEVICE0:
                        ...
                        MSPIn(ui32Module)->DEV0CFG_b.SEPIO0       = 0;
                        MSPIn(ui32Module)->DEV0XIP_b.XIPMIXED0    = MSPI0_CTRL_PIOMIXED_NORMAL;
                        MSPIn(ui32Module)->PADOVER_b.OVERRIDE     &= ~(1 << 9); //FIXME: This is the workaround to keep DM pin always low during write
...
// #### INTERNAL BEGIN ####
    //
    // SW Workaround for FALC-8 Issue.  CLKDIV for DEV0 and DEV1 must be the same.
    //
#warning "SW Workaround for FALC-8 Issue.  Needs to be an Errata before release!"
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#else
#warning "Check for SW Workaround for FALC-8 Issue is not being made."
#warning "MSPI Device 0 and 1 must have the same CLKDIV values."
// #### INTERNAL END ####
#endif // AM_HAL_DISABLE_API_VALIDATION
    ...
    //
    // FIXME in RevB
    if ((( pConfig->eClockFreq == AM_HAL_MSPI_CLK_96MHZ ) || ( pConfig->eClockFreq == AM_HAL_MSPI_CLK_48MHZ )) && pConfig->bEmulateDDR == 0)
    {
    ...
    if (pMSPIState->pTCB)
    {
        ...
        // FIXME in RevB
        if ( pConfig->eClockFreq == AM_HAL_MSPI_CLK_48MHZ && MSPIn(ui32Module)->DEV0DDR_b.EMULATEDDR0 == 0)
        {
                             ...
                             const void *pConfig)
{
    ...
    switch ( eRequest )
    {
        ...
        case AM_HAL_MSPI_REQ_DDR_DIS:
            ...
            // Disable DDR.
            //
            MSPIn(ui32Module)->DEV0DDR_b.EMULATEDDR0 = 0; //FIXME - Add Device1 support later
            break;
        ...
        case AM_HAL_MSPI_REQ_DDR_EN:
            ...
            // Enable DDR.
            //
            MSPIn(ui32Module)->DEV0DDR_b.EMULATEDDR0 = 1; //FIXME - Add Device1 support later
            break;
        //FIXME - Add Device1 support later
        ...
        case AM_HAL_MSPI_REQ_CLOCK_CONFIG:
            {
                ...
                //
                //FIXME in RevB
                if ( eClockFreq == AM_HAL_MSPI_CLK_48MHZ && MSPIn(ui32Module)->DEV0DDR_b.EMULATEDDR0 == 0)
                {
