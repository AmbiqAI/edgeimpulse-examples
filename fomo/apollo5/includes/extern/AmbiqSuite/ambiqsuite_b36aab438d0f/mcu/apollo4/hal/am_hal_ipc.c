...
//! @file
//!
//! @brief Functions for Hardware Mutex
//!
//! @addtogroup mutex4 Mutex
//! @ingroup apollo4hal
//! @{
...
// Queue full/empty/used-word status
...
//*****************************************************************************
//
//! @brief get the information of the peer queue from the shared memory
//!
//! The function copies the shared memory data for the queues into the internal
//! descriptor. There are two queues: Rx and Tx. The Tx is mostly held inside the
//! peer so the Tx descriptor gets updated by the most recent info. The Rx queue
//! is managed by the current core and only the read index should be updated inside
//! the other core.
//! Also, the eventMask is updated from the shared memory.
...
//*****************************************************************************
//
//! @brief write the information of the queue to the shared memory
//!
//! The function copies the descriptor queue address and event mask bits to the
//! shared memory.
//!
//! parameters.
//!
...
//! @brief write the information of the queue to the shared memory
//!
//! This clears the local copy of the Rx and Tx queue. The shared memory and the
//! peer copy of the data are not changed by this function. To update the shared memory,
//! the am_hal_ipc_update_shared_mem_with_queue_data should be called. To update the
//! peer queue, a management event should be sent to the other queue, so other queue
...
//*****************************************************************************
//
//! @brief called when a queue is destroyed by the peer
//!
//! This is an internal function and is not intended to be used by any other function
//! outside the IPC hal.
//! After peer destroys the Rx/Tx queues, sends a  management event to other side.
//! When the remote side gets the event, if the shared memory addresses for the queues
//! are invalid, then the management event is consider as a termination event and this
//! function(am_hal_ipc_shortmsg_termination_event) is called. The termination process
//! clears the shared memory and then clears the Rx/Tx queue descriptor.
//!
//! parameters.
//!
//! @return None.
...
//*****************************************************************************
//
//! @brief the management init event service
//!
//!
//!
//! parameters.
...
//*****************************************************************************
//
//! @brief Register a function for an event and optionally send a management event to other cores
//!
//! This is an internal function and is not intended to be used by any other function
//! outside the IPC hal.
//! The function registers a handler for an incoming event. Also update the maskEvent
//! in local and shared memory. And then can send a management event to the other cores.
//!
//! parameters.
//!
//! @return None.
...
//! @brief the event handler for any incoming queue message
//!
//! The function only get the data from the message buffer without incrementing the read pointer.
//! This function is useful when a host send a message and wait for the remote side to process the message.
//! The host may assume, when the message is read, the message processing is done, so there is no need to
//! keep the resource associated with that message. To make sure host can safely rely on the Fifo-empty
//! message to release the resource,  am_hal_ipc_shortmsg_get_only_msg only reads the message without
//! incrementing the index pointer. After reading the message with this function, always am_hal_ipc_shortmsg_get
//! should be executed to actually receive the message and inform the Tx side(host) that the message is successfully
//! received.
//!
//! parameters.
//!
...
//! @brief the event handler for any incoming queue message
//!
//! The  am_hal_ipc_shortmsg_get check the queue empty status. If the Rx queue is not empty, the function reads the
//! queue contents and then increment rx-queue read pointer. After message completely removed from the rx queue, the
//! tx side(host) informed by incrementing the read pointer inside host by one.
//!
//! parameters.
//!
//! @return None.
