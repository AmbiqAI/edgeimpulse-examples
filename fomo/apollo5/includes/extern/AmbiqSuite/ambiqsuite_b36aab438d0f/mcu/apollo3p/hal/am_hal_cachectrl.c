...
// #### INTERNAL BEGIN ####
#if 0
    .bEnableCacheMonitoring     = false,
    .bEnableCacheClockGating    = true,
    .bEnableLightSleep          = false,
    .bEnableDataClockGating     = true,
#endif
// #### INTERNAL END ####
};
// #### INTERNAL BEGIN ####
#if CACHECTRL_INTERNAL_TEST
const am_hal_cachectrl_config_t am_hal_cachectrl_coremark =
{
    .bLRU                       = 0,
    .eDescript                  = AM_HAL_CACHECTRL_DESCR_1WAY_128B_1024E,
...
const am_hal_cachectrl_flashcfg_t am_hal_cachectrl_flashcfg_coremark =
{
    .LPMMODE                    = AM_HAL_CACHECTRL_FLASHCFG_LPMMODE_ALWAYS,
    .LPM2ReadWaitstates         = 0x3,
    .SEDelay                    = 0x6,
    .ReadWaitstates             = 0x1,
};
...
#if 0
    //
    // Set default values for FLASHCFG.
    //
    CACHECTRL->FLASHCFG =
        _VAL2FLD(CACHECTRL_FLASHCFG_LPMMODE, 0x0)       |
        _VAL2FLD(CACHECTRL_FLASHCFG_LPM_RD_WAIT, 0x8)   |
        _VAL2FLD(CACHECTRL_FLASHCFG_SEDELAY, 0x7)       |
        _VAL2FLD(CACHECTRL_FLASHCFG_RD_WAIT, 0x3);
#endif
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
    //
    // Note: If silicon validation should reveal that the 4 instances require
    // slightly different timing, any required deltas between instances would
    // be handled here.
    //
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#if CACHECTRL_INTERNAL_TEST
        case AM_HAL_CACHECTRL_CONTROL_FLASHCFG_SET:
            ui32SetMask = 0;
            ...
            am_hal_cachectrl_flashcfg_t *pFlashCfg;
            pFlashCfg = (am_hal_cachectrl_flashcfg_t *)pArgs;

            CACHECTRL->FLASHCFG =
                _VAL2FLD(CACHECTRL_FLASHCFG_LPMMODE, pFlashCfg->LPMMODE)                |
                _VAL2FLD(CACHECTRL_FLASHCFG_LPM_RD_WAIT, pFlashCfg->LPM2ReadWaitstates) |
                _VAL2FLD(CACHECTRL_FLASHCFG_SEDELAY, pFlashCfg->SEDelay)                |
                _VAL2FLD(CACHECTRL_FLASHCFG_RD_WAIT, pFlashCfg->ReadWaitstates);
            return AM_HAL_STATUS_SUCCESS;
#endif // CACHECTRL_INTERNAL_TEST
// #### INTERNAL END ####
