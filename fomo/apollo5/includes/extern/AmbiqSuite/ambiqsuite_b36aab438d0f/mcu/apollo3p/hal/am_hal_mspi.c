...
// #### INTERNAL BEGIN ####
#if 0
    uint32_t                    ui32DummyAddr3;
    uint32_t                    ui32DummyVal3;
    uint32_t                    ui32DummyAddr4;
    uint32_t                    ui32DummyVal4;
    uint32_t                    ui32DummyAddr5;
    uint32_t                    ui32DummyVal5;
#endif
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
    ...
    //
    // Check whether Continuation transfer is allowed
    //
    if (pTransaction->bContinue)
    {
        if (pMSPIState->eClockFreq != AM_HAL_MSPI_CLK_24MHZ)  // Is MSPI Clock frequency != 24MHz?
        {
            return AM_HAL_STATUS_INVALID_ARG;  // Apollo3 Blue MCU ERR023.
        }
    }
#endif // A3DS-25
// #### INTERNAL END ####
...
uint32_t am_hal_mspi_interrupt_service(void *pHandle, uint32_t ui32IntStatus)
{
    ...
    if (pMSPIState->bHP)
    {
        ...
        if (pMSPIState->ui32TxnInt & (AM_HAL_MSPI_INT_DMACMP | AM_HAL_MSPI_INT_ERR))
        {
            ...
            //
            // #### INTERNAL BEGIN ####
            // TODO - We are waiting for DMATIP indefinitely in the ISR
