...
{
    movs r0, #0xC3      // 20C3
    movw r1, #0x4014    // F2440114
    movt r1, #0x4001    // F2C40101
    str r0, [r1]        // 6008
    movw r0, #0x4034    // F2440034
    movt r0, #0x4000    // F2C40000
    ldr r0, [r0]        // 6800
    mov r2, #160        // F04F02A0
    tst r0, #0x00000004 // F0100F04
    beq NormalMode      // D000
    lsls r2, r2, #1     // 0052
NormalMode
    mov r0, r2          // 4610
    movw r1, #0x009D    // F240019D
    movt r1, #0x0800    // F6C00100
    bx r1               // 4708
    nop                 // BF00
}
#endif
...
{
// #### INTERNAL BEGIN ####
// fixme check for possible boot loader and use page erase to avoid it.
// #### INTERNAL END ####
...
{
// #### INTERNAL BEGIN ####
// fixme check for possible boot loader and avoid it.
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#if 0 // CORESEL of 1 is not supported for Corvette. Assume it's always 0.
    //
    // Since Apollo3 supports a CORESEL of 0, it must also be considered.
    //
    if ( CLKGEN->CCTRL )
    {
        ui32Iterations >>= 1;
...
#if 0
//
// NOTE: The Apollo3 BOOTROM flash_util_write_word() cannot be depended on for
//   safe writes since it could return before the write has actually completed
//   (due to the inherent latencies in Corvette bus accesses).
// We will therefore use an SRAM-based function similarly to what was originally
//   done for reads on Apollo.
//
// The Apollo3 BOOTROM flash_util_read_word() function should be okay as is.
//
//*****************************************************************************
//
...
//*****************************************************************************
void
am_hal_flash_store_ui32(uint32_t *pui32Address, uint32_t ui32Data)
{
    g_am_hal_flash.flash_util_write_word(pui32Address, ui32Data);
} // am_hal_flash_store_ui32()

//*****************************************************************************
//
// Read a value while executing in SRAM.
//
// Set up a very simple function that resides in SRAM which can be used to do
//  reads of a given memory location while executing outside of flash.
//
// Prototypes:
//  uint32_t void SRAM_read_ui32(ui32Addr);
//
...
// Returns the value that was read from the location.
//
//*****************************************************************************
uint32_t SRAM_read_ui32[8 / 4] =
{
...
am_hal_flash_load_ui32(uint32_t *pui32Address)
{
    ...
    //
    uint32_t SRAMCode = (uint32_t)SRAM_read_ui32 | 0x1;
    uint32_t (*pFunc)(uint32_t*) = (uint32_t (*)(uint32_t*))SRAMCode;
    return (*pFunc)(pui32Address);
...
// #### INTERNAL BEGIN ####
    //
    // A2SD-738:
    // This note to be REMOVED after testing this func on a later GCC version!!
    // Also remove this note in Apollo4 am_hal_flash_store_ui32().
    // REH 9/14/18
    //
    // The local version of SRAM_write_ui32[] does not work on gcc v5.3.1 (which
    // is the version that atollic was using) in high optimization -O3.  It does
    // work for --O0. Reportedly gcc v5.4 fixed some stack alignment issues that
    // were present in previous versions.
    // A global version of SRAM_write_ui32[] does work on v5.3.1 in any
    // optimization level.  However the local version is preferred so that SRAM
    // allocation is minimized.
    // Finally, there are no problems with the local version on IAR or Keil.
    //
// #### INTERNAL END ####
