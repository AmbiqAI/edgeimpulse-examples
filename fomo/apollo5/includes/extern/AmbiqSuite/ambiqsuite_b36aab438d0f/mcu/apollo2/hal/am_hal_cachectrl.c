...
// #### INTERNAL BEGIN ####
    ...
    //
    // Write a default configuration to the CACHEMODE register.
    //
    AM_REG(CACHECTRL, CACHEMODE) =                                  \
        AM_REG_CACHECTRL_CACHEMODE_THROTTLE6(0)                 |   \
        AM_REG_CACHECTRL_CACHEMODE_THROTTLE5(0)                 |   \
        AM_REG_CACHECTRL_CACHEMODE_THROTTLE4(0)                 |   \
        AM_REG_CACHECTRL_CACHEMODE_THROTTLE3(0)                 |   \
        AM_REG_CACHECTRL_CACHEMODE_THROTTLE2(0)                 |   \
        AM_REG_CACHECTRL_CACHEMODE_THROTTLE1(0);

    //
    // Write a default configuration to the CACHEDBG register.
    //
    AM_REG(CACHECTRL, CACHEDBG) =                                   \
        AM_REG_CACHECTRL_CACHEDBG_CDBG_ENABLE(0)                |   \
        AM_REG_CACHECTRL_CACHEDBG_CDBG_SEL(0)                   |   \
        AM_REG_CACHECTRL_CACHEDBG_CDBG_ADDR(0);

    //
    // Write a default configuration to the CACHEDBGDATA register.
    //
    AM_REG(CACHECTRL, CACHEDBGDATA) =                               \
        AM_REG_CACHECTRL_CACHEDBGDATA_CDBG_DATA(0);
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
    ...
    //
    // Write a default configuration to all of the DEBUGxxx registers.
    //
    AM_REG(CACHECTRL, DEBUGCTRL) =                                  \
        AM_REG_CACHECTRL_DEBUGCTRL_DBGSEL(                          \
            AM_REG_CACHECTRL_DEBUGCTRL_DBGSEL_DISABLED >>           \
            AM_REG_CACHECTRL_DEBUGCTRL_DBGSEL_S );

    AM_REG(CACHECTRL, DEBUGDBUS) =                                  \
        AM_REG_CACHECTRL_DEBUGDBUS_D_DBUS(0);

    AM_REG(CACHECTRL, DEBUGIBUS) =                                  \
        AM_REG_CACHECTRL_DEBUGIBUS_D_IBUS(0);

    AM_REG(CACHECTRL, DEBUGTAG) =                                   \
        AM_REG_CACHECTRL_DEBUGTAG_D_TAG(0);

    AM_REG(CACHECTRL, DEBUGBUFFER0) =                               \
        AM_REG_CACHECTRL_DEBUGBUFFER0_D_BUFFER0(0);

    AM_REG(CACHECTRL, DEBUGBUFFER1) =                               \
        AM_REG_CACHECTRL_DEBUGBUFFER1_D_BUFFER1(0);

    AM_REG(CACHECTRL, DEBUGBUFFER2) =                               \
        AM_REG_CACHECTRL_DEBUGBUFFER2_D_BUFFER2(0);

    AM_REG(CACHECTRL, DEBUGLLREAD) =                                \
        AM_REG_CACHECTRL_DEBUGLLREAD_D_LLREAD(0);

    AM_REG(CACHECTRL, DEBUGTIMING) =                                \
        AM_REG_CACHECTRL_DEBUGTIMING_D_TIMING(0);

    AM_REG(CACHECTRL, DEBUGPARAADDR) =                              \
        AM_REG_CACHECTRL_DEBUGPARAADDR_D_PARAADDR(0);

    AM_REG(CACHECTRL, DEBUGPARADATA) =                              \
        AM_REG_CACHECTRL_DEBUGPARADATA_D_PARADATA(0);

    AM_REG(CACHECTRL, DEBUGPARACTRL) =                              \
        AM_REG_CACHECTRL_DEBUGPARACTRL_D_PARACTRL(0);

    AM_REG(CACHECTRL, DEBUGPARACRC) =                               \
        AM_REG_CACHECTRL_DEBUGPARACRC_D_PARACRC(0);
// #### INTERNAL END ####
}
...
am_hal_cachectrl_config(am_hal_cachectrl_config_t *psConfig)
{
    ...
    //
    // Arrange all of the members of the data structure into a single ui32 that
...
//!
//! @param ui32EnableMask  - Mask of features to be enabled.
//! @param ui32DisableMask - Mask of features to be disabled.
//!
...
uint32_t
am_hal_cachectrl_cache_enables(uint32_t ui32EnableMask, uint32_t ui32DisableMask)
{
    ...
    //
    ui32EnableMask  &= CACHECTRL_VALID_ENABLES;
    ui32DisableMask &= CACHECTRL_VALID_ENABLES;

    //
    // First, do the disables.
    //
    AM_REG(CACHECTRL, CACHECFG) &= ~ui32DisableMask;

    //
    // Now set the enables.
    //
    AM_REG(CACHECTRL, CACHECFG) |= ui32EnableMask;
