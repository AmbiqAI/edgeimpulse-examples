...
//*****************************************************************************
// #### INTERNAL BEGIN ####
// Define a structure to map CE (from Steve Sheafor's excel spreadsheet for IOM4 only.
// #### INTERNAL END ####
//
                                 ...
                                 uint32_t ui32Options)
{
    ...
    // #### INTERNAL BEGIN ####
    // CAJ: Do we want to check for RAW writes vs. OFFSET writes like I do
    // below? I think each type of write will need a different workaround.
    // #### INTERNAL END ####
    //
...
typedef struct
{
    ...
    uint32_t ui32Write;
    uint32_t ui32Read;
    uint32_t ui32FifoLevel;
}
loop_record_t;

loop_record_t g_LoopRecord[4096];
...
{
// #### INTERNAL BEGIN ####
    //uint32_t ui32TransferSize, ui32SpaceInFifo
    //uint32_t bCmdCmp = false;
// #### INTERNAL END ####
...
#if 0
    ui32RecordIndex = 0;
    g_LoopRecord[ui32RecordIndex].ui32BytesWritten = ui32DataWritten;
    g_LoopRecord[ui32RecordIndex].ui32BytesRead = ui32DataRead;
    g_LoopRecord[ui32RecordIndex].ui32Write = ui32WriteSize;
    g_LoopRecord[ui32RecordIndex].ui32Read = ui32ReadSize;
    g_LoopRecord[ui32RecordIndex].ui32FifoLevel = am_hal_iom_fifo_full_slots(ui32Module);
    ui32RecordIndex++;
#endif
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
        ...
        if (ui32RecordIndex == 4096)
        {
            ui32RecordIndex = 1;
        }

        while (AM_BFMn(IOMSTR, ui32Module, INTSTAT, FOVFL));
        while (AM_BFMn(IOMSTR, ui32Module, INTSTAT, FUNDFL));
#endif
// #### INTERNAL END ####
    }
...
am_hal_iom_int_status_get(uint32_t ui32Module, bool bEnabledOnly)
{
    ...
    if ( bEnabledOnly )
    {
        uint32_t ui32RetVal = AM_REGn(IOMSTR, ui32Module, INTSTAT);
        return ui32RetVal & AM_REGn(IOMSTR, ui32Module, INTEN);
    }
    else
    {
...
//*****************************************************************************
//
//! @brief Transact on IOM
//!
//! @param ui32Module - Module number.
//! @param pOper - Details of operation desired.
//!
//! @return none
//
//*****************************************************************************
am_hal_iom_status_e
am_hal_iom_operate(uint32_t ui32Module, am_hal_iom_oper_t *pOper)
{
    am_hal_iom_status_e ui32Status = AM_HAL_IOM_SUCCESS;
    am_hal_debug_assert_msg(pOper->proto < AM_HAL_IOM_PROTO_MAX,
            "Invalid IOM Protocol");
    am_hal_debug_assert_msg(pOper->mode < AM_HAL_IOM_MODE_MAX,
            "Invalid IOM Mode");
    am_hal_debug_assert_msg(pOper->oper < AM_HAL_IOM_OPER_MAX,
            "Invalid IOM Operation");

    if ( pOper->proto == AM_HAL_IOM_PROTO_SPI )
    {
        switch ( pOper->mode )
        {
            case AM_HAL_IOM_MODE_POLL:
                if ( pOper->oper == AM_HAL_IOM_OPER_WRITE )
                {
                    ui32Status = am_hal_iom_spi_write_nq(ui32Module, pOper->peerInfo.ui32ChipSelect,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options);
                }
                else
                {
                    ui32Status = am_hal_iom_spi_read_nq(ui32Module, pOper->peerInfo.ui32ChipSelect,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options);
                }
                if ( (ui32Status == AM_HAL_IOM_SUCCESS) && pOper->pfnCallback )
                {
                    pOper->pfnCallback();
                }
                break;
            case AM_HAL_IOM_MODE_NB:
                if ( pOper->oper == AM_HAL_IOM_OPER_WRITE )
                {
                    ui32Status = am_hal_iom_spi_write_nb(ui32Module, pOper->peerInfo.ui32ChipSelect,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options, pOper->pfnCallback);
                }
                else
                {
                    ui32Status = am_hal_iom_spi_read_nb(ui32Module, pOper->peerInfo.ui32ChipSelect,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options, pOper->pfnCallback);
                }
                break;
            case AM_HAL_IOM_MODE_QUEUE:
                if ( pOper->oper == AM_HAL_IOM_OPER_WRITE )
                {
                    ui32Status = am_hal_iom_queue_spi_write(ui32Module, pOper->peerInfo.ui32ChipSelect,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options, pOper->pfnCallback);
                }
                else
                {
                    ui32Status = am_hal_iom_queue_spi_read(ui32Module, pOper->peerInfo.ui32ChipSelect,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options, pOper->pfnCallback);
                }
                break;
            default:
                // Error!
                ui32Status = AM_HAL_IOM_ERR_INVALID_PARAM;
        }
    }
    else if ( pOper->proto == AM_HAL_IOM_PROTO_I2C )
    {
        switch ( pOper->mode )
        {
            case AM_HAL_IOM_MODE_POLL:
                if ( pOper->oper == AM_HAL_IOM_OPER_WRITE )
                {
                    ui32Status = am_hal_iom_i2c_write_nq(ui32Module, pOper->peerInfo.ui32BusAddress,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options);
                }
                else
                {
                    ui32Status = am_hal_iom_i2c_read_nq(ui32Module, pOper->peerInfo.ui32BusAddress,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options);
                }
                if ( (ui32Status == AM_HAL_IOM_SUCCESS) && pOper->pfnCallback )
            ...
            case AM_HAL_IOM_MODE_NB:
                if ( pOper->oper == AM_HAL_IOM_OPER_WRITE )
                {
                    ui32Status = am_hal_iom_i2c_write_nb(ui32Module, pOper->peerInfo.ui32BusAddress,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options, pOper->pfnCallback);
                }
                else
                {
                    ui32Status = am_hal_iom_i2c_read_nb(ui32Module, pOper->peerInfo.ui32BusAddress,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options, pOper->pfnCallback);
                }
                break;
            case AM_HAL_IOM_MODE_QUEUE:
                if ( pOper->oper == AM_HAL_IOM_OPER_WRITE )
                {
                    ui32Status = am_hal_iom_queue_i2c_write(ui32Module, pOper->peerInfo.ui32BusAddress,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options, pOper->pfnCallback);
                }
                else
                {
                    ui32Status = am_hal_iom_queue_i2c_read(ui32Module, pOper->peerInfo.ui32BusAddress,
                        pOper->pui32Data, pOper->ui32NumBytes,
                        pOper->ui32Options, pOper->pfnCallback);
                }
                break;
