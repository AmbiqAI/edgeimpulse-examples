...
typedef struct
{
    ...
    //! UART clock source.
    //
    am_hal_clkmgr_clock_id_e ui32ClkSrc;
    ...
    switch (ePowerState)
    {
        ...
        case AM_HAL_SYSCTRL_WAKE:
            ...
            if (bRetainState)
            {
                ...
                am_hal_clkmgr_clock_request(pState->ui32ClkSrc, (am_hal_clkmgr_user_id_e)(AM_HAL_CLKMGR_USER_ID_UART0 + ui32Module));
        ...
        case AM_HAL_SYSCTRL_DEEPSLEEP:
            ...
            am_hal_clkmgr_clock_release(pState->ui32ClkSrc, (am_hal_clkmgr_user_id_e)(AM_HAL_CLKMGR_USER_ID_UART0 + ui32Module));
...
config_baudrate(uint32_t ui32Module, uint32_t ui32DesiredBaudrate, uint32_t *pui32ActualBaud)
{
    ...
    {
// #### INTERNAL BEGIN ####
// Remove 96MHz per CYSV-308
//      case UART0_CR_CLKSEL_HFRC_96MHz:
//          ui32UartClkFreq = 96000000;
//          break;
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#if (APOLLO5_FPGA == 8) || (APOLLO5_FPGA == 12) || (APOLLO5_FPGA == 24) || (APOLLO5_FPGA == 48) || (APOLLO5_FPGA == 96)
    //
...
// #### INTERNAL END ####
    // For 3Mbps, use PLL as clock source to get lower drifting
    if ( ( psConfig->ui32BaudRate == 3000000 ) && ( APOLLO5_GE_B1 ) )
    {
        pState->ui32ClkSrc = AM_HAL_CLKMGR_CLK_ID_SYSPLL;
// #### INTERNAL BEGIN ####
//CAB-1442 The definition of D2ASPARE is "Bit22-25:When set, it will force uart0/1/2/3_gate.clken to be 1,
//making uart0/1/2/3.pclk to be always-on"
// #### INTERNAL END ####
        // Set D2ASPARE, force uart_gate.clken to be 1, making uart.pclk to be always-on
        MCUCTRL->D2ASPARE_b.D2ASPARE |= (0x1UL << (22 + ui32Module));
        UARTn(ui32Module)->CR_b.CLKSEL = UART0_CR_CLKSEL_PLL_CLK;
    }
    else
    {
        pState->ui32ClkSrc = AM_HAL_CLKMGR_CLK_ID_HFRC;
        if ( psConfig->ui32BaudRate > 1500000 )
        {
            ...
            UARTn(ui32Module)->CR_b.CLKSEL = UART0_CR_CLKSEL_HFRC_48MHZ;
        }
        else
        {
            UARTn(ui32Module)->CR_b.CLKSEL = UART0_CR_CLKSEL_HFRC_24MHZ;
        }
    }
    am_hal_clkmgr_clock_request(pState->ui32ClkSrc, (am_hal_clkmgr_user_id_e)(AM_HAL_CLKMGR_USER_ID_UART0 + ui32Module));
