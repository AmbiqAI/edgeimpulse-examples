...
//
// Check whether to boost VDDC.
//
static void mspi_vddc_boost_set(am_hal_mspi_clock_e eCurCLK, bool bCurDDRMode , am_hal_mspi_clock_e eReqCLK, bool bReqDDRMode)
{
    bool isCurVDDCBoost = false;
    bool isReqVDDCBoost = false;

    if ( ((eCurCLK >= AM_HAL_MSPI_CLK_250MHZ) && (bCurDDRMode == true)) || ((eCurCLK >= AM_HAL_MSPI_CLK_125MHZ)  && (bCurDDRMode == false)) )
    {
        //
        // Current VDDC is boosted as MSPI IO clock >= 125MHz.
        //
        isCurVDDCBoost = true;
    }
    else
    {
        //
        // Current VDDC is not boosted.
        //
        isCurVDDCBoost = false;
    }

    if ( ((eReqCLK >= AM_HAL_MSPI_CLK_250MHZ) && (bReqDDRMode == true)) || ((eReqCLK >= AM_HAL_MSPI_CLK_125MHZ)  && (bReqDDRMode == false)) )
    {
        //
        // Require Boost VDDC for MSPI IO clock >= 125MHz.
        //
        isReqVDDCBoost = true;
    }
    else
    {
        //
        // Do not require Boost VDDC.
        //
        isReqVDDCBoost = false;
    }

    if (isReqVDDCBoost == isCurVDDCBoost)
    {
        //
        // Return if VDDC boost state is not needed to change.
        //
        return;
    }

    if (isReqVDDCBoost == true)
    {
        //
        // Boost VDDC for MSPI IO clock >= 125MHz.
        //
    ...
    {
        //
        // Restore VDDC if MSPI IO clock < 125MHz.
        //
...
//
// Check whether to boost VDDF.
//
static void mspi_vddf_boost_set(am_hal_mspi_clock_e eCurCLK, bool bCurDDRMode , am_hal_mspi_clock_e eReqCLK, bool bReqDDRMode)
{
    am_hal_pwrctrl_vddf_boost_level_e current_level = AM_HAL_PWRCTRL_VDDF_BOOST_DEFAULT;
    am_hal_pwrctrl_vddf_boost_level_e request_level = AM_HAL_PWRCTRL_VDDF_BOOST_DEFAULT;

    if ( ((eCurCLK >= AM_HAL_MSPI_CLK_250MHZ) && (bCurDDRMode == true)) || ((eCurCLK >= AM_HAL_MSPI_CLK_125MHZ)  && (bCurDDRMode == false)) )
    {
        //
        // Current VDDF is boosted to level2 as MSPI IO clock >= 125MHz.
        //
        current_level = AM_HAL_PWRCTRL_VDDF_BOOST_LEVEL2;
    }
    else if ( ((eCurCLK >= AM_HAL_MSPI_CLK_192MHZ) && (bCurDDRMode == true)) || ((eCurCLK >= AM_HAL_MSPI_CLK_96MHZ) && (bCurDDRMode == false)) )
    {
        //
        // Current VDDF is boosted to level1 as MSPI IO clock >= 96MHz and < 125MHz.
        //
        current_level = AM_HAL_PWRCTRL_VDDF_BOOST_LEVEL1;
    }
    else
    {
        //
        // Current VDDF is NOT boosted.
        //
        current_level = AM_HAL_PWRCTRL_VDDF_BOOST_DEFAULT;
    }

    if ( ((eReqCLK >= AM_HAL_MSPI_CLK_250MHZ) && (bReqDDRMode == true)) || ((eReqCLK >= AM_HAL_MSPI_CLK_125MHZ)  && (bReqDDRMode == false)) )
    {
        //
        // Require Boost VDDF for MSPI IO clock >= 125MHz.
        //
        request_level = AM_HAL_PWRCTRL_VDDF_BOOST_LEVEL2;
    }
    else if ( ((eReqCLK >= AM_HAL_MSPI_CLK_192MHZ) && (bReqDDRMode == true)) || ((eReqCLK >= AM_HAL_MSPI_CLK_96MHZ) && (bReqDDRMode == false)) )
    {
        //
        // Require Boost VDDF for MSPI IO clock >= 96MHz and < 125MHz.
        //
        request_level = AM_HAL_PWRCTRL_VDDF_BOOST_LEVEL1;
    }
    else
    {
        //
        // Do not require to boost VDDF.
        //
        request_level = AM_HAL_PWRCTRL_VDDF_BOOST_DEFAULT;
    }

    if (current_level == request_level)
    {
        //
        // Return if VDDF boost state is not needed to change.
        //
        return;
    }

    am_hal_pwrctrl_vddf_restore(current_level);
    am_hal_pwrctrl_vddf_boost(request_level);
}
...
am_hal_mspi_initialize(uint32_t ui32Module, void **ppHandle)
{
    ...
    g_MSPIState[ui32Module].bDDRMode = false;
                             ...
                             const am_hal_mspi_dev_config_t *pConfig)
{
    ...
    //
    // Check the MSPI frequency setting based on TRIM version.
    //
    if ( (g_ui32TrimVer != 0xFFFFFFFF) && (g_ui32TrimVer >= 7) )
    {
        if ((ui32Module == 1) || (ui32Module == 2))
        {
            if (pConfig->eClockFreq > AM_HAL_MSPI_CLK_96MHZ)
            {
                return AM_HAL_STATUS_OUT_OF_RANGE;
            }
        }
        else if ((ui32Module == 0) || (ui32Module == 3))
        {
            if ((pConfig->eClockFreq > AM_HAL_MSPI_CLK_250MHZ))
            {
                return AM_HAL_STATUS_OUT_OF_RANGE;
            }
            else if ((pConfig->eClockFreq > AM_HAL_MSPI_CLK_125MHZ) && (pConfig->bEmulateDDR == false))
            {
                return AM_HAL_STATUS_OUT_OF_RANGE;
            }
        }
    ...
    else if ( g_ui32TrimVer == 6 )
    {
        ...
        {
            if ((pConfig->eClockFreq > AM_HAL_MSPI_CLK_192MHZ))
            {
                return AM_HAL_STATUS_OUT_OF_RANGE;
            }
            else if ((pConfig->eClockFreq > AM_HAL_MSPI_CLK_96MHZ) && (pConfig->bEmulateDDR == false))
            {
                return AM_HAL_STATUS_OUT_OF_RANGE;
            }
        }
    ...
    switch ( pConfig->eClockFreq )
    {
        ...
        case AM_HAL_MSPI_CLK_3P91MHZ:
            ...
            {
                case 0:
                    CLKGEN->MSPIIOCLKCTRL_b.MSPI0IOCLKSEL = AM_HAL_MSPI_IO_CLK_HFRC2_250MHZ;
                    break;
                case 1:
                    CLKGEN->MSPIIOCLKCTRL_b.MSPI1IOCLKSEL = AM_HAL_MSPI_IO_CLK_HFRC2_250MHZ;
                    break;
                case 2:
                    CLKGEN->MSPIIOCLKCTRL_b.MSPI2IOCLKSEL = AM_HAL_MSPI_IO_CLK_HFRC2_250MHZ;
                    break;
                case 3:
                    CLKGEN->MSPIIOCLKCTRL_b.MSPI3IOCLKSEL = AM_HAL_MSPI_IO_CLK_HFRC2_250MHZ;
                    break;
                default:
                    return AM_HAL_STATUS_OUT_OF_RANGE;
            }
        ...
        case AM_HAL_MSPI_CLK_3MHZ:
            ...
            {
                case 0:
                    CLKGEN->MSPIIOCLKCTRL_b.MSPI0IOCLKSEL = AM_HAL_MSPI_IO_CLK_HFRC_192MHZ;
                    break;
                case 1:
                    CLKGEN->MSPIIOCLKCTRL_b.MSPI1IOCLKSEL = AM_HAL_MSPI_IO_CLK_HFRC_192MHZ;
                    break;
                case 2:
                    CLKGEN->MSPIIOCLKCTRL_b.MSPI2IOCLKSEL = AM_HAL_MSPI_IO_CLK_HFRC_192MHZ;
                    break;
                case 3:
                    CLKGEN->MSPIIOCLKCTRL_b.MSPI3IOCLKSEL = AM_HAL_MSPI_IO_CLK_HFRC_192MHZ;
                    break;
                default:
                    return AM_HAL_STATUS_OUT_OF_RANGE;
            }
    ...
    else
    {
        ...
        MSPIn(ui32Module)->DEV0CFG1_b.CELATENCY0 = AM_HAL_MSPI_CE_LATENCY_NORMAL;
    }
    ...
    //
    // Check whether to boost or restore VDDC and VDDF
    //
    if ( (g_ui32TrimVer != 0xFFFFFFFF) && (g_ui32TrimVer >= MIN_PCM_TRIM_VER) )
    {
        mspi_vddc_boost_set( pMSPIState->eClockFreq, pMSPIState->bDDRMode, pConfig->eClockFreq, pConfig->bEmulateDDR);
        mspi_vddf_boost_set( pMSPIState->eClockFreq, pMSPIState->bDDRMode, pConfig->eClockFreq, pConfig->bEmulateDDR);
    }
    ...
    //
    // Store the DDR mode state.
    //
    pMSPIState->bDDRMode = pConfig->bEmulateDDR;
    //
    // Return the status.
    //
    return AM_HAL_STATUS_SUCCESS;
...
// #### INTERNAL BEGIN ####
        // TODO - Need to also make sure that the transaction finished out of the MSPI.
        // am_hal_sysctrl_bus_write_flush() just ensures it has made in to MSPI
        #warning "FIXME: - Need to wait for all XIPMM transactions to flush out of MSPI."
// #### INTERNAL END ####
    //
    // Return the status.
    //
                             ...
                             const void *pConfig)
{
    ...
    switch ( eRequest )
    {
        ...
        case AM_HAL_MSPI_REQ_CE_LATENCY:
        {
            ...
            //
            // Set CE Latency of New(125MHz) DDR mode.
            //
        ...
        case AM_HAL_MSPI_REQ_SDR250_DIS:
            //
        ...
        case AM_HAL_MSPI_REQ_SDR250_EN:
            //
            // Enable SDR 250MHz mem interface.
            //
            MSPIn(ui32Module)->DEV0CFG1_b.SDR250EN0 = 1;
...
// #### INTERNAL END ####
        ...
        case AM_HAL_MSPI_REQ_CLOCK_CONFIG:
        {
            ...
            if ( (g_ui32TrimVer != 0xFFFFFFFF) && (g_ui32TrimVer >= 7) )
            {
                ...
                {
                    if ((eClockFreq > AM_HAL_MSPI_CLK_250MHZ))
                    {
                        return AM_HAL_STATUS_OUT_OF_RANGE;
                    }
                    else if ((eClockFreq > AM_HAL_MSPI_CLK_125MHZ) && (pMSPIState->bDDRMode == false))
                    {
                        return AM_HAL_STATUS_OUT_OF_RANGE;
                    }
                }
            ...
            else if ( g_ui32TrimVer == 6 )
            {
                ...
                {
                    if ((eClockFreq > AM_HAL_MSPI_CLK_192MHZ))
                    {
                        return AM_HAL_STATUS_OUT_OF_RANGE;
                    }
                    else if ((eClockFreq > AM_HAL_MSPI_CLK_96MHZ) && (pMSPIState->bDDRMode == false))
                    {
                        return AM_HAL_STATUS_OUT_OF_RANGE;
                    }
                }
            ...
            if ( (g_ui32TrimVer != 0xFFFFFFFF) && (g_ui32TrimVer >= MIN_PCM_TRIM_VER) )
            {
                mspi_vddc_boost_set( pMSPIState->eClockFreq, pMSPIState->bDDRMode, eClockFreq, pMSPIState->bDDRMode);
                mspi_vddf_boost_set( pMSPIState->eClockFreq, pMSPIState->bDDRMode, eClockFreq, pMSPIState->bDDRMode);
            }
...
// #### INTERNAL BEGIN ####
        // TODO - Need to test and check whether HFRC2 software control is must have.
        // If yes, then we need to make it conditional to the frequency.
#warning "TODO FIXME: - Need to check whether HFRC2 software control is must have."
// #### INTERNAL END ####
#if HFRC2_SW_CTRL_EN
            //
            // HFRC2 ON request.
            ...
            if (bRetainState)
            {
                ...
                //
                // Check whether to boost VDDC and VDDF for high speed mode
                //
                if ( (g_ui32TrimVer != 0xFFFFFFFF) && (g_ui32TrimVer >= MIN_PCM_TRIM_VER) )
                {
                    bool isReqDDRmode = false;
                    isReqDDRmode = (pMSPIState->registerState.regDEV0DDR & MSPI0_DEV0DDR_EMULATEDDR0_Msk) ? true : false ;

                    mspi_vddc_boost_set( AM_HAL_MSPI_CLK_INVALID, false, pMSPIState->eClockFreq, isReqDDRmode);
                    mspi_vddf_boost_set( AM_HAL_MSPI_CLK_INVALID, false, pMSPIState->eClockFreq, isReqDDRmode);
                }
...
// #### INTERNAL END ####
            ...
            //
            // restore VDDC and VDDF after MSPI power down
            //
            if ( (g_ui32TrimVer != 0xFFFFFFFF) && (g_ui32TrimVer >= MIN_PCM_TRIM_VER) )
            {
                mspi_vddc_boost_set( pMSPIState->eClockFreq, pMSPIState->bDDRMode, AM_HAL_MSPI_CLK_INVALID, false);
                mspi_vddf_boost_set( pMSPIState->eClockFreq, pMSPIState->bDDRMode, AM_HAL_MSPI_CLK_INVALID, false);
            }
        break;
