...
am_hal_systick_init(am_hal_systick_t eClockSource)
{
    ...
    //
    // Set the CLKGEN SysTick clock enable/disable to save power when external reference is not in use.
    //
    switch (eClockSource)
    {
        case AM_HAL_SYSTICK_CLKSRC_EXT:
            // Enable the Systick Clock if using internal clock source.
            CLKGEN->CLKCTRL_b.STCLKDIS = CLKGEN_CLKCTRL_STCLKDIS_EN;
            break;
        case AM_HAL_SYSTICK_CLKSRC_INT:
            // Disable the external Systick reference if using internal clock source.
            CLKGEN->CLKCTRL_b.STCLKDIS = CLKGEN_CLKCTRL_STCLKDIS_DIS;
            break;
    }
...
am_hal_systick_ticks_per_ms(uint32_t *TicksPerMs)
{
    ...
    switch(g_SysTickClockSource)
    {
        case AM_HAL_SYSTICK_CLKSRC_EXT:
            val = SysTick->CALIB;
            *TicksPerMs = _FLD2VAL(SysTick_CALIB_TENMS, val) / 10;
            break;
