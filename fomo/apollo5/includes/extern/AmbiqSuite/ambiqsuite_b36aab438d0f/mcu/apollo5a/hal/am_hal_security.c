...
am_hal_security_get_info(am_hal_security_info_t *pSecInfo)
{
    ...
    //
    // On the Apollo5 RevA, the SBL_VERSION and SBR_VERSION defines were removed
    // from the INFO1 registers. Though the SBR version is obtainable from the
    // SBR, the SBL version is not available so return default values.
    //
