...
am_hal_gpio_pinconfig(uint32_t ui32GpioNum, const am_hal_gpio_pincfg_t sGpioCfg)
{
    ...
    // #### INTERNAL BEGIN ####
    // We currently don't know exactly which pins can't be used as GPIO.
    // All information is derived from CYSV-89 and Cayenne A0 Daily Bring-Up.
    #warning "Pins that can't be used as gpio may be incomplete."
    // #### INTERNAL END ####
    //
    // These pins can't be used as GPIO
    //
    if ((ui32GpioNum == 114) || (ui32GpioNum == 199))
    {
        if (sGpioCfg.GP.cfg_b.uFuncSel == AM_HAL_PIN_0_GPIO)
        {
    ...
    {
        //
        // For high-speed pads, there is only 50kohm pull-up/pull-down options.
        //
