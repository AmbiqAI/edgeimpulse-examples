...
{
    DSI->MAXRETPACSZE = (uint32_t) (!bHS) << DSI_MAXRETPACSZE_HSLP_Pos | (uint32_t)ui8DataLen;
    return AM_HAL_STATUS_SUCCESS;
}
...
//
//*****************************************************************************
#define REG_MCU_CTRL_DSIBIST            0x400204A0UL
#define REG_DSI_STOP_STATE_BITFIELD     (0x1UL << 28)
uint32_t
...
// #### INTERNAL END ####
    ...
    uint32_t ui32RegDSIBist = AM_REGVAL(REG_MCU_CTRL_DSIBIST);
    //
    // bitfields 8-15 -> 6'b01_0011
    //
    AM_REGVAL(REG_MCU_CTRL_DSIBIST) = (ui32RegDSIBist & ~0xFF00UL) | 0x1300UL;
    ...
    //
    if ( AM_HAL_STATUS_SUCCESS != am_hal_delay_us_status_change(1000, (uint32_t)REG_MCU_CTRL_DSIBIST, REG_DSI_STOP_STATE_BITFIELD, REG_DSI_STOP_STATE_BITFIELD) )
    {
        return AM_HAL_STATUS_TIMEOUT;
    }
    ...
    //
    AM_REGVAL(REG_MCU_CTRL_DSIBIST) = ui32RegDSIBist;
