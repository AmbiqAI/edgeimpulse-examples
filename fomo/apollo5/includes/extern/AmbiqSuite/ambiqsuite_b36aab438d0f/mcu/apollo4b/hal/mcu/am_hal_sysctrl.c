...
// #### INTERNAL BEGIN ####
#else
                    MCUCTRL->VREFGEN2_b.TVRGVREFTRIM = _FLD2VAL(MCUCTRL_VREFGEN2_TVRGVREFTRIM, g_trim_reg_origvals.ui32VREFGEN2);
#endif
// #### INTERNAL END ####
                    am_hal_delay_us(1);
                }
...
// #### INTERNAL BEGIN ####
#else
                MCUCTRL->SIMOBUCK13_b.SIMOBUCKACTTRIMVDDS = _FLD2VAL(MCUCTRL_SIMOBUCK13_SIMOBUCKACTTRIMVDDS, g_trim_reg_origvals.ui32SB13);
#endif
// #### INTERNAL END ####
    ...
    if ( bBuckIntoLPinDS )
    {
        ...
        // Turn on coreldo and memldo again when exit from deepsleep.
        //
        // ################################################################################
        //
        // Jamie says (8/18/21):
        // We should add the 20us delay after the switch to LDO.  When we switch to LDO mode,
        // adding a 20us delay will allow VDDC and VDDF to charge to their correct
        // active levels. Because VDDC and VDDF will be charged to their correct
        // active levels, there is no need to add an additional delay
        // after enabling simobuck. We made a decision to make
        // sure the simobuck gets enabled before the enabling the LDOs in
        // parallel since we haven't yet tested the other way around.
        // As long as we enabled the LDOs in parallel immediately after the simobuck
        // gets enabled (within microseconds) the external cap will provide enough
        // charge until the LDOs are enabled in parallel so there is no window of
        // vulnerability.  However, if we can't guarantee the time between enabling
        // the simobuck and enabling the LDOs in parallel, we should swap the two.
        // #### INTERNAL END ####
        // #### INTERNAL BEGIN ####
        // 1/20/22
        // A2SD-2248 Software workaround required for disabling the simobuck
        // Due to VDDC dropping when coming out of deepsleep at cold, need to
        // enable the LDOs in parallel and force into active before disabling
        // the simobuck.
        // This issue applies to both revB and revC as part of FB-164.
        // #### INTERNAL END ####
        //
