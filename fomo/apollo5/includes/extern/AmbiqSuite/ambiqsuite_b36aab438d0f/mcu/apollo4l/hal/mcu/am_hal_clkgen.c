...
clkgen_sample_and_ave_HF2VAL(uint32_t ui32NumSamples, uint32_t ui32UsecDelay)
{
    ...
    if (ui32NumSamples)
    {
        // #### INTERNAL BEGIN ####
#if 0
        #define SAVED_SAMPLE_SIZE_MAX 16
        int32_t samples[SAVED_SAMPLE_SIZE_MAX];
#endif
        // #### INTERNAL END ####
        for (uint32_t i = 0; i < ui32NumSamples; i++)
        {
            ...
            // #### INTERNAL BEGIN ####
#if 0
            if ( i < SAVED_SAMPLE_SIZE_MAX) samples[i] = i32SampleVal;
#endif
            // #### INTERNAL END ####
        ...
        // Debug prints
#if 0
        for (int i = 0; i < ui32NumSamples; i+= 4){
            if ( (i+3) >= SAVED_SAMPLE_SIZE_MAX) break;
            am_util_stdio_printf("samples %d %08lx %08lx %08lx %08lx\n", i, samples[i+0],samples[i+1],samples[i+2],samples[i+3] );
        }
#endif
        // #### INTERNAL END ####
    }
...
// #### INTERNAL BEGIN ####
            ...
            am_util_stdio_printf("hfrc2 tune update = %08lx %08lx\n", ui32Hfrc2Trim, MCUCTRL->HFRC2);
#endif
// #### INTERNAL END ####
        }
        else
...
// #### INTERNAL BEGIN ####
#if 0   // FIXME, the XTAL24MCTRL register was removed from the Falcon design
            am_util_stdio_printf("hfrc2 tune no change = %08lx  %08lx\n", ui32Hfrc2Trim, MCUCTRL->HFRC2);
#endif
// #### INTERNAL END ####
