...
AM_SHARED_RW uint32_t DMATCBBuf[2048];
AM_SHARED_RW am_devices_cooper_buffer(1024) g_psWriteData;
AM_SHARED_RW am_devices_cooper_buffer(1024) g_psReadData;
#else
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#if defined APOLLO4_ATE
#define ATE_GPIO_PIN        (33)

#define ATE_RAM_START_ADDR       0x10150000
#define ATE_RAM_SIZE             0x400
#define ATE_COUNT_THRESHOLD_ADDR ATE_RAM_START_ADDR
#define ATE_RX_TEST_RESULT_ADDR  (ATE_COUNT_THRESHOLD_ADDR + 4)
#define ATE_RESPONSE_LEN_ADDR    (ATE_RX_TEST_RESULT_ADDR + 4)
#define ATE_RESPONSE_ADDR        (ATE_RESPONSE_LEN_ADDR + 4)

uint8_t end_test_cmd[HCI_VSC_CMD_LENGTH(0)] = HCI_RAW_CMD(0x201F, 0);
uint8_t ui8RamClear[ATE_RAM_SIZE] = {0};

#define HCI_ATE_TYPE             9
#define HCI_ATE_HEADER_LEN       3
#define HCI_DEBUG_TYPE           8
#define HCI_DEBUG_HEADER_LEN     1
...
// #### INTERNAL BEGIN ####
#if defined APOLLO4_ATE
static bool read_apollo_memory(uint32_t start_addr, eMemAccess_type size, uint32_t length);
static bool write_apollo_memory(uint32_t start_addr, eMemAccess_type size, uint32_t length, uint8_t* data, bool loop);
#endif
// #### INTERNAL END ####
//*****************************************************************************
//
...
// #### INTERNAL BEGIN ####
#if defined APOLLO4_ATE
        am_hal_mcuctrl_control_arg_t ctrlArgs = g_amHalMcuctrlArgBLEDefault;
...
uint16_t serial_rx_hci_state_machine(uint8_t *pBuf, uint16_t len)
{
    ...
    while (len)
    {
        ...
        else if (stateRx == HCI_RX_STATE_HEADER)
        {
            ...
            // #### INTERNAL BEGIN ####
            #if defined APOLLO4_ATE
            else if ( pktIndRx == HCI_ATE_TYPE )      // ATE command
            {
                hdrLen = HCI_ATE_HEADER_LEN;
            }
            else if ( pktIndRx == HCI_DEBUG_TYPE )    // For debug use
            {
                hdrLen = HCI_DEBUG_HEADER_LEN;
            }
            #endif
            // #### INTERNAL END ####
            else
            ...
            if (iRx == hdrLen)
            {
                ...
                else if ( pktIndRx == HCI_DEBUG_TYPE )
                {
                    dataLen = 0;
                }
                #endif
...
// #### INTERNAL BEGIN ####
#if defined(USE_NONBLOCKING_HCI)
    AM_SHARED_RW am_devices_cooper_buffer(1024) psReadData;
#else
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#if defined APOLLO4_ATE
//    if ( memcmp(pui32Data, end_test_cmd, (uint32_t)4) == 0 ) // for now do not check to make ATE work
    {
        uint32_t ui32Threshold = (*(volatile uint32_t*)(ATE_COUNT_THRESHOLD_ADDR));
        uint32_t ui32Count = ((uint32_t)psReadData.bytes[7]) | (((uint32_t)psReadData.bytes[8]) << 8);
        if ( ui32Count >= ui32Threshold )
        {
            (*(volatile uint32_t*)(ATE_RX_TEST_RESULT_ADDR)) = 1;
            am_hal_gpio_state_write(ATE_GPIO_PIN, AM_HAL_GPIO_OUTPUT_SET);
        }
        else
        {
            (*(volatile uint32_t*)(ATE_RX_TEST_RESULT_ADDR)) = 2;
            am_hal_gpio_state_write(ATE_GPIO_PIN, AM_HAL_GPIO_OUTPUT_CLEAR);
        }
    }
    (*(volatile uint32_t*)(ATE_RESPONSE_LEN_ADDR)) = ui32Length;
    write_apollo_memory(ATE_RESPONSE_ADDR, RD_8_Bit, ATE_RAM_SIZE - (ATE_RESPONSE_ADDR - ATE_RAM_START_ADDR), ui8RamClear, false);
    write_apollo_memory(ATE_RESPONSE_ADDR, RD_8_Bit, ui32Length, psReadData.bytes, true);
#else
// #### INTERNAL END ####
// #### INTERNAL BEGIN ####
#ifdef HCI_DEBUG_ON
    am_util_stdio_printf("AP4 RX: %d \n", ui32Length);
    {
        uint16_t i = 0;
        for ( i = 0; i < ui32Length; i++ )
        {
            am_util_stdio_printf("%02x ", g_psReadData.bytes[i]);
        }
...
// #### INTERNAL BEGIN ####
#if defined(USE_NONBLOCKING_HCI)
    AM_SHARED_RW am_devices_cooper_buffer(1024) psWriteData;
#else
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#if defined APOLLO4_ATE
    else if ( psWriteData.bytes[0] == 0x09 )
    {
        uint32_t ui32StartAddr = (psWriteData.bytes[4]) | (psWriteData.bytes[5] << 8) | (psWriteData.bytes[6] << 16) | (psWriteData.bytes[7] << 24);
        // Read Falcon memory
        if ( (psWriteData.bytes[1] == 0x01) && (psWriteData.bytes[2] == 0xFC) )
        {
            read_apollo_memory(ui32StartAddr, (eMemAccess_type)psWriteData.bytes[8], psWriteData.bytes[9]);
        }
        // Write Falcon memory
        else if ( (psWriteData.bytes[1] == 0x02) && (psWriteData.bytes[2] == 0xFC) )
        {
            write_apollo_memory(ui32StartAddr, (eMemAccess_type)psWriteData.bytes[8], psWriteData.bytes[9], &psWriteData.bytes[10], true);
        }
        // Validate Cooper exchange memory
        else if ( (psWriteData.bytes[1] == 0x04) && (psWriteData.bytes[2] == 0xFC) )
        {
            uint32_t ui32Length = (psWriteData.bytes[9]) | (psWriteData.bytes[10] << 8) | (psWriteData.bytes[11] << 16) | (psWriteData.bytes[12] << 24);
            cooper_eram_test_execute(ui32StartAddr, (eMemAccess_type)psWriteData.bytes[8], ui32Length);
        }
    }
    else if ( psWriteData.bytes[0] == 0x08 )
    {
        if ( psWriteData.bytes[1] == 0x01 )
        {
            am_util_stdio_printf("hci_reset \r\n");
            am_util_ble_hci_reset(g_IomDevHdl);
        }
        else if ( psWriteData.bytes[1] == 0x02 )
        {
            am_util_stdio_printf("disable_sleep \r\n");
            am_util_ble_sleep_set(g_IomDevHdl, false);
        }
        else if ( psWriteData.bytes[1] == 0x03 )
        {
            am_util_stdio_printf("enable transmitter test \r\n");
            am_util_ble_trasmitter_test_ex(g_IomDevHdl, 0, 1);
        }
        else if ( psWriteData.bytes[1] == 0x04 )
        {
            am_util_stdio_printf("reset Cooper\r\n");
            am_devices_cooper_reset_with_sbl_check(g_IomDevHdl, &stCooperConfig);
        }
        else if ( psWriteData.bytes[1] == 0x05 )
        {
            am_util_stdio_printf("clear GPIO33\r\n");
            am_hal_gpio_state_write(ATE_GPIO_PIN, AM_HAL_GPIO_OUTPUT_CLEAR);
        }
    }
#endif
// #### INTERNAL END ####
    else
    {
        ...
        {
            uint16_t i = 0;
            am_util_stdio_printf("AP4 TX: %d \n", ui32Length);

            for ( i = 0; i < ui32Length; i++ )
            {
                am_util_stdio_printf("%02x ", psWriteData.bytes[i]);
            }
            am_util_stdio_printf("\n\n");
        }
        #endif
...
#if defined APOLLO4_ATE
    // ATE GPIO init
    am_hal_gpio_pinconfig(ATE_GPIO_PIN, am_hal_gpio_pincfg_output);
    am_hal_gpio_state_write(ATE_GPIO_PIN, AM_HAL_GPIO_OUTPUT_CLEAR);
#endif
// #### INTERNAL END ####
}
...
{
    uint32_t BytesNum = 0;
    if ( (length > ATE_RAM_SIZE )
            || ((size != RD_8_Bit) && (size != RD_16_Bit) && (size != RD_32_Bit)))
    {
        return false;
    }
    BytesNum = size / RD_8_Bit * length;
    for ( uint32_t i = 0; i < BytesNum; i++ )
    {
        g_psReadData.bytes[i] = (*(volatile uint8_t*)(start_addr + i));
    }

    serial_data_write(g_psReadData.bytes, BytesNum);
...
static bool write_apollo_memory(uint32_t start_addr, eMemAccess_type size, uint32_t length, uint8_t* data, bool loop)
{
    uint32_t BytesNum, ui32Value, index = 0;
    if ( (length > ATE_RAM_SIZE )
            || ((size != RD_8_Bit) && (size != RD_16_Bit) && (size != RD_32_Bit)) )
    {
        return false;
    ...
    BytesNum = size / RD_8_Bit * length;
    for ( index = 0; index < BytesNum / 4; index++ )
    {
        ui32Value = (uint32_t)data[index * 4] | ((uint32_t)data[index * 4 + 1]) << 8 | ((uint32_t)data[index * 4 + 2]) << 16 | ((uint32_t)data[index * 4 + 3]) << 24;
        (*(volatile uint32_t*)(start_addr + index * 4)) = ui32Value;
    }
    BytesNum %= 4;
    if ( BytesNum )
    {
        ui32Value = (*(volatile uint32_t*)(start_addr + index * 4));
        for ( uint8_t i = 0; i < BytesNum; i++ )
        {
            ui32Value = (ui32Value & (~(0xF << (8 * i)))) | (((uint32_t)data[index * 4 + i]) << (8 * i));
        }
        (*(volatile uint32_t*)(start_addr + index * 4)) = ui32Value;
    }
    if ( loop )
    {
        read_apollo_memory(start_addr, size, length);
    }
    return true;
}
#endif
...
// #### INTERNAL BEGIN ####
    ...
    //
    // We need to set interrupt priority higher than 0 to make RevB crypto workaround work, make it
    // same as the one (NVIC_configMAX_SYSCALL_INTERRUPT_PRIORITY) used in our FreeRTOS.
    //
    NVIC_SetPriority(COOPER_IOM_IRQn, 0x04);
...
// #### INTERNAL BEGIN ####
#if defined APOLLO4_ATE
    write_apollo_memory(ATE_RAM_START_ADDR, RD_8_Bit, ATE_RAM_SIZE, ui8RamClear, false);
#endif
// #### INTERNAL END ####
