...
crypto_power_up(void)
{
    ...
    uint32_t ui32Status = am_hal_pwrctrl_periph_enabled(AM_HAL_PWRCTRL_PERIPH_OTP, &bOTPEnabled);
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
        PRINT("Error during read of OTP power status\n");
    }
    if (!bOTPEnabled)
    {
        ...
        if (AM_HAL_STATUS_SUCCESS != ui32Status)
        {
            PRINT("Error in power up of OTP module\n");
        }
    }
    //
    ...
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
        PRINT("Error in power up of Crypto module\n");
    }
#endif
}
...
crypto_power_down(void)
{
    ...
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
        PRINT("Error in power down of Crypto module\n");
    }
    //
    ...
    if (AM_HAL_STATUS_SUCCESS != ui32Status)
    {
        PRINT("Error in power down of OTP module\n");
    }
    else
    {
        PRINT("Crypto powered down!\n");
    }
#endif
}
