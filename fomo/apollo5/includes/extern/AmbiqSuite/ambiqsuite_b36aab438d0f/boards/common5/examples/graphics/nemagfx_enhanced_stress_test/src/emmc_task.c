...
#define MAX_EMMC_TASK_DELAY    (200)
...
AM_SHARED_RW uint8_t gEmmcSSRAMWrBuf[BUF_LEN];
AM_SHARED_RW uint8_t gEmmcSSRAMRdBuf[BUF_LEN];
...
void EmmcTask(void *pvParameters)
{
    ...
    uint8_t         *ui8PsramWrBuf;
    uint8_t         *ui8WrBuf;
    uint8_t         *ui8RdBuf;
    ui8PsramWrBuf = (uint8_t *)MSPI_XIP_BASE_ADDRESS;
    ...
    //
    ui8WrBuf = ui8PsramWrBuf;
    for (int i = 0; i < BUF_LEN; i++)
    {
        ui8WrBuf[i] = i % 256;
    }
        ui8RdBuf = (uint8_t *)(MSPI_XIP_BASE_ADDRESS) + BUF_LEN;
    ...
    for (int i = 0; i < BUF_LEN; i++)
    {
        gEmmcSSRAMWrBuf[i] = i % 256;
        gEmmcSSRAMRdBuf[i] = 0x0;
    }
...
#endif
        //switch between SSRAM and XIPMM
        if ( ui8WrBuf == ui8PsramWrBuf)
        {
            ui8WrBuf = gEmmcSSRAMWrBuf;
            ui8RdBuf = gEmmcSSRAMRdBuf;
        }
        else
        {
            ui8WrBuf = ui8PsramWrBuf;
            ui8RdBuf = ui8PsramWrBuf + BUF_LEN;
        }
