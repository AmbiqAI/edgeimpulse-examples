...
setup_task(void *pvParameters)
{
    ...
    xTaskCreate(MspiTask, "MspiTask", 1024, 0, TASK_PRIORITY_LOW, &MspiTaskHandle);
#endif
    ...
    //
    // Create the MVE workload task
    //
#if ( MVE_TASK_ENABLE == 1 )
    xTaskCreate(MveTask, "MveTask", 512, 0, TASK_PRIORITY_MIDDLE, &mve_task_handle);
#endif
...
run_tasks(void)
{
    ...
    //
    #if ( XIP_TASK_ENABLE == 1 )
        #if (MSPI_PSRAM_XIPCODE_MODULE == 3)
            prvHeapInit_external((void*)(MSPI3_APERTURE_START_ADDR + PSRAM_HEAP_AREA_OFFSET), PSRAM_HEAP_AREA_SIZE);
        #elif (MSPI_PSRAM_XIPCODE_MODULE == 0)
            prvHeapInit_external((void*)(MSPI0_APERTURE_START_ADDR + PSRAM_HEAP_AREA_OFFSET), PSRAM_HEAP_AREA_SIZE);
        #endif
    #endif
