...
static void throughputProcMsg(tpMsg_t *pMsg)
{
    ...
    // #### INTERNAL BEGIN ####
    #if RSSI_TEST
        case RSSI_READ_TIMER_IND:
            if ((throughputCb.connId = AppConnIsOpen()) != DM_CONN_ID_NONE)
            {
                /* read RSSI value for the active connection */
                DmConnReadRssi(throughputCb.connId);
            }
            WsfTimerStartMs(&throughputCb.rssi_read, 1000);
        break;

        case DM_CONN_READ_RSSI_IND:
        {
            hciReadRssiCmdCmplEvt_t *evt = (hciReadRssiCmdCmplEvt_t*) pMsg;
    ...
    // #### INTERNAL END ####
        ...
        case DM_CONN_UPDATE_IND:
        {
            ...
            if (evt->status == 0)
            {
                ...
                // #### INTERNAL BEGIN ####
                #if RSSI_TEST
                throughputCb.connId = evt->handle;
                WsfTimerStartMs(&throughputCb.rssi_read, 1000);
                #endif
                // #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
#if RSSI_TEST
    throughputCb.rssi_read.handlerId = handlerId;
    throughputCb.rssi_read.msg.event = RSSI_READ_TIMER_IND;
#endif
// #### INTERNAL END ####
