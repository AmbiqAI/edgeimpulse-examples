...
AM_SHARED_RW uint8_t gHplpSSRAMBuf[BUFFER_SIZE];
...
HPLPTask(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        //copy data from MRAM to sram
        memcpy(gHplpSSRAMBuf, MRAM_BUFFER_LOCATION, BUFFER_SIZE);

        //calculate checksum in  sram
        stride = rand() % MAX_STRIDE + 1;

        sum_ssram = checksum_calculate(gHplpSSRAMBuf, stride);
