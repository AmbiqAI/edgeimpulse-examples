...
int mith_report_results(ee_workload *workload, uint64_t total_time, e_u32 num_contexts, unsigned int num_workers);
...
/* Function: mith_main
    ...
    */
uint64_t mith_main_loop(ee_workload *workload, unsigned int num_iterations, unsigned int num_contexts, Bool oversubscribe_allowed) {
    uint64_t total_time;
    e_u32 i;
...
int mith_main(ee_workload *workload, unsigned int num_iterations, unsigned int num_contexts, Bool oversubscribe_allowed, unsigned int num_workers) {
    uint64_t total_time=mith_main_loop(workload, num_iterations, num_contexts, oversubscribe_allowed);
#if REPORT_THMALLOC_STATS
