...
void touch_process(void)
{
    ...
    {
        if (am_devices_chsc5816_get_point(g_pCHSC5816Handle, &g_sIntTouchInfo) == AM_DEVICES_CHSC5816_STATUS_SUCCESS)
        {
            ...
            if(touch_press == true)
            {
                if (asset_touch_info(&g_sTouchInfo))
                {
                    if (g_sTouchInfo.y0 < 200)
                    {
                        ...
                        am_devices_chsc5816_fw_update(g_pCHSC5816Handle, FW_NEW_VERSION);
                    ...
                    else if (g_sTouchInfo.y0 > 266)
                    {
                        ...
                        am_devices_chsc5816_fw_update(g_pCHSC5816Handle, FW_OLD_VERSION);
...
void fw_update_main_screen(uint8_t fw_ver, uint32_t up_button_color, uint32_t down_button_color)
{
    ...
    memcpy(buf, "Touch to burn FW v 4\n", strlen("Touch to burn FW v 4\n"));
    nema_print(buf, 130, 100, 200, 32, KLEIN_BLUE, NEMA_ALIGNX_LEFT);

    memset(buf, 0, sizeof(buf));
    memcpy(buf, "Touch to burn FW v 3\n", strlen("Touch to burn FW v 3\n"));
...
void fw_update_print(void)
{
    ...
    {
        memcpy(buf, "Complete, Touch FW ver is v 3\n", strlen("Complete, Touch FW ver is v 3\n"));
    }
    else if (g_sTouchInfo.fw_ver == 4)
    {
        memcpy(buf, "Complete, Touch FW ver is v 4\n", strlen("Complete, Touch FW ver is v 4\n"));
    }
...
void crossing_lines(uint8_t screen_clear)
{
    char buf[12];
    int x1, y1;
    x1 = RESX / 2 - 40;
    ...
    {
        nema_draw_line(0, g_sTouchInfo.y1, RESX, g_sTouchInfo.y1, GREEN);
        nema_draw_line(g_sTouchInfo.x1, 0, g_sTouchInfo.x1, RESY, GREEN);
        nema_draw_circle(g_sTouchInfo.x1, g_sTouchInfo.y1, 20, GREEN);
    }
