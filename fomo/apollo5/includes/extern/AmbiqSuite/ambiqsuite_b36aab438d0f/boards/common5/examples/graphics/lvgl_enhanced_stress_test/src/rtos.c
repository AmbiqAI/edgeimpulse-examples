...
setup_task(void *pvParameters)
{
    ...
    xTaskCreate(HPLPTask, "HPLPTask", 512, 0, 1, &HPLPTaskHandle);
#endif
    ...
    xTaskCreate(XipTask, "XipTask", 1024, 0, 1, &XipTaskHandle);
#endif
    ...
    xTaskCreate(crypto_task, "crypto_task", 1024, 0, 1, &CryptoTaskHandle);
#endif
    ...
    //
    xTaskCreate(GuiTask, "GuiTask", (4*1024), 0, 2, &GuiTaskHandle);
#endif
    ...
    xTaskCreate(DisplayTask, "DisplayTask", 1024, 0, 3, &DisplayTaskHandle);
#endif
    //
    // Create the mspi tasks
    //
#if ( MSPI_TASK_ENABLE == 1 )
    xTaskCreate(MspiTask, "MspiTask", 1024, 0, 3, &MspiTaskHandle);
#endif
    ...
    xTaskCreate(IomPsramTask, "IomPsramTask", 512, 0, 3, &IomPsramTaskHandle);
#endif
    ...
    xTaskCreate(AdcTask, "AdcTask", 512, 0, 3, &AdcTaskHandle);
#endif
    ...
    xTaskCreate(EmmcTask, "EmmcTask", 512, 0, 3, &emmc_task_handle);
#endif
    //
    // Create the uart tasks
    //
#if ( UART_TASK_ENABLE == 1 )
    xTaskCreate(UARTTask, "UartTask", 512, 0, 2, &UARTTaskHandle);
#endif
    //
