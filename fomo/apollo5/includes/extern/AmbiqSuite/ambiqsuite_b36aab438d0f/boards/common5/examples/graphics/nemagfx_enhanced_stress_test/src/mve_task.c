...
//
// 0: disable, 1: enable
#define PMU_PROFILING_EN 0

#define GPIO_MVE_TASK 135
...
//
// The mve task as configured takes ~10ms per loop to complete.  During this time,
// the idle task will never be called and the CPU will never go to sleep.
// Set the maximum task delay to 2x the task duration to ensure the chip spends
// enough time entering and exiting deepsleep
//
#define MAX_MVE_TASK_DELAY    (10 * NUM_LOOPS * 2)
...
MveTask(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        randomDelay = rand() % MAX_MVE_TASK_DELAY;
