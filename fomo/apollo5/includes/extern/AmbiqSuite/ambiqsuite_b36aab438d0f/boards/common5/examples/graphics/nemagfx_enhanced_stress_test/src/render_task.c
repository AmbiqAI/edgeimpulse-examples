...
RenderTask(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        //start GPU, submit CL
        nema_cl_submit(&cl);
				// This delay is added as a workaround to the external reset issue,
				// where the test fails when an external reset is button is pressed.
				//am_util_delay_ms(20);
#ifdef USE_DEBUG_PIN_IN_GUI_TASK
        ...
        if ( eGPUMode == AM_HAL_PWRCTRL_GPU_MODE_HIGH_PERFORMANCE )
        {
            ...
            am_util_stdio_printf("Enter %s MODE!\n", mode_string);
        }
        else
        {
