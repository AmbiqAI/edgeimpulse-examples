...
uint32_t uart_test_instances[] =  {0};
...
typedef struct
{
    ...
    uint8_t pui8QueueTx[UART_BUFFER_SIZE] __attribute__((aligned(4096)));;
    uint8_t pui8QueueRx[UART_BUFFER_SIZE] __attribute__((aligned(4096)));;

    uint8_t pui8SpaceTx[UART_BUFFER_SIZE] __attribute__((aligned(4096)));;
    uint8_t pui8SpaceRx[UART_BUFFER_SIZE] __attribute__((aligned(4096)));;
    ...
    while(1)
    {
        for (i = 0; i < UART_TEST_INSTANCE_TOTAL; i++)
        {
            ...
            if ( *sTransaction.pui32BytesTransferred != sTransaction.ui32NumBytes )
            {
                ...
                am_util_stdio_printf("\nBytes Transferred :%d, Total Bytes :%d ", *sTransaction.pui32BytesTransferred, sTransaction.ui32NumBytes );
                vErrorHandler();
            }
