...
//
// The xip task loops_all_mid_10k_sp_main takes ~120 seconds
// to complete.  During this time, the idle task will never be called and the
// CPU will never go to sleep.  Set the maximum task delay to 2x the task duration
// to ensure the chip spends enough time entering and exiting deepsleep
//
#define MAX_XIP_TASK_DELAY    (120 * 1000 * 2)
...
XipTask(void *pvParameters)
{
    ...
    // Workaround for XIP hardfault issue.
    am_hal_cachectrl_dcache_invalidate(NULL, true);
    am_util_stdio_printf("Invalidating DCACHE\r\n");

#else
    #error "Please select a memory to handle the XIP operations.!"
#endif

    //
    // Wait 10 seconds before starting the crypto task to allow the 
    // device to enter and exit deepsleep several times first,
    // as this workload will keep the device awake for ~2 minutes
    //
    vTaskDelay(10000);
