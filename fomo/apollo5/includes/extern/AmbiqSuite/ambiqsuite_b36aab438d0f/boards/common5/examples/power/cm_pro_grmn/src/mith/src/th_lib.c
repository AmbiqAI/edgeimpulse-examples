...
// va_end( args );
   rv = am_util_stdio_printf(format );
   return rv;
}
/* Function: th_vfprintf
    ...
    Note:
        Redirected to stdio in initial version of MITH
*/
int      th_vfprintf(ee_FILE *fp, const char *format, va_list ap){*fp = *fp; return am_util_stdio_printf(format, ap);}
/* Function: th_fseek
...
int th_fpprintf( const char *fmt, ... )
{
   ...
   if (l<500) {
       ...
       *po=0;
//     va_start( args, fmt );
       rv=am_util_stdio_printf(cfmt ,args);
//     va_end( args );
   } else {
//     va_start( args, fmt );
       rv=am_util_stdio_printf(fmt ,args);
//     va_end( args );
   }
   return rv;
}
