...
bool user_task_queue_send(UserQueueElem_t *elem)
{
    if (xPortIsInsideInterrupt() == pdTRUE)
    {
        ...
        xResult =
            xQueueSendFromISR(g_UserQueue, elem, &xHigherPriorityTaskWoken);
...
void user_task_setup(void)
{
    ...
    {
        am_util_debug_printf("user task queue create failed\r\n");
    }

    int ret = metal_thread_init(&user_task_handle, user_task, "user task",
                                2 * 1024, 0, 3);
    if (ret)
    {
...
static void user_task(void *pvParameters)
{
    ...
    am_util_debug_printf("audio_pipeline_start\r\n");
