...
#define NSECS_PER_SEC   MY_CORECLK_HZ
#define EE_TIMER_TICKER_RATE (NSECS_PER_SEC / TIMER_RES_DIVIDER)
#define ALTIMETYPE uint64_t
#define GETMYTIME(_t) (*_t = ((uint64_t)0x00FFFFFF - (uint64_t)am_hal_systick_count() + g_ui64SysTickWrappedTime))
#define MYTIMEDIFF(fin, ini) (fin) - (ini)
#define START_PA_DUMP (*((volatile uint32_t *)0x4ffff014))
/* setting to 1/1000 of a second resolution by default with linux */
#ifndef TIMER_RES_DIVIDER
  #define TIMER_RES_DIVIDER (1)
#endif
...
void al_signal_start( void )
{
    ...
    GETMYTIME (&initial );
//  th_printf( "g_ui64SysTickWrappedTime = 0x%llx\n", g_ui64SysTickWrappedTime );
//  th_printf( "al_signal_start = %lld\n", initial );
#endif
}
...
uint64_t al_signal_finished( void )
{
#if CALLGRIND_RUN
    ...
    al_power_finished();
    GETMYTIME( &final );
//  th_printf( "al_signal_finished = %d\n", final );
    elapsed = (MYTIMEDIFF(final, initial));
//  th_printf( "g_ui64SysTickWrappedTime = 0x%llx\n", g_ui64SysTickWrappedTime );
//  th_printf( "initial = %lld, final = %lld, elapsed = %lld\n", initial, final, elapsed );
#if (REPORT_THMALLOC_STATS > 1)
    th_printf("Total allocated memory at signal finish: %.02fMB\n",(e_f32)th_malloc_total/(1024.0*1024.0));
#endif
    return elapsed;
...
//#error "If not using host example code for timing, you must implement a method to measure time"
    ALTIMETYPE now;
    GETMYTIME( &now );
//  th_printf( "al_tick_granulariity = %d\n", now );
    return ( now );
#endif
}
...
//{
////  return    am_sprintf(fmt,args);
//    return am_util_stdio_printf(fmt, args);
//}
