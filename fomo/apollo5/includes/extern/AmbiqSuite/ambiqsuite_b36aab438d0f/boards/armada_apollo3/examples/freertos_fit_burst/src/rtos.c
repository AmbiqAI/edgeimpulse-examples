...
setup_task(void *pvParameters)
{
    ...
    //
    // Let the Radio task run.
    //
    taskYIELD();

    //
    // Create a random time to go into burst mode 5 + u[0,2] seconds.
    //
    ui32CurrentTime = am_hal_stimer_counter_get();
    srand(ui32CurrentTime);
    randNum = rand();
    TickType_t xDelay = (5000 + (((uint64_t)2000 * randNum) / RAND_MAX) ) / portTICK_PERIOD_MS;
    am_util_stdio_printf("Apollo3 Burst Mode Delay is %f seconds\n", (float)xDelay / 1000.0);
    vTaskDelay( xDelay );
