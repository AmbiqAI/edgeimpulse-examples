...
scheduler_timer_init(void)
{
    ...
    am_hal_ctimer_config_t cTimer1Config =
    {
        .ui32Link = 1,
    ...
    //
    am_hal_ctimer_config(0, &cTimer1Config);
    //
    // Enable the timer.
    //
...
{
    uint32_t ui32CurrentTime, ui32ElapsedTime;
    static uint32_t initialRefTime = 0;
    static uint32_t initialRefTick = 0;
    ...
    if (initialRefTime == 0)
    {
        initialRefTime = am_hal_ctimer_read(0, AM_HAL_CTIMER_BOTH);
        initialRefTick = ui32CurrentTime;
        am_util_debug_printf("Initial Time: Ref:0x%x Tick:0x%x\r\n", initialRefTime, initialRefTick);
    }
    else
    {
        uint32_t curRefTime = am_hal_ctimer_read(0, AM_HAL_CTIMER_BOTH);
        am_util_debug_printf("Time: Ref:0x%x Tick:0x%x\r\n", ((curRefTime - initialRefTime) / 32), ui32CurrentTime - initialRefTick);
    }
