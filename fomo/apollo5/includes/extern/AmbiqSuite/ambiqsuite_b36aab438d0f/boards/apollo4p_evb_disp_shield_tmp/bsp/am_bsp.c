...
static am_bsp_board_info_e
am_bsp_external_board_version_get(void)
{
    uint32_t ui32Ret, ui32Value;
    static uint32_t ui32ReadValue = 0;

    static uint8_t ui8ReadOnce = 0;

    if (ui8ReadOnce == 0)
    {
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_BOARD_INFO0, g_AM_BSP_GPIO_REV3_BOARD_INFO0);
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_BOARD_INFO1, g_AM_BSP_GPIO_REV3_BOARD_INFO1);

        ui32Ret = am_hal_gpio_state_read(AM_BSP_GPIO_REV3_BOARD_INFO0, AM_HAL_GPIO_INPUT_READ, &ui32Value);
        ...
        ui32Ret = am_hal_gpio_state_read(AM_BSP_GPIO_REV3_BOARD_INFO1, AM_HAL_GPIO_INPUT_READ, &ui32Value);
    ...
    if (ui32ReadValue == 0)
    {
        return BOARD_INFO_V2;
    }
    else if (ui32ReadValue == 1)
    {
        return BOARD_INFO_V3;
    }
    else
    {
        return BOARD_INFO_ERROR;
    }
}

void
am_bsp_external_board_parameter_get(void *psParameters)
{
    am_devices_display_hw_config_t sCurrentParameter;

    // psParameters = &sCurrentParameter;

    if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
    {
        sCurrentParameter.eIC = DISP_IC_RM69330,
        sCurrentParameter.ui16ResX = 454;
        sCurrentParameter.ui16ResY = 454;
        sCurrentParameter.ui16Offset = 16;
    }
    else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
    {
        sCurrentParameter.eIC = DISP_IC_CO5300,
        sCurrentParameter.ui16ResX = 466;
        sCurrentParameter.ui16ResY = 466;
        sCurrentParameter.ui16Offset = 6;
    }
    else
    {
        return;
    ...
    ((am_devices_display_hw_config_t *)(psParameters))->eIC = sCurrentParameter.eIC;
    ((am_devices_display_hw_config_t *)(psParameters))->ui16ResX = sCurrentParameter.ui16ResX;
    ((am_devices_display_hw_config_t *)(psParameters))->ui16ResY = sCurrentParameter.ui16ResY;
    ((am_devices_display_hw_config_t *)(psParameters))->ui16Offset = sCurrentParameter.ui16Offset;
...
am_bsp_external_accel_device_enable(bool bEnable)
{
    ...
    if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
    {
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_ACCEL_DEVICE_EN, g_AM_BSP_GPIO_REV2_ACCEL_DEVICE_EN);
        am_hal_gpio_state_write(AM_BSP_GPIO_REV2_ACCEL_DEVICE_EN, eGpioWrType);
    }
    else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
    {
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_ACCEL_DEVICE_EN, g_AM_BSP_GPIO_REV3_ACCEL_DEVICE_EN);
        am_hal_gpio_state_write(AM_BSP_GPIO_REV3_ACCEL_DEVICE_EN, eGpioWrType);
    }
}
...
am_bsp_disp_pins_enable(void)
{
    ...
    switch (g_sDispCfg.eInterface)
    {
        case DISP_IF_QSPI:
            if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_QSPI_CS,  g_AM_BSP_GPIO_REV2_DISP_QSPI_CS);
            }
            else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_DISP_QSPI_CS,  g_AM_BSP_GPIO_REV3_DISP_QSPI_CS);
            }
        ...
        case DISP_IF_DSPI:
            if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_DSPI_CS,  g_AM_BSP_GPIO_REV2_DISP_DSPI_CS);
            }
            else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_DISP_DSPI_CS,  g_AM_BSP_GPIO_REV3_DISP_DSPI_CS);
            }
        ...
        case DISP_IF_SPI4:
            if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_SPI_CS,  g_AM_BSP_GPIO_REV2_DISP_SPI_CS);
            }
            else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_DISP_SPI_CS,  g_AM_BSP_GPIO_REV3_DISP_SPI_CS);
            }
    ...
    if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
    {
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_IFSEL1, g_AM_BSP_GPIO_REV2_DISP_IFSEL1);
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_IFSEL2, g_AM_BSP_GPIO_REV2_DISP_IFSEL2);
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_DEVICE_RST, g_AM_BSP_GPIO_REV2_DISP_DEVICE_RST);
    }
    else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
    {
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_DISP_DEVICE_RST, g_AM_BSP_GPIO_REV3_DISP_DEVICE_RST);
    }
    am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_DEVICE_EN, g_AM_BSP_GPIO_DISP_DEVICE_EN);
    ...
    if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
    {
        am_hal_gpio_state_write(AM_BSP_GPIO_REV2_DISP_IFSEL2, eGpioType2);
        am_hal_gpio_state_write(AM_BSP_GPIO_REV2_DISP_IFSEL1, eGpioType1);
    }
    else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
    {
...
am_bsp_disp_pins_disable(void)
{
    ...
    switch (g_sDispCfg.eInterface)
    {
        case DISP_IF_QSPI:
            if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_QSPI_CS,  am_hal_gpio_pincfg_disabled);
            }
            else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_DISP_QSPI_CS,  am_hal_gpio_pincfg_disabled);
            }
        ...
        case DISP_IF_DSPI:
            if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_DSPI_CS,  am_hal_gpio_pincfg_disabled);
            }
            else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_DISP_DSPI_CS,  am_hal_gpio_pincfg_disabled);
            }
        ...
        case DISP_IF_SPI4:
            if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_SPI_CS,  am_hal_gpio_pincfg_disabled);
            }
            else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
            {
                am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_DISP_SPI_CS,  am_hal_gpio_pincfg_disabled);
            }
    ...
    if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
    {
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_DEVICE_RST, am_hal_gpio_pincfg_disabled);
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_IFSEL1, am_hal_gpio_pincfg_disabled);
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_DISP_IFSEL2, am_hal_gpio_pincfg_disabled);
    }
    else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
    {
        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_DISP_DEVICE_RST, am_hal_gpio_pincfg_disabled);
    }
...
am_bsp_disp_reset_pin_output_set(bool bEnable)
{
    ...
    if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
    {
        am_hal_gpio_state_write(AM_BSP_GPIO_REV2_DISP_DEVICE_RST, eGpioWrType);
    }
    else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
    {
        am_hal_gpio_state_write(AM_BSP_GPIO_REV3_DISP_DEVICE_RST, eGpioWrType);
    }
        ...
        case 0:
            ...
            switch ( eMSPIDevice )
            {
                case AM_HAL_MSPI_FLASH_SERIAL_CE0:
                    if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
                    {
                        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_PSRAM_CE0, g_AM_BSP_GPIO_REV2_PSRAM_CE0);
                    }
                    else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
                    {
                        am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_PSRAM_CE0, g_AM_BSP_GPIO_REV3_PSRAM_CE0);
                    }
    ...
    case 1:
        ...
        switch ( eMSPIDevice )
        {
            case AM_HAL_MSPI_FLASH_SERIAL_CE0:
                if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
                {
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_REV2_NOR_CE0, g_AM_BSP_GPIO_REV2_NOR_CE0);
                } else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
                {
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_REV3_NOR_CE0, g_AM_BSP_GPIO_REV3_NOR_CE0);
                }
        ...
        case 0:
            ...
            switch ( eMSPIDevice )
            {
                ...
                case AM_HAL_MSPI_FLASH_HEX_DDR_CE0:
                    if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
                    {
                        *pPinnum = AM_BSP_GPIO_REV2_PSRAM_CE0;
                        *pPincfg = g_AM_BSP_GPIO_REV2_PSRAM_CE0;
                    }
                    else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
                    {
                        *pPinnum = AM_BSP_GPIO_REV3_PSRAM_CE0;
                        *pPincfg = g_AM_BSP_GPIO_REV3_PSRAM_CE0;
                    }
                    break;
                default:
                    break;
        ...
        case 1:
            ...
            switch ( eMSPIDevice )
            {
                ...
                case AM_HAL_MSPI_FLASH_OCTAL_DDR_CE0:
                    if (am_bsp_external_board_version_get () == BOARD_INFO_V2)
                    {
                        *pPinnum = AM_BSP_GPIO_REV2_NOR_CE0;
                        *pPincfg = g_AM_BSP_GPIO_REV2_NOR_CE0;
                    }
                    else if (am_bsp_external_board_version_get () == BOARD_INFO_V3)
                    {
                        *pPinnum = AM_BSP_GPIO_REV2_NOR_CE0;
