...
static void load_texture_to_psram(void)
{
    ...
    HYQiHei_font = lv_font_load("E:ambiq_HYQiHei.bin");
}

static void load_texture_to_ssram(void)
{
    memcpy(g_sHourHand.bo.base_virt, (void*)img_hour1_psram.data, img_hour1_psram.data_size);
    memcpy(g_sMinHand.bo.base_virt, (void*)img_minute1_psram.data, img_minute1_psram.data_size);
    memcpy(g_sSecHand.bo.base_virt, (void*)img_second1_psram.data, img_second1_psram.data_size);
}
...
GuiTask(void *pvParameters)
{
    ...
    lv_obj_set_style_text_font(label_ambiq, HYQiHei_font, 0);
    ...
    while (1)
    {
        ...
        else if ( current_tile == NULL || current_tile == tile0 )
        {
            if ( watchface_num == 0 )
            {
                watchface0_init();
...
#if defined(USE_BLUETOOTH)
                if(xSemaphoreTake(g_semBLEAdv, 0) == pdTRUE)
                {
                    if(lv_ble == NULL)
                    {
                        lv_ble = lv_img_create(watch_bg);
                        lv_img_set_src(lv_ble, &img_ble_psram);
                        lv_obj_align(lv_ble, LV_ALIGN_TOP_LEFT, 208, 355);
                        lv_obj_set_style_opa(lv_ble, LV_OPA_100, 0);
                        
                        lv_anim_init(&ble_anim);
                        lv_anim_set_repeat_count(&ble_anim, LV_ANIM_REPEAT_INFINITE);
                        
                        lv_anim_set_exec_cb(&ble_anim, ble_set_opa_anim);
                        lv_anim_set_values(&ble_anim, 0, 255);
                        lv_anim_set_time(&ble_anim, 1000);
                        lv_anim_set_playback_time(&ble_anim, 1000);
                        ble_anim_timeline = lv_anim_timeline_create();
                        lv_anim_timeline_add(ble_anim_timeline, 0, &ble_anim);
                    }
                    else
                    {
                        am_util_stdio_printf("\n>>>>>BLE Connection Closed<<<<<\n");
                    }
                    lv_anim_timeline_start(ble_anim_timeline);
                    am_util_stdio_printf("\n>>>>>BLE Advertisement Starts<<<<<\n");
                }
                else if(xSemaphoreTake(g_semBLECon, 0) == pdTRUE)
                {
                    lv_anim_timeline_stop(ble_anim_timeline);
                    lv_obj_set_style_opa(lv_ble, LV_OPA_100, 0);
