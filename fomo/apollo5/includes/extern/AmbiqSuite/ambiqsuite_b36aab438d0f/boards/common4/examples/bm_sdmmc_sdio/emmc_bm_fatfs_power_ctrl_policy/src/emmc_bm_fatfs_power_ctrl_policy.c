...
am_hal_card_pwr_ctrl_policy_e eEmmcCardPwrCtrlPolicy;
...
main(void)
{
    ...
    // Print the banner.
    //
    am_util_stdio_printf("\nApollo4 eMMC FatFs with power ctrl policies example\n");

    for ( eEmmcCardPwrCtrlPolicy = AM_HAL_CARD_PWR_CTRL_SDHC_OFF; eEmmcCardPwrCtrlPolicy <= AM_HAL_CARD_PWR_CTRL_SDHC_AND_CARD_OFF; eEmmcCardPwrCtrlPolicy ++ )
    {
        ...
        res = f_unmount((TCHAR const*)eMMCPath);
        if ( res == FR_OK )
        {
        ...
        am_util_stdio_printf("\nApollo4 eMMC FatFs power ctrl policy:%x \n", eEmmcCardPwrCtrlPolicy);
    }

    //
    // End banner.
    //
    am_util_stdio_printf("\nApollo4 eMMC FatFs power ctrl policy example complete \n");
