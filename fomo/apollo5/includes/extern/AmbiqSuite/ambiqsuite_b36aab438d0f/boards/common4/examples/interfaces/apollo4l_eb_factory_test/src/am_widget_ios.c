...
//! IOS_CE   to IOM0_CS
//! IOS_INT  to GPIO53 (as defined below)
//!
//! I2C:
//! IOS_SCL to IOM0_SCL
//! IOS_SDA to IOM0_SDA
...
#define HANDSHAKE_IOM_PIN        53 // GPIO used to connect the IOSINT to host side
...
#else
// #### INTERNAL END ####
    TimerCfg.ui32Compare0 = 6000000 * seconds;  // FIXME - Need to check this setting.
// #### INTERNAL BEGIN ####
#endif
// #### INTERNAL END ####
...
am_widget_iosspi_test(void *pWidget, void *pTestCfg, char *pErrStr)
{
    ...
    if (bContinuous == false)
    {
        ...
        if (AM_TEST_TIMEOUT)
        {
            timeout_check_init(AM_TEST_TIMEOUT*5);
        }
        while (1)
        {
