...
int masking_example()
{
    ...
    nema_vg_grad_set(gradient, COLOR_STOPS, mask_stops, mask_colors);
    ...
    for ( int i = 0; i < 4; i++ )
    {
        ...
        nema_mat3x3_translate(m_path, i * 75 + 5, RESX/2 - A_masks[i]->h/2); 
        ...
        nema_vg_set_mask_translation(0, RESX/2 - A_masks[i]->h/2); 
