...
//! @file
//!
//! @brief This is BLE central's application code for 2.5ms connection interval
//!        testing based on Amdtpc code.
//!
//! @{
//
// ****************************************************************************
...
enum
{
  ...
  AMDTP_SCAN_TIMER_IND,                 /*! AMDTP scanning timeout timer expired */
};

// Start from the offset to parse the received data packet, the maximum offset is 8
#define AMDTP_DATA_OFFSET_MAX              (8)

// Timer to start scanning, in millisecond unit
#define AMDTP_START_SCAN_TIMER_MS         (500)
// Timer to re-start scanning/stop scanning, in second unit
#define AMDTP_SCAN_TIMER_MIN_SEC          (10)
#define AMDTP_SCAN_TIMER_MAX_SEC          (15)
...
struct
{
    ...
    wsfTimer_t        scanTimer;                      /*! Timer for scanning after connection */
} amdtpcCb;
...
static void amdtpcScanReport(dmEvt_t *pMsg)
{
            ...
            pMsg->scanReport.pData))!=NULL)
    {
        ...
        if (pData[DM_AD_LEN_IDX] >= (ATT_128_UUID_LEN + 1))
        {
            uint8_t len = pData[DM_AD_LEN_IDX] - 1;
            pData += DM_AD_DATA_IDX;

            while ((!connect) && (len >= ATT_128_UUID_LEN))
            {
                uint8_t amdpt_uuid[ATT_128_UUID_LEN] = {ATT_UUID_AMDTP_SERVICE};

                /* Connect if AMDTP service UUID is included */
                if (memcmp(pData, amdpt_uuid, ATT_128_UUID_LEN) == 0)
                {
                    connect = TRUE;
                    break;
                }

                pData += ATT_128_UUID_LEN;
                len -= ATT_128_UUID_LEN;
            }
        }
    }
...
static void amdtpcProcMsg(dmEvt_t *pMsg)
{
    ...
    {
        // Parse the received notification packet, and validate the data content.
        case ATTC_HANDLE_VALUE_NTF:
        {
            uint8_t ref = 0;
            static uint8_t dataOffset = 0;
            attEvt_t * pAttMsg = (attEvt_t *)pMsg;

            for (uint16_t i = 0; i < pAttMsg->valueLen; i++)
            {
                ref = ((i & 0xFF) + dataOffset) % 0x100;

                // If the data content mismatch, close the connection.
                if (pAttMsg->pValue[i] != ref)
                {
                    APP_TRACE_ERR3("Comparison failed at Index %2d with Rcv Value %2.2X while expected value was %2.2X\n", i, pAttMsg->pValue[i], ref);
                    AppConnClose(pAttMsg->hdr.param);
                    break;
                }
            }

            if ( ++dataOffset == AMDTP_DATA_OFFSET_MAX )
            {
                dataOffset = 0;
            }
        }
        ...
        case DM_RESET_CMPL_IND:
        {
            ...
            // Automatically start scanning when power-up.
            amdtpcCb.autoConnect = TRUE;
            AppScanStart(amdtpcMasterCfg.discMode, amdtpcMasterCfg.scanType,
            amdtpcMasterCfg.scanDuration);
        ...
        case AMDTP_SCAN_TIMER_IND:
        {
            // will start/stop scanning when timer expires if connection established
            if ( AppConnIsOpen() )
            {
                if ( !amdtpcCb.scanning )
                {
                ...
                WsfTimerStartSec(&amdtpcCb.scanTimer, random_num(AMDTP_SCAN_TIMER_MAX_SEC, AMDTP_SCAN_TIMER_MIN_SEC));
            }
        }
        break;
        ...
        case DM_SEC_ENCRYPT_IND:
        {
            ...
            // Start timer to initiate scanning.
            WsfTimerStartMs(&amdtpcCb.scanTimer, AMDTP_START_SCAN_TIMER_MS);
        }
        break;
...
void AmdtpcHandlerInit(wsfHandlerId_t handlerId)
{
    ...
    amdtpcCb.scanTimer.handlerId = handlerId;
    amdtpcCb.scanTimer.msg.event = AMDTP_SCAN_TIMER_IND;
