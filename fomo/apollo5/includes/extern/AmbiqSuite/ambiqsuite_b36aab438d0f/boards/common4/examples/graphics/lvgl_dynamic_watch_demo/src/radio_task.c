...
#include "am_devices_cooper.c"
#include "hci_dbg_trc.c"
#include "hci_drv_cooper.c"
#include "hci_vs_cooper.c"
#include "am_util_ble_cooper.c"
...
//*****************************************************************************
//
// Event handler for customized events.
//
//*****************************************************************************
void
CustHandler(wsfEventMask_t event, wsfMsgHdr_t *pMsg)
{
    if(event & CUST_BLE_ADV_START_EVENT)
    {
        xSemaphoreGive(g_semBLEAdv);
    }
    else if(event & CUST_BLE_CONN_OPEN_EVENT)
    {
        xSemaphoreGive(g_semBLECon);
    }
}
...
exactle_stack_init(void)
{
    ...
    handlerId = WsfOsSetNextHandler(CustHandler);
    CustHandlerInit(handlerId);
}
...
#else
// Define empty functions to get rid of compiling error for non-BLE BSPs
