...
run_tasks(void)
{
    ...
    HeapRegion_t xHeapRegions[] =
    {
        ...
        { ( uint8_t * ) EXRAM_BASEADDR, EXRAM_MAX_SIZE },
        { ( uint8_t * ) MSPI_XIP_BASE_ADDRESS, MSPI_PSRAM_SIZE },
        { NULL, 0 }
    };
    ...
    {
        //The start part of ssram0 is used by HAL to hold several global data.
        { ( uint8_t * ) SSRAM0_BASEADDR+0x400, SSRAM0_MAX_SIZE-0x400 },
        { ( uint8_t * ) EXTRAM_BASEADDR, EXTRAM_MAX_SIZE },
        { ( uint8_t * ) SSRAM1_BASEADDR, SSRAM1_MAX_SIZE },
        { ( uint8_t * ) MSPI_XIP_BASE_ADDRESS, MSPI_PSRAM_SIZE },
        { NULL, 0 }
    };
#else
#error "coremark_pro run on apollo4p or apollo4b only!"
#endif
