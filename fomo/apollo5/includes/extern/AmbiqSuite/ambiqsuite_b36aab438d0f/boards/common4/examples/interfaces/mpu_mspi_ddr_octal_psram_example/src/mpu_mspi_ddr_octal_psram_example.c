...
//!
//! @defgroup mpu_mspi_ddr_octal_psram_example MPU MSPI DDR OCTAL PSRAM Example
    ...
    *(volatile uint8_t *) AM_REG_SYSCTRL_MMFSR = ui8mmfsr; //Write back MMFSR to clear logged faults
    if ((ui8mmfsr & 0x80) != 0)   //MMAR holds a valid fault address if MMARVALID = 1
    {
        if (ui32mmfar >= DSPRAM_START_ADDR && ui32mmfar < DSPRAM_END_ADDR)
        {
            ...
            am_hal_mpu_region_disable(sDSPRAM_A.ui8RegionNumber);
            am_hal_mpu_region_disable(sDSPRAM_B.ui8RegionNumber);
            am_hal_mpu_region_enable(sPSRAM.ui8RegionNumber);
            am_hal_mpu_region_enable(sSSRAM_A.ui8RegionNumber);
            am_hal_mpu_region_enable(sSSRAM_B.ui8RegionNumber);
            am_util_debug_printf("Enable MPU for PSRAM and SSRAM\n");
        }
        else if ((ui32mmfar >= SSRAM_START_ADDR && ui32mmfar < SSRAM_END_ADDR) ||
                 (ui32mmfar >= PSRAM_START_ADDR && ui32mmfar < PSRAM_END_ADDR))
        {
            am_hal_mpu_region_disable(sPSRAM.ui8RegionNumber);
            am_hal_mpu_region_disable(sSSRAM_A.ui8RegionNumber);
            am_hal_mpu_region_disable(sSSRAM_B.ui8RegionNumber);
            am_hal_mpu_region_enable(sDSPRAM_A.ui8RegionNumber);
            am_hal_mpu_region_enable(sDSPRAM_B.ui8RegionNumber);
            am_util_debug_printf("Enable MPU for Extended RAM\n");
        }
    }
}
...
am_stimer_cmpr0_isr(void)
{
    ...
    g_timer_isr_count++;
    am_util_debug_printf("\nWrite Extended RAM in STIMER ISR\n");
...
main(void)
{
    ...
    // Set the MPU regions.
    //
    am_hal_mpu_region_configure(&sSSRAM_A, false);
    am_hal_mpu_region_configure(&sSSRAM_B, false);
    am_hal_mpu_region_configure(&sPSRAM, false);
    am_hal_mpu_region_configure(&sDSPRAM_A, true);
    am_hal_mpu_region_configure(&sDSPRAM_B, true);
    ...
    //
    // Trigger MPU ISR by access Extended RAM firstly
    //
    ...
    *g_pTestCount2 = 0;
    ...
    //
    // Switch between Extended RAM and PSRAM/SSRAM
    //
    uint8_t testByte;
