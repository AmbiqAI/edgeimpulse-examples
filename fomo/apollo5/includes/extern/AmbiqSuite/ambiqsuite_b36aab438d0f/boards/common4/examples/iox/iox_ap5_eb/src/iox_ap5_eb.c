...
uint32_t g_ui32MuxOEnGpioList[] = {GPIO_MSPI0_OEn_SEL, GPIO_MSPI1_OEn_SEL0,
    ...
    GPIO_IOS_LB_PC_OEn, GPIO_TRACE_OEn_SEL, GPIO_SWD_OEn_SEL};
...
void gpio_init(void)
{
    ...
    //
    // Enable MSPI1/2 Load switch by default.  It is pulled up to SYS_VDD so can be pushpull.
    //
    am_hal_gpio_output_set(GPIO_MSPI1_LS_EN);
    am_hal_gpio_output_set(GPIO_MSPI2_LS_EN);
    am_hal_gpio_pinconfig(GPIO_MSPI1_LS_EN, am_hal_gpio_pincfg_output);
    am_hal_gpio_pinconfig(GPIO_MSPI2_LS_EN, am_hal_gpio_pincfg_output);

    //
    // SDIO0/1 RSTn do not go anywhere (U19 and U20 are de-popped, see CAYNSWS-679)
    // Configure as inputs
    //
    am_hal_gpio_pinconfig(GPIO_SDIO0_RSTn, am_hal_gpio_pincfg_input);
    am_hal_gpio_pinconfig(GPIO_SDIO1_RSTn, am_hal_gpio_pincfg_input);
    ...
    if ( mspi == 1 )
    {
        switch (eState)
        {
            ...
            case MSPI12_PC:
                ...
                write_mux_pin(GPIO_MSPI1_QUAD_SEL, 0);
                am_hal_gpio_output_clear(GPIO_MSPI1_LS_EN);
                break;
            default:
    ...
    else if ( mspi == 2 )
    {
        switch (eState)
        {
            ...
            case MSPI12_PC:
                ...
                write_mux_pin(GPIO_MSPI2_QUAD_SEL, 0);
                am_hal_gpio_output_clear(GPIO_MSPI2_LS_EN);
                break;
            default:
                am_util_stdio_printf("ERROR: config_mspi12 - state %d is invalid\n", eState);
                break;
