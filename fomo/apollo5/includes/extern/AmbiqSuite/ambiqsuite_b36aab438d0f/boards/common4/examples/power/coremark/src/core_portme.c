...
#define NSECS_PER_SEC_192MHZ    (AM_CORECLK_HZ * 2)
#ifdef TIME_64
...
#define EE_TICKS_PER_SEC_192MHZ    (NSECS_PER_SEC_192MHZ / TIMER_RES_DIVIDER)
#endif
...
secs_ret time_in_secs(CORE_TICKS ticks)
{
    ...
    {
        retval = ((secs_ret)ticks) / (secs_ret)EE_TICKS_PER_SEC_192MHZ;
    }
    else
    {
...
#elif defined(AM_PART_APOLLO4P) || defined(AM_PART_APOLLO4L) // AM_PART_APOLLO4B
// #### INTERNAL BEGIN ####
    //AM_REGVAL(AM_REG_MRAMPWRCTRL_ADDR) = 0x5; // Modify 2022-04-21
// #### INTERNAL END ####
...
// #### INTERNAL BEGIN ####
//#define PWRSW0_MASK     ((1 << 18) | (1 << 21) | (1 << 24))
//#define PWRSW0_MASK     ((1 << 20) | (1 << 23) | (1 << 26))
#if 0   // DanC PWRSW0 test settings.
#elif defined(PWRSW0_MASK)
    MCUCTRL->PWRSW0 &= ~PWRSW0_MASK;
#else
    // These settings were recommended by DanC, but actually ended up adding ~4uA.
// #### INTERNAL END ####
...
portable_init(core_portable *p, int *argc, char *argv[])
{
    ...
    if (g_eAmbClkSpd == AM_CLK_SPEED_192MHz)
    {
        am_util_stdio_printf("\n\n\n\nAmbiq Micro Coremark 192MHz test...\n\n");
    }
    else
    {
