...
int texture_framebuffer_init(void)
{
    ...
    g_sMask.bo.base_phys = SSRAM1_BASEADDR;
    g_sMask.bo.base_virt = (void *)SSRAM1_BASEADDR;
    g_sMask.bo.size      = nema_texture_size(g_sMask.format, NEMA_TEX_MIRROR,
                                        g_sMask.w, g_sMask.h);
    ...
    // load image data
    memcpy(g_sBoyImage.bo.base_virt, boy_rgba565, boy_rgba565_length);
    memcpy(g_sGirlImage.bo.base_virt, girl_rgba565, girl_rgba565_length);
