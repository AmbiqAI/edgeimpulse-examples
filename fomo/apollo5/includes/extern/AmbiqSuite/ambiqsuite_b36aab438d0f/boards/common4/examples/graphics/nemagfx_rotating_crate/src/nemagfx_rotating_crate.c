...
//!
//! @brief NemaGFX Rotating Crate Example.
//!
...
//! @{
//!
//! Purpose: This example shows a rotating crate with texture rending support. with Nema
...
#if defined (AM_PART_APOLLO4B)
    g_sCrate.bo = nema_buffer_create(sizeof(g_ui8Crate320x320RGBA565));
    if(g_sCrate.bo.base_virt == NULL)
    {
        am_util_stdio_printf("Failed to create FB!\n");
        while(1);
    }
    memcpy(g_sCrate.bo.base_virt, g_ui8Crate320x320RGBA565, sizeof(g_ui8Crate320x320RGBA565));
#else
    g_sCrate.bo = nema_buffer_create(sizeof(crate_bgr24));
    if(g_sCrate.bo.base_virt == NULL)
    {
        am_util_stdio_printf("Failed to create FB!\n");
        while(1);
    }
    memcpy(g_sCrate.bo.base_virt, crate_bgr24, sizeof(crate_bgr24));
#endif
}
