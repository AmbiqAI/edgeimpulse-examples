...
// #### INTERNAL BEGIN ####
    //
    // Internal note:
    // LPMMODE ALWAYS mode does not provide any real Coremark power savings, yet
    //  requires more attention to wait state and other settings, which can
    //  cause performance degradation when numerous flash accesses are required.
    // STANDBY mode for Coremark is a best of both worlds since it automatically
    //  handles any of the few flash accesses required by Coremark by taking it
    //  out of LPM. Thus we get most of the power benefit in a safer manner.
    //
#if 0
    CACHECTRL->FLASHCFG =
        _VAL2FLD(CACHECTRL_FLASHCFG_LPMMODE, 2)     |
        _VAL2FLD(CACHECTRL_FLASHCFG_RD_WAIT, 1)     |
        _VAL2FLD(CACHECTRL_FLASHCFG_LPM_RD_WAIT, 3) |
        _VAL2FLD(CACHECTRL_FLASHCFG_SEDELAY, 6);
#endif
// #### INTERNAL END ####
