...
// #### INTERNAL BEGIN ####
#define CACHE_HIT_RATE_TEST   0

#if CACHE_HIT_RATE_TEST

wsfTimer_t hitRateTimer;

// Every 1 second to read cache register
#define HIT_RATE_TIMER_MS           (1000)

// Count for calculate average hit rate
#define HIT_RATE_CNT_MAX            (20)

#define CACHE_HIT_RATE_TIMER_IND    (0xA1)

// BLE state for hit rat testing
typedef enum {
    BLE_STATE_IDLE,
    BLE_STATE_ADV_ENABLED,
    BLE_STATE_CONNECTED,
}ble_state_t;
...
static void throughputProcMsg(tpMsg_t *pMsg)
{
    ...
    switch(pMsg->hdr.event)
    {
        ...
        // #### INTERNAL BEGIN ####
        #if CACHE_HIT_RATE_TEST
        case CACHE_HIT_RATE_TIMER_IND:
        {
            uint32_t ret = 0;
            static uint8_t rate_cnt = 0;
            static float icache_average_total = 0;
            float icache_aver_hit_rate_total = 0;

            //Disable cache monitoring to prevent the monitor counters from further counting.
            ret = am_hal_cachectrl_control(AM_HAL_CACHECTRL_CONTROL_MONITOR_DISABLE, 0);

            uint32_t instruct_cache_total_access = CACHECTRL->IMON0;
            uint32_t instruct_cache_lookups      = CACHECTRL->IMON1;
            uint32_t instruct_cache_hits         = CACHECTRL->IMON2;
            uint32_t instruct_cache_line_hits    = CACHECTRL->IMON3;

            float icache_aver_hit_rate = ((float)instruct_cache_hits*100.0F/(float)instruct_cache_lookups);

            if(rate_cnt < HIT_RATE_CNT_MAX)
            {
                rate_cnt++;

                icache_average_total += icache_aver_hit_rate;
            }
            else
            {
                rate_cnt = 0;

                icache_aver_hit_rate_total = icache_average_total/HIT_RATE_CNT_MAX;

                if(ble_state == BLE_STATE_ADV_ENABLED)
                {
                    am_util_debug_printf("Adv state, iCache hit rate:%6.2f%%\r\n", icache_aver_hit_rate_total);
                }
                else if(ble_state == BLE_STATE_CONNECTED)
                {
                    am_util_debug_printf("Connect state, iCache hit rate:%6.2f%%\r\n", icache_aver_hit_rate_total);
                }

                icache_average_total = 0;
            }

            ret = am_hal_cachectrl_control(AM_HAL_CACHECTRL_CONTROL_MONITOR_ENABLE, 0);

            if(ret !=0 )
                am_util_debug_printf("enable monitor failed:%d\r\n", ret);

            ret = am_hal_cachectrl_control(AM_HAL_CACHECTRL_CONTROL_STATISTICS_RESET, 0);
            if(ret !=0 )
                am_util_debug_printf("reset statistics failed:%d\r\n", ret);

            if(ret !=0 )
                am_util_debug_printf("disable monitor failed:%d\r\n", ret);

            WsfTimerStartMs(&hitRateTimer, HIT_RATE_TIMER_MS);
        }
        break;
        #endif
        // #### INTERNAL END ####
        ...
        case DM_ADV_START_IND:
            ...
            {
                ble_state = BLE_STATE_ADV_ENABLED;
                am_hal_cachectrl_control(AM_HAL_CACHECTRL_CONTROL_STATISTICS_RESET, 0);
                WsfTimerStartMs(&hitRateTimer, HIT_RATE_TIMER_MS);
            }
            #endif
            // #### INTERNAL END ####
...
void ThroughputHandlerInit(wsfHandlerId_t handlerId)
{
    ...
    // #### INTERNAL BEGIN ####
    #if CACHE_HIT_RATE_TEST
    hitRateTimer.handlerId = handlerId;
    hitRateTimer.msg.event = CACHE_HIT_RATE_TIMER_IND;
    #endif
    // #### INTERNAL END ####
