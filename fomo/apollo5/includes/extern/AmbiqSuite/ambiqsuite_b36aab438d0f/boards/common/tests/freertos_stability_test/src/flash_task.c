...
//! @file flash_task.c
//!
//! @brief Task to simulate flash I/O operation using a SPI FRAM device.
//!
//*****************************************************************************
...
//*****************************************************************************
//
// Handle for Flash Task related events.
//
//*****************************************************************************
EventGroupHandle_t xFlashEventHandle;
...
// Transfer buffers
//
static unsigned char g_pucInBuffer[FLASH_TRANSFER_SIZE];
static unsigned char g_pucOutBuffer[FLASH_TRANSFER_SIZE];

//
// IOM Configuration
//
am_hal_iom_config_t FLASH_IOM_Config =
{
...
flash_ctimer_handler(void)
{
  ...
  //
  // Send event to the Flash Task.
  //
  xResult = xEventGroupSetBitsFromISR(xFlashEventHandle, 1,
                                      &xHigherPriorityTaskWoken);
  //
  // If the Flash Task is higher-priority than the context we're currently
  // running from, we should yield now and run the task.
  //
  if (xResult != pdFAIL)
  {
...
init_timer(void)
{
  ...
  //
  am_hal_ctimer_int_register(FLASH_CTIMER_INT, flash_ctimer_handler);
  ...
  //
  // Start a timer to trigger the SPI FRAM read (1 second).
  //
  am_hal_ctimer_config_single(FLASH_CTIMER, AM_HAL_CTIMER_TIMERA,
                              AM_HAL_CTIMER_LFRC_512HZ |
                                AM_HAL_CTIMER_FN_REPEAT |
                                  AM_HAL_CTIMER_INT_ENABLE);

  am_hal_ctimer_int_enable(FLASH_CTIMER_INT);

  am_hal_ctimer_period_set(FLASH_CTIMER, AM_HAL_CTIMER_TIMERA, 511, 0);


  //
  // Start the timer.
  //
  am_hal_ctimer_start(FLASH_CTIMER, AM_HAL_CTIMER_TIMERA);
...
//
//*****************************************************************************
static uint32_t flash_iom_init(uint32_t ui32IOMModule, am_hal_iom_config_t *psIOMConfig)
{
  uint32_t status = 0;
...
//*****************************************************************************
//
// Perform initial setup for the Flash task.
//
//*****************************************************************************
void
FlashTaskSetup(void)
{
  ...
  // Print the banner
  //
  am_util_debug_printf("Flash Task: setup\r\n");

  //
  // Create an event handle for our wake-up events.
  //
  xFlashEventHandle = xEventGroupCreate();

  //
  // Make sure we actually allocated space for the events we need.
  //
  while (xFlashEventHandle == NULL);

  //
  // Set up the task LED.
  //
  am_devices_led_init(&am_bsp_psLEDs[FLASH_LED]);

  //
  // Set up IOM for the SPI FRAM
  //
  flash_iom_init(FLASH_IOM, &FLASH_IOM_Config);

  //
  // Get the SPI FRAM identifiers and validate
  //
  am_devices_spifram_mt_id(FLASH_IOM, 0, &manufID, &prodID);
  if ((manufID != 0x04) && (prodID != 0x0302))
  {
    am_util_debug_printf("SPI FRAM Device not found to simulate external Flash\r\n");
  }
  ...
  //
  am_devices_spifram_mt_write_mode(FLASH_IOM, 0, g_pucOutBuffer, 0, FLASH_TRANSFER_SIZE, AM_HAL_IOM_MODE_NB, 0);
...
FlashTask(void *pvParameters)
{
  ...
  // Print the banner
  //
  am_util_debug_printf("Flash Task: running\r\n");
  ...
  while (1)
  {
    ...
    //
    xEventGroupWaitBits(xFlashEventHandle, 1, pdTRUE,
                        pdFALSE, portMAX_DELAY);

    //
    // Simulate external Flash write/read using SPI FRAM
    //
    am_devices_spifram_mt_read_mode(FLASH_IOM, 0, g_pucInBuffer, 0, FLASH_TRANSFER_SIZE, AM_HAL_IOM_MODE_NB, 0);
    ...
    if (bCompareOk)
    {
      ...
      am_devices_led_toggle(am_bsp_psLEDs, FLASH_LED);
      if (0 == ((ui32DbgCnt++) % 2))
      {
        am_util_debug_printf("\r\n");
      }
