...
#ifdef AM_MCU_APOLLO_H
//
// Statically allocate these buffers
//
uint32_t Buffer1[(sizeof(Rec_Type) + 3) / 4];
uint32_t Buffer2[(sizeof(Rec_Type) + 3) / 4];
#else
extern char     *malloc ();
Enumeration     Func_1 ();
...
#define Too_Small_Time (2*AM_HAL_CLKGEN_FREQ_MAX_HZ)
#else // AM_MCU_APOLLO_H
...
#ifdef AM_MCU_APOLLO_H
  Next_Ptr_Glob = (Rec_Pointer)Buffer1;
  Ptr_Glob = (Rec_Pointer)Buffer2;
...
#if 0//def AM_MCU_APOLLO_H
  User_Time /= AM_HAL_CLKGEN_FREQ_MAX_HZ;
#endif // AM_MCU_APOLLO_H
...
#ifdef AM_MCU_APOLLO_H
    fprintf (fstdout," ... Need %d, got %d, End_Time=%d, Begin_time=%d.\n", Too_Small_Time, User_Time, End_Time, Begin_Time);
#endif // AM_MCU_APOLLO_H
...
// For this to work, User_Time must be in seconds.
#ifdef AM_MCU_APOLLO_H
    User_Time /= AM_HAL_CLKGEN_FREQ_MAX_HZ;
    fprintf (fstdout,"AMBIQ: Run time = %d (End=%d, Begin=%d).\n", User_Time, End_Time, Begin_Time);
#endif // AM_MCU_APOLLO_H
    ...
    fprintf (fstdout,"%6.1f \n", Dhrystones_Per_Second);
#ifdef AM_MCU_APOLLO_H
    fprintf (fstdout,"Dhrystone per Seconds per MHz:              ");
    fprintf (fstdout,"%6.1f \n", Dhrystones_Per_Second / AM_HAL_CLKGEN_FREQ_MAX_HZ);
#endif // AM_MCU_APOLLO_H
    fprintf (fstdout,"\n");
  }
#ifdef AM_MCU_APOLLO_H
