...
am_bsp_external_board_version_get(void)
{
    ...
    if (ui8ReadOnce == 0)
    {
        am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_CARD_VERSION, g_AM_BSP_GPIO_DISP_CARD_VERSION);
        am_hal_gpio_state_write(AM_BSP_GPIO_DISP_CARD_VERSION, AM_HAL_GPIO_OUTPUT_CLEAR);
        am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_CARD_VERSION, am_hal_gpio_pincfg_input);
        am_util_delay_ms(10);
        am_hal_gpio_state_read(AM_BSP_GPIO_DISP_CARD_VERSION, AM_HAL_GPIO_INPUT_READ, &ui32ReadValue);
        am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_CARD_VERSION, am_hal_gpio_pincfg_disabled);
        ui8ReadOnce = 1;
    }
    if ( ui32ReadValue )
    {
...
am_bsp_disp_pins_enable(void)
{
    ...
    {
        //
        // Apollo5b_eb_revb + dual-lane MIPI display card has not connected the correct pin for TE.
    ...
    switch (g_sDispCfg.eInterface)
    {
        ...
        case DISP_IF_DSI:
            ...
            if ( am_bsp_external_board_version_get() == BOARD_INFO_V2 )
            {
                ...
                g_sDispCfg.ui16ResX = 468;
                g_sDispCfg.ui16ResY = 468;
            ...
            {
                g_sDispCfg.eIC = DISP_IC_RM69330;
                g_sDispCfg.ui16ResX = 456;
                g_sDispCfg.ui16ResY = 456;
                g_sDispCfg.ui16Offset = 14;
                am_hal_gpio_pinconfig(AM_BSP_GPIO_M2_DISP_SPI_TE,  g_AM_BSP_GPIO_M2_DISP_SPI_TE);
                am_hal_gpio_pinconfig(AM_BSP_GPIO_M2_DISP_SPI_RES, g_AM_BSP_GPIO_M2_DISP_SPI_RES);
                am_hal_gpio_pinconfig(AM_BSP_GPIO_M2_DISP_DEVICE_EN, g_AM_BSP_GPIO_M2_DISP_DEVICE_EN);
            }
