...
am_bsp_mspi_clkond4_pins_enable(uint32_t ui32Module, am_hal_mspi_device_e eMSPIDevice)
{
    ...
    case 2:
        ...
        switch ( eMSPIDevice )
        {
            ...
            case AM_HAL_MSPI_FLASH_SERIAL_CE0:
                // ##### INTERNAL BEGIN #####
                // This is a SW workaround to avoid a possible PCB/hardware issue on 4l_eb.
                // The WP(D2) and Hold(D3) pin has to maintain high level for nand flash serial mode.
                // ##### INTERNAL END #####
                am_hal_gpio_output_set(AM_BSP_GPIO_MSPI2_D3);
                am_hal_gpio_output_set(AM_BSP_GPIO_MSPI2_D2);
                am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI2_D3,  am_hal_gpio_pincfg_output);
                am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI2_D2,  am_hal_gpio_pincfg_output);
                am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI2_D1,  g_AM_BSP_GPIO_MSPI2_D1);
                am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI2_D0,  g_AM_BSP_GPIO_MSPI2_D0);
                am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI2_CE0, g_AM_BSP_GPIO_MSPI2_CE0);
                //
