...
am_bsp_debug_printf_enable(void)
{
    switch (g_ePrintInterface)
    {
        ...
        case AM_BSP_PRINT_IF_UART:
            am_bsp_uart_printf_enable(g_sCOMUART);
            break;

        case AM_BSP_PRINT_IF_BUFFERED_UART:
            am_bsp_buffered_uart_printf_enable(g_sCOMUART);
            break;
#endif

// #### INTERNAL BEGIN ####
        case AM_BSP_PRINT_IF_MEMORY:
            am_bsp_memory_printf_enable(g_psPrintfState);
            break;
// #### INTERNAL END ####
...
am_bsp_disp_spi_pins_enable(void)
{
    ...
    // workround for CS
    GPIO->PADKEY = 0x73;
    GPIO->PINCFG50 = 0x00000009;  // TE, available on GPIO10 and 50
    GPIO->PINCFG70 = 0x00280007;  // CS, 26=DC_DPI_DE, 27=DISP_CONT_CSX, 28=DC_SPI_CS_N, 29=DC_QSPI_CS_N
    GPIO->PINCFG73 = 0x00000000;
    GPIO->PINCFG74 = 0x00000009;
    GPIO->PINCFG75 = 0x00000009;
    GPIO->PINCFG78 = 0x00000009;
    GPIO->PADKEY = 0;
#else
    ...
    am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_SPI_SCX, g_AM_BSP_GPIO_DISP_SPI_SCX);
    am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_SPI_REF, g_AM_BSP_GPIO_DISP_SPI_REF);
    am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_SPI_REF, g_AM_BSP_GPIO_DISP_SPI_RES);
    am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_SPI_REF, g_AM_BSP_GPIO_DISP_SPI_TE);
#endif
...
am_bsp_disp_qspi_pins_enable(void)
{
    ...
    // workround for CS
    GPIO->PADKEY = 0x73;
    // FIXME: not found in GPIO html
    GPIO->PINCFG50 = 0x00000009;  // TE, available on GPIO10 and 50
    GPIO->PINCFG70 = 0x00290007;  // CS, 26=DC_DPI_DE, 27=DISP_CONT_CSX, 28=DC_SPI_CS_N, 29=DC_QSPI_CS_N
    GPIO->PINCFG71 = 0x00000003;  // OLED_EN, configured as a GPIO
    GPIO->PINCFG73 = 0x00000003;  // RESET, configured as a GPIO
    GPIO->PINCFG74 = 0x00000002;  // QSPI_D0
    GPIO->PINCFG75 = 0x00000002;  // QSPI_D1
    GPIO->PINCFG76 = 0x00000002;  // QSPI_D2
    GPIO->PINCFG77 = 0x00000002;  // QSPI_D3
    GPIO->PINCFG78 = 0x00000002;  // QSPI_SCK
    GPIO->PADKEY = 0;
#else
    am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_SPI_CS,  g_AM_BSP_GPIO_DISP_SPI_CS);
    am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_SPI_SD,  g_AM_BSP_GPIO_DISP_SPI_SD);
...
am_bsp_disp_dspi_pins_enable(void)
{
    ...
    // FIXME: not found in GPIO html
    GPIO->PINCFG50 = 0x00000009;  // TE, available on GPIO10 and 50
    GPIO->PINCFG70 = 0x00290007;  // CS, 26=DC_DPI_DE, 27=DISP_CONT_CSX, 28=DC_SPI_CS_N, 29=DC_QSPI_CS_N //FIXME
    GPIO->PINCFG71 = 0x00000003;  // OLED_EN, configured as a GPIO
    GPIO->PINCFG73 = 0x00000003;  // RESET, configured as a GPIO
    GPIO->PINCFG74 = 0x00000002;  // DSPI_D0
    GPIO->PINCFG75 = 0x00000009;  // SPI_DCX/DSPI_D1
    GPIO->PINCFG78 = 0x00000002;  // DSPI_SCK
    GPIO->PADKEY = 0;
#else
    am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_SPI_CS,  g_AM_BSP_GPIO_DISP_SPI_CS);
    am_hal_gpio_pinconfig(AM_BSP_GPIO_DISP_SPI_SD,  g_AM_BSP_GPIO_DISP_SPI_SD);
...
//*****************************************************************************
void
am_bsp_uart_printf_enable(void *pvHandle)
{
#if AM_BSP_ENABLE_UART
    //
...
am_bsp_mspi_pins_disable(uint32_t ui32Module, am_hal_mspi_device_e eMSPIDevice)
{
        ...
        case 0:
            switch ( eMSPIDevice )
            {
                case AM_HAL_MSPI_FLASH_SERIAL_CE0:
                    ...
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D0,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D1,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_SCK, am_hal_gpio_pincfg_default);
                    break;
                case AM_HAL_MSPI_FLASH_QUAD_CE0:
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_CE0, am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D0,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D1,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D2,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D3,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_SCK, am_hal_gpio_pincfg_default);
                    break;
                case AM_HAL_MSPI_FLASH_OCTAL_CE0:
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_CE0, am_hal_gpio_pincfg_default);
                    ...
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D2,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D3,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D4,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D5,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D6,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_D7,  am_hal_gpio_pincfg_default);
                    am_hal_gpio_pinconfig(AM_BSP_GPIO_MSPI0_SCK, am_hal_gpio_pincfg_default);
                    break;
                default:
                    break;
    ...
    case 1:
        return;
#if 0  // FIXME - Need to define MSPI1 pins in bsp_pins.src.
        switch ( eMSPIDevice )
        {
            case AM_HAL_MSPI_FLASH_SERIAL_CE0:
    ...
    case 2:
        return;
#if 0 // Need to define MSPI2 pins in bsp_pins.src.
        switch ( eMSPIDevice )
        {
            case AM_HAL_MSPI_FLASH_SERIAL_CE0:
